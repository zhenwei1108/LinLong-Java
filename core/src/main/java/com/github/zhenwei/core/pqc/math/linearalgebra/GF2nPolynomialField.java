...
/**
 * This class implements the abstract class <tt>GF2nField</tt> for polynomial representation. It
 * computes the field polynomial and the squaring matrix. GF2nField is used by GF2nPolynomialElement
 * which implements the elements of this field.
 *
 * @see GF2nField
...
public class GF2nPolynomialField
  ...
  /**
   * constructs an instance of the finite field with 2<sup>deg</sup> elements and characteristic 2.
   *
   ...
   * @param random source of randomness for generating new polynomials.
   * @param file   true if you want to read the field polynomial from the file false if you want to
   *               use a random fielpolynomial (this can take very long for huge degrees)
   */
  ...
  /**
   * Returns true if the field polynomial is a trinomial. The coefficient can be retrieved using
   * getTc().
   *
  ...
  /**
   * Returns true if the field polynomial is a pentanomial. The coefficients can be retrieved using
   * getPc().
   *
  ...
  /**
   * Returns the degree of the middle coefficient of the used field trinomial (x^n + x^(getTc()) +
   * 1).
   *
  ...
  /**
   * Returns the degree of the middle coefficients of the used field pentanomial (x^n +
   * x^(getPc()[2]) + x^(getPc()[1]) + x^(getPc()[0]) + 1).
   *
  ...
  /**
   * Tests all trinomials of degree (n+1) until a irreducible is found and stores the result in
   * <i>field polynomial</i>. Returns false if no irreducible trinomial exists in GF(2^n). This can
   * take very long for huge degrees.
   *
  ...
  /**
   * Tests all pentanomials of degree (n+1) until a irreducible is found and stores the result in
   * <i>field polynomial</i>. Returns false if no irreducible pentanomial exists in GF(2^n). This
   * can take very long for huge degrees.
   *
  ...
  /**
   * Tests random polynomials of degree (n+1) until an irreducible is found and stores the result in
   * <i>field polynomial</i>. This can take very long for huge degrees.
   *
   * @return true
   */
