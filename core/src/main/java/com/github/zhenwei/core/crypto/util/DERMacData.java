...
/**
 * Builder and holder class for preparing SP 800-56A compliant MacData. Elements in the data are
 * encoded as DER objects with empty octet strings used to represent nulls in compulsory fields.
 */
public final class DERMacData {
  ...
  public static final class Builder {
    ...
    private byte[] concatenate(byte[] header, byte[] id1, byte[] id2, byte[] ed1, byte[] ed2,
        byte[] text) {
      return Arrays.concatenate(Arrays.concatenate(header, id1, id2),
          Arrays.concatenate(ed1, ed2, text));
    }
  }
