...
/**
 * This class implements an ASN.1 encoded Rainbow public key. The ASN.1 definition of this structure
 * is:
 * <pre>
...
public class RainbowPublicKey
  ...
  private RainbowPublicKey(ASN1Sequence seq) {
    ...
    for (int quadSize = 0; quadSize < asnCoeffQuad.size(); quadSize++) {
      coeffQuadratic[quadSize] = ASN1OctetString.getInstance(asnCoeffQuad.getObjectAt(quadSize))
          .getOctets();
    }
    ...
    for (int singSize = 0; singSize < asnCoeffSing.size(); singSize++) {
      coeffSingular[singSize] = ASN1OctetString.getInstance(asnCoeffSing.getObjectAt(singSize))
          .getOctets();
    }
  ...
  public RainbowPublicKey(int docLength, short[][] coeffQuadratic, short[][] coeffSingular,
      short[] coeffScalar) {
