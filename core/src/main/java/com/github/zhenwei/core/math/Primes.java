package com.github.zhenwei.core.math;
...
public abstract class Primes {
  ...
  /**
   * Used to return the output from the {@linkplain org.bouncycastle.math.Primes#enhancedMRProbablePrimeTest(BigInteger,
   * SecureRandom, int) Enhanced Miller-Rabin Probabilistic Primality Test}
   */
  ...
  /**
   * Used to return the output from the {@linkplain org.bouncycastle.math.Primes#generateSTRandomPrime(Digest,
   * int, byte[]) Shawe-Taylor Random_Prime Routine}
   */
   ...
   *
   * @param hash      the {@link Digest} instance to use (as "Hash()"). Cannot be null.
   * @param length    the length (in bits) of the prime to be generated. Must be at least 2.
   * @param inputSeed the seed to be used for the generation of the requested prime. Cannot be null
   *                  or empty.
   ...
   * information about a composite candidate, which may be useful when generating or validating RSA
   * moduli.
   *
   ...
   */
  public static MROutput enhancedMRProbablePrimeTest(BigInteger candidate, SecureRandom random,
      int iterations) {
   ...
   *
   * @param candidate the {@link BigInteger} instance to test for division by small factors.
   ...
   * (so <code>candidate</code> is definitely NOT prime), or else <code>true</code> (indicating
   * primality with some probability dependent on the number of iterations that were performed).
   */
  public static boolean isMRProbablePrime(BigInteger candidate, SecureRandom random,
      int iterations) {
    checkCandidate(candidate, "candidate");
  ...
  private static boolean implMRProbablePrimeToBase(BigInteger w, BigInteger wSubOne, BigInteger m,
      int a, BigInteger b) {
    BigInteger z = b.modPow(m, w);
