 ...
 * <p/>
 * BLAKE2xs offers a built-in keying mechanism to be used directly for authentication ("Prefix-MAC")
 * rather than a HMAC construction.
 * <p/>
 * BLAKE2xs offers a built-in support for a salt for randomized hashing and a personal string for
 * defining a unique hash function for each application.
 * <p/>
 * BLAKE2xs is optimized for 32-bit platforms and produces digests of any size between 1 and 2^16-2
 * bytes. The length can also be unknown and then the maximum length will be 2^32 blocks of 32
 * bytes.
 */
public class Blake2xsDigest
  ...
  /**
   * Overall position of the digest. It is useful when the length is known in advance to get last
   * block length.
   */
  private int digestPos = 0;

  /**
   * Keep track of the round number to detect the end of the digest after 2^32 blocks of 32 bytes.
   */
   ...
   *
   * @param digestBytes     The desired digest length in bytes. Must be above 1 and less than
   *                        2^16-1
   * @param key             A key up to 32 bytes or null
  ...
  /**
   * Return the size in bytes of the internal buffer the digest applies its compression function
   * to.
   *
  ...
  /**
   * Reset the digest back to its initial state. The key, the salt and the personal string will
  ...
  /**
   * Close the digest, producing the final digest value. The doFinal() call leaves the digest reset.
   * Key, salt, personal string remain.
   *
  ...
  /**
   * Start outputting the results of the final calculation for this digest. Unlike doFinal, this
   * method will continue producing output until the Xof is explicitly reset, or signals otherwise.
   *
   * @param out    output array to write the output bytes to.
   * @param outOff offset to start writing the bytes at.
   * @param outLen the number of output bytes requested.
  ...
  public int doOutput(byte[] out, int outOff, int outLen) {
    ...
    for (int i = 0; i < outLen; i++) {
      if (bufPos >= Blake2xsDigest.DIGEST_LENGTH) {
        Blake2sDigest h = new Blake2sDigest(computeStepLength(), Blake2xsDigest.DIGEST_LENGTH,
            nodeOffset);
