 ...
 * <br> Initialising the engine with {@link SkeinParameters} allows standard and arbitrary
 * parameters to be applied during the Skein hash function.
 * <p>
 * Implemented:
 * <ul>
...
public class SkeinEngine
   ...
   *
   * @param blockSizeBits  the internal state size in bits - one of {@link #SKEIN_256}, {@link
   *                       #SKEIN_512} or {@link #SKEIN_1024}.
   * @param outputSizeBits the output/digest size to produce in bits, which must be an integral
   *                       number of bytes.
   */
  ...
  private void createInitialState() {
    long[] precalc = (long[]) INITIAL_STATES.get(
        variantIdentifier(getBlockSize(), getOutputSize()));
