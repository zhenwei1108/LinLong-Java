...
/**
 * ISO 9796-1 padding. Note in the light of recent results you should only use this with RSA (rather
 * than the "simpler" Rabin keys) and you should never use it with anything other than a hash (ie.
 * even if the message is small don't sign the message, sign it's hash) or some "random" value. See
...
public class ISO9796d1Encoding
  ...
  /**
   * return the input block size. The largest message we can process is (key_size_in_bits + 3)/16,
   * which in our world comes to key_size_in_bytes / 2.
   */
