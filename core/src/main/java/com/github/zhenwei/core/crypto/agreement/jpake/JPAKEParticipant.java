 ...
 * rather just two <i>participants</i>. An instance of {@link JPAKEParticipant} represents one
 * participant, and is the primary interface for executing the exchange.
 * <p>
 * To execute an exchange, construct a {@link JPAKEParticipant} on each end, and call the following
 * 7 methods (once and only once, in the given order, for each participant, sending messages between
 * them as described):
 * <ol>
...
public class JPAKEParticipant {
  ...
  /**
   * Unique identifier of this participant. The two participants in the exchange must NOT share the
   * same id.
   */
  private final String participantId;

  /**
   * Shared secret.  This only contains the secret between construction and the call to {@link
   * #calculateKeyingMaterial()}.
   * <p>
   * i.e. When {@link #calculateKeyingMaterial()} is called, this buffer overwritten with 0's, and
  ...
  /**
   * Convenience constructor for a new {@link JPAKEParticipant} that uses the {@link
   * JPAKEPrimeOrderGroups#NIST_3072} prime order group, a SHA-256 digest, and a default {@link
   * SecureRandom} implementation.
   * <p>
   * After construction, the {@link #getState() state} will be  {@link #STATE_INITIALIZED}.
   *
   * @param participantId unique identifier of this participant. The two participants in the
   *                      exchange must NOT share the same id.
   * @param password      shared secret. A defensive copy of this array is made (and cleared once
   *                      {@link #calculateKeyingMaterial()} is called). Caller should clear the
   *                      input password as soon as possible.
  ...
  /**
   * Convenience constructor for a new {@link JPAKEParticipant} that uses a SHA-256 digest and a
   * default {@link SecureRandom} implementation.
   * <p>
   * After construction, the {@link #getState() state} will be  {@link #STATE_INITIALIZED}.
   *
   * @param participantId unique identifier of this participant. The two participants in the
   ...
   *                      {@link #calculateKeyingMaterial()} is called). Caller should clear the
   *                      input password as soon as possible.
   * @param group         prime order group. See {@link JPAKEPrimeOrderGroups} for standard groups
   * @throws NullPointerException     if any argument is null
   * @throws IllegalArgumentException if password is empty
   */
  public JPAKEParticipant(
   ...
   *                      input password as soon as possible.
   * @param group         prime order group. See {@link JPAKEPrimeOrderGroups} for standard groups
   * @param digest        digest to use during zero knowledge proofs and key confirmation (SHA-256
   *                      or stronger preferred)
   * @param random        source of secure random data for x1 and x2, and for the zero knowledge
   *                      proofs
   * @throws NullPointerException     if any argument is null
   * @throws IllegalArgumentException if password is empty
   */
  ...
  /**
   * Gets the current state of this participant. See the <tt>STATE_*</tt> constants for possible
   * values.
   */
  public int getState() {
    return this.state;
  ...
  public JPAKERound1Payload createRound1PayloadToSend() {
    ...
    BigInteger[] knowledgeProofForX1 = JPAKEUtil.calculateZeroKnowledgeProof(p, q, g, gx1, x1,
        participantId, digest, random);
    BigInteger[] knowledgeProofForX2 = JPAKEUtil.calculateZeroKnowledgeProof(p, q, g, gx2, x2,
        participantId, digest, random);

    this.state = STATE_ROUND_1_CREATED;

    return new JPAKERound1Payload(participantId, gx1, gx2, knowledgeProofForX1,
        knowledgeProofForX2);
  }
  ...
  public void validateRound1PayloadReceived(JPAKERound1Payload round1PayloadReceived)
    ...
    if (this.state >= STATE_ROUND_1_VALIDATED) {
      throw new IllegalStateException(
          "Validation already attempted for round1 payload for" + participantId);
    }
   ...
   * <p>
   * {@link #validateRound1PayloadReceived(JPAKERound1Payload)} must be called prior to this
   * method.
   * <p>
   * After execution, the {@link #getState() state} will be  {@link #STATE_ROUND_2_CREATED}.
   *
   * @throws IllegalStateException if called prior to {@link #validateRound1PayloadReceived(JPAKERound1Payload)},
   *                               or multiple times
   */
  public JPAKERound2Payload createRound2PayloadToSend() {
    ...
    BigInteger[] knowledgeProofForX2s = JPAKEUtil.calculateZeroKnowledgeProof(p, q, gA, A, x2s,
        participantId, digest, random);
   ...
   * <p>
   * Note that this DOES NOT detect a non-common password. The only indication of a non-common
   * password is through derivation of different keys (which can be detected explicitly by executing
   * round 3 and round 4)
   * <p>
  ...
  public void validateRound2PayloadReceived(JPAKERound2Payload round2PayloadReceived)
    ...
    if (this.state >= STATE_ROUND_2_VALIDATED) {
      throw new IllegalStateException(
          "Validation already attempted for round2 payload for" + participantId);
    }
    if (this.state < STATE_ROUND_1_VALIDATED) {
      throw new IllegalStateException(
          "Round1 payload must be validated prior to validating Round2 payload for "
              + this.participantId);
    }
    ...
    JPAKEUtil.validateParticipantIdsEqual(this.partnerParticipantId,
        round2PayloadReceived.getParticipantId());
   ...
   * externally (i.e. not by {@link JPAKEParticipant}).
   * <p>
   ...
   * JPAKEParticipant} for details on how to execute rounds 3 and 4.
   * <p>
   * The keying material will be in the range <tt>[0, p-1]</tt>.
   * <p>
   * {@link #validateRound2PayloadReceived(JPAKERound2Payload)} must be called prior to this
   * method.
   * <p>
   * As a side effect, the internal {@link #password} array is cleared, since it is no longer
   * needed.
   * <p>
   ...
   * @throws IllegalStateException if called prior to {@link #calculateKeyingMaterial()}, or
   *                               multiple times
   */
  public JPAKERound3Payload createRound3PayloadToSend(BigInteger keyingMaterial) {
    ...
    if (this.state < STATE_KEY_CALCULATED) {
      throw new IllegalStateException(
          "Keying material must be calculated prior to creating Round3 payload for "
              + this.participantId);
    }
   ...
   *                               multiple times
   */
  public void validateRound3PayloadReceived(JPAKERound3Payload round3PayloadReceived,
      BigInteger keyingMaterial)
      throws CryptoException {
    if (this.state >= STATE_ROUND_3_VALIDATED) {
      throw new IllegalStateException(
          "Validation already attempted for round3 payload for" + participantId);
    }
    if (this.state < STATE_KEY_CALCULATED) {
      throw new IllegalStateException(
    ...
    JPAKEUtil.validateParticipantIdsEqual(this.partnerParticipantId,
        round3PayloadReceived.getParticipantId());
