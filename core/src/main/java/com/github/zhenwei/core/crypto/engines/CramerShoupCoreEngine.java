...
/**
 * Essentially the Cramer-Shoup encryption / decryption algorithms according to "A practical public
 * key cryptosystem provably secure against adaptive chosen ciphertext attack." (Crypto 1998)
 */
public class CramerShoupCoreEngine {
  ...
  /**
   * Return the maximum size for an input block to this engine. For Cramer Shoup this is always one
   * byte less than the key size on encryption, and the same length as the key size on decryption.
   * TODO: correct?
   *
  ...
  /**
   * Return the maximum size for an output block to this engine. For Cramer Shoup this is always one
   * byte less than the key size on decryption, and the same length as the key size on encryption.
   * TODO: correct?
   *
  ...
  public byte[] convertOutput(BigInteger result) {
    ...
    if (!forEncryption) {
      if (output[0] == 0 && output.length
          > getOutputBlockSize()) { // have ended up with an extra zero byte, copy down.
      ...
      if (output.length
          < getOutputBlockSize()) {// have ended up with less bytes than normal, lengthen
