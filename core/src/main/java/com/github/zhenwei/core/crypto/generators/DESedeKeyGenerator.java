...
public class DESedeKeyGenerator
  ...
  /**
   * initialise the key generator - if strength is set to zero the key generated will be 192 bits in
   * size, otherwise strength can be 128 or 192 (or 112 or 168 if you don't count parity bits),
   * depending on whether you wish to do 2-key or 3-key triple DES.
   *
  ...
  public byte[] generateKey() {
    ...
    while (++count < MAX_IT && (DESedeParameters.isWeakKey(newKey, 0, newKey.length)
        || !DESedeParameters.isRealEDEKey(newKey, 0)));

    if (DESedeParameters.isWeakKey(newKey, 0, newKey.length) || !DESedeParameters.isRealEDEKey(
        newKey, 0)) {
