...
public class ObjectStoreData
  ...
  public ObjectStoreData(AlgorithmIdentifier integrityAlgorithm, Date creationDate,
      Date lastModifiedDate, ObjectDataSequence objectDataSequence, String comment) {
  ...
  private ObjectStoreData(ASN1Sequence seq) {
    ...
    this.comment =
        (seq.size() == 6) ? ASN1UTF8String.getInstance(seq.getObjectAt(5)).getString() : null;
  }
