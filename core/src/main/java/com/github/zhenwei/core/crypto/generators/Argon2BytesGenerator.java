...
/**
 * Argon2 PBKDF - Based on the results of https://password-hashing.net/ and
...
public class Argon2BytesGenerator {
  ...
  public void init(Argon2Parameters parameters) {
    ...
    if (parameters.getLanes() < Argon2BytesGenerator.MIN_PARALLELISM) {
      ...
      throw new IllegalStateException(
          "memory is less than: " + (2 * parameters.getLanes()) + " expected " + (2
              * parameters.getLanes()));
  ...
  public int generateBytes(char[] password, byte[] out, int outOff, int outLen) {
    return generateBytes(parameters.getCharToByteConverter().convert(password), out, outOff,
        outLen);
  }
  ...
  private void doInit(Argon2Parameters parameters) {
    ...
    this.segmentLength =
        memoryBlocks / (parameters.getLanes() * Argon2BytesGenerator.ARGON2_SYNC_POINTS);
    this.laneLength = segmentLength * Argon2BytesGenerator.ARGON2_SYNC_POINTS;

    /* Ensure that all segments have equal length */
    memoryBlocks =
        segmentLength * (parameters.getLanes() * Argon2BytesGenerator.ARGON2_SYNC_POINTS);
  ...
  private void fillSegment(FillBlock filler, Position position) {
    ...
    for (int index = startingIndex; index < segmentLength; ++index) {
      long pseudoRandom = getPseudoRandom(filler, index, addressBlock, inputBlock, prevOffset,
  ...
  private void initAddressBlocks(FillBlock filler, Position position, Block inputBlock,
      Block addressBlock) {
  ...
  private long getPseudoRandom(FillBlock filler, int index, Block addressBlock, Block inputBlock,
      int prevOffset,
...
//    }
  ...
  private void initialize(byte[] tmpBlockBytes, byte[] password, int outputLength) {
    ...
    int[] values = {parameters.getLanes(), outputLength, parameters.getMemory(),
