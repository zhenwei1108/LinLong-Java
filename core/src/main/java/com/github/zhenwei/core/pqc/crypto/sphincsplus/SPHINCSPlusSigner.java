 ...
 * implementation on github as at November 9th 2021. This version includes the changes for the
 * countermeasure for the long-message second preimage attack - see "https://github.com/sphincs/sphincsplus/commit/61cd2695c6f984b4f4d6ed675378ed9a486cbede"
 * for further details.
 * </p>
 */
...
//        # Output: SPHINCS+ signature SIG
    ...
    for (int i = 0; i != sig_fors.length; i++) {
      sigComponents[1 + i] = Arrays.concatenate(sig_fors[i].sk,
          Arrays.concatenate(sig_fors[i].authPath));
    }
  ...
  public boolean verifySignature(byte[] message, byte[] signature) {
    ...
    ADRS adrs = new ADRS();
    SIG sig = new SIG(engine.N, engine.K, engine.A, engine.D, engine.H_PRIME, engine.WOTS_LEN,
        signature);
