...
/**
 * This class provides some default behavior and common implementation for a X500NameStyle. It
 * should be easily extendable to support implementing the desired X500NameStyle.
 */
public abstract class AbstractX500NameStyle
  ...
  /**
   * For all string values starting with '#' is assumed, that these are already valid ASN.1 objects
   * encoded in hex.
   * <p>
   * All other string values are send to {@link AbstractX500NameStyle#encodeStringValue(ASN1ObjectIdentifier,
   * String)}.
   * </p>
   * Subclasses should overwrite {@link AbstractX500NameStyle#encodeStringValue(ASN1ObjectIdentifier,
   * String)} to change the encoding of specific types.
   *
  ...
  public boolean areEqual(X500Name name1, X500Name name2) {
    ...
    if (rdns1[0].getFirst() != null && rdns2[0].getFirst() != null) {
      reverse = !rdns1[0].getFirst().getType()
          .equals(rdns2[0].getFirst().getType());  // guess forward
    }
