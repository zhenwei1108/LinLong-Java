...
public abstract class WNafUtil {
   ...
   *              number <code>w</code>, such that for any
   ...
   *
   * @param bits              the bit-length of the scalar to multiply by
   * @param windowSizeCutoffs a monotonically increasing list of bit sizes at which to increment the
   *                          window width
   * @return the window size to use
   */
  ...
  public static WNafPreCompInfo precompute(final ECPoint p, final int minWidth,
    ...
    return (WNafPreCompInfo) c.precompute(p, PRECOMP_NAME, new PreCompCallback() {
                ...
                if (!twiceP.isInfinity() && ECAlgorithms.isFpCurve(c) && c.getFieldSize() >= 64) {
                  switch (c.getCoordinateSystem()) {
                    ...
                    case ECCurve.COORD_JACOBIAN_MODIFIED: {
                      ...
                      isoTwiceP = c.createPoint(twiceP.getXCoord().toBigInteger(),
                          twiceP.getYCoord()
                              .toBigInteger());
      ...
      private boolean checkExisting(WNafPreCompInfo existingWNaf, int width, int reqPreCompLen,
  ...
  public static WNafPreCompInfo precomputeWithPointMap(final ECPoint p, final ECPointMap pointMap,
