...
public class NTRUSigningKeyPairGenerator
   ...
   */
  private void minimizeFG(IntegerPolynomial f, IntegerPolynomial g, IntegerPolynomial F,
      IntegerPolynomial G, int N) {
    int E = 0;
    for (int j = 0; j < N; j++) {
  ...
  private FGBasis generateBasis() {
    ...
    do {
      do {
        f = params.polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_SIMPLE
            ? DenseTernaryPolynomial.generateRandom(N, d + 1, d,
            CryptoServicesRegistrar.getSecureRandom())
            : ProductFormPolynomial.generateRandom(N, d1, d2, d3 + 1, d3,
                CryptoServicesRegistrar.getSecureRandom());
    ...
    if (params.keyGenAlg == NTRUSigningKeyGenerationParameters.KEY_GEN_ALG_RESULTANT) {
      ...
      C = fRev.mult(
          B);   // fRev.mult(B) is actually faster than new SparseTernaryPolynomial(fRev).mult(B), possibly due to cache locality?
      C.add(gRev.mult(A));
      C = C.mult(rt.rho);
      C.div(rt.res);
      ...
      BigDecimalPolynomial fInv = rf.rho.div(new BigDecimal(rf.res),
          B.getMaxCoeffLength() + 1 + log10N);
      BigDecimalPolynomial gInv = rg.rho.div(new BigDecimal(rg.res),
          A.getMaxCoeffLength() + 1 + log10N);
  ...
  /**
   * Creates a basis such that <code>|F| &lt; keyNormBound</code> and <code>|G| &lt;
   * keyNormBound</code>
   *
  ...
  /**
   * A subclass of Basis that additionally contains the polynomials <code>F</code> and
   * <code>G</code>.
   */
  public class FGBasis
    ...
    FGBasis(Polynomial f, Polynomial fPrime, IntegerPolynomial h, IntegerPolynomial F,
        IntegerPolynomial G, NTRUSigningKeyGenerationParameters params) {
