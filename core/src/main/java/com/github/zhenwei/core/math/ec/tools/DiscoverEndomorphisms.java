...
//        System.out.println("[" + curveName + "]");
  ...
  private static void discoverEndomorphisms(X9ECParameters x9, String displayName) {
    ...
    if (ECAlgorithms.isFpCurve(c)) {
      ...
      if (c.getB().isZero() && characteristic.mod(ECConstants.FOUR).equals(ECConstants.ONE)) {
        System.out.println(
            "Curve '" + displayName + "' has a 'GLV Type A' endomorphism with these parameters:");
        printGLVTypeAParameters(x9);
      }

      if (c.getA().isZero() && characteristic.mod(ECConstants.THREE).equals(ECConstants.ONE)) {
        System.out.println(
            "Curve '" + displayName + "' has a 'GLV Type B' endomorphism with these parameters:");
  ...
  private static void printGLVTypeAParameters(X9ECParameters x9, BigInteger lambda,
      ECFieldElement[] iValues) {
    /*
  ...
  private static void printGLVTypeBParameters(X9ECParameters x9, BigInteger lambda,
      ECFieldElement[] betaValues) {
    /*
  ...
  private static void printScalarDecompositionParameters(BigInteger n, BigInteger lambda) {
    ...
    v2 = chooseShortest(new BigInteger[]{rt[0], rt[1].negate()},
        new BigInteger[]{rt[4], rt[5].negate()});
    ...
    if (!isVectorBoundedBySqrt(v2, n) && areRelativelyPrime(v1[0], v1[1])) {
      ...
      if (vw != null) {
        ...
        if (range != null) {
          for (BigInteger alpha = range[0]; alpha.compareTo(range[1]) <= 0;
  ...
  private static BigInteger[] solveQuadraticEquation(BigInteger n, BigInteger a, BigInteger b,
      BigInteger c) {
  ...
  private static ECFieldElement[] findNonTrivialOrder3FieldElements(ECCurve c) {
    ...
    BigInteger b;
    do {
      BigInteger r = BigIntegers.createRandomInRange(ECConstants.TWO, q.subtract(ECConstants.TWO),
          random);
      b = r.modPow(e, q);
    }
  ...
  private static ECFieldElement[] findNonTrivialOrder4FieldElements(ECCurve c) {
    ...
    if (i == null) {
      throw new IllegalStateException(
          "Calculation of non-trivial order-4  field elements failed unexpectedly");
    }
  ...
  private static BigInteger modSqrtComplex(BigInteger x, BigInteger q, BigInteger p,
      BigInteger pSub1Halved) {
