...
/**
 * EC-NR as described in IEEE 1363-2000 - a signature algorithm for Elliptic Curve which also offers
 * message recovery.
 */
public class ECNRSigner
   ...
   *
   * @param forSigning true if we are generating a signature, false for verification or if we want
   *                   to use the signer for message recovery.
  ...
  public BigInteger[] generateSignature(
    ...
    do // generate r
    {
      ...
      BigInteger Vx = V.getQ().getAffineXCoord()
          .toBigInteger();                    // get the point's x coordinate
  ...
  /**
   * Returns the data used for the signature generation, assuming the public key passed to init() is
   * correct.
   *
