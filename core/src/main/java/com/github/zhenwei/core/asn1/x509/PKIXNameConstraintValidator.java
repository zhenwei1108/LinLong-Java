...
public class PKIXNameConstraintValidator
   ...
   *
   * @param name The GeneralName.
   * @throws NameConstraintValidatorException If the <code>name</code> is excluded.
   */
  public void checkExcluded(GeneralName name)
    ...
    switch (name.getTagNo()) {
      ...
      case GeneralName.uniformResourceIdentifier:
        ...
        break;
      case GeneralName.iPAddress:
        checkExcludedIP(excludedSubtreesIP,
            ASN1OctetString.getInstance(name.getName()).getOctets());
        break;
      default:
        // other tags to be ignored.
  ...
  /**
   * Updates the permitted set of these name constraints with the intersection with the given
   * subtree.
   *
  ...
  public boolean equals(Object o) {
    ...
    return collectionsAreEqual(constraintValidator.excludedSubtreesDN, excludedSubtreesDN)
        ...
        && collectionsAreEqual(constraintValidator.excludedSubtreesOtherName,
        ...
        && collectionsAreEqual(constraintValidator.permittedSubtreesOtherName,
  ...
  private static boolean withinDNSubtree(
    ...
    for (int j = 0; j < subtree.size(); j++) {
      ...
      if (subtreeRdn.size() == dnsRdn.size()) {
        ...
        if (subtreeRdn.size() == 1 && subtreeRdn.getFirst().getType()
            .equals(RFC4519Style.serialNumber)) {
          if (!dnsRdn.getFirst().getValue().toString()
   ...
   *
   * @param excluded A <code>Set</code> of excluded IP addresses with their subnet mask as byte
   *                 arrays.
   ...
   * @param ipWithSubmask1 The first IP address with its subnet mask.
   * @param ipWithSubmask2 The second IP address with its subnet mask.
   * @return A <code>Set</code> with the single IP address with its subnet mask as a byte array or
   * an empty <code>Set</code>.
   */
   ...
   * @return An array with two elements. Each element contains the IP address and the subnet mask in
   * this order.
   */
  ...
  /**
   * Based on the two IP addresses and their subnet masks the IP range is computed for each IP
   * address - subnet mask pair and returned as the minimum IP address and the maximum address of
   * the range.
   *
   ...
   * @return A array with two elements. The first/second element contains the min and max IP address
   * of the first/second IP address and its subnet mask.
   */
  ...
  /**
   * The common part of <code>email1</code> and <code>email2</code> is added to the union
   * <code>union</code>. If <code>email1</code> and
  ...
  /**
   * Compares IP address <code>ip1</code> with <code>ip2</code>. If ip1 is equal to ip2 0 is
   * returned. If ip1 is bigger 1 is returned, -1 otherwise.
   *
   * @param ip1 The first IP address.
   * @param ip2 The second IP address.
