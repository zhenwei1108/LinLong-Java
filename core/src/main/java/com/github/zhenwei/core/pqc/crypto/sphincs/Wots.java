...
class Wots {
  ...
  static void gen_chain(HashFunctions hs, byte[] out, int outOff, byte[] seed, int seedOff,
    ...
    for (i = 0; i < chainlen && i < WOTS_W; i++) {
      hs.hash_n_n_mask(out, outOff, out, outOff, masks,
          masksOff + (i * SPHINCS256Config.HASH_BYTES));
    }
  }


  void wots_pkgen(HashFunctions hs, byte[] pk, int pkOff, byte[] sk, int skOff, byte[] masks,
      int masksOff) {
    int i;
    expand_seed(pk, pkOff, sk, skOff);
    for (i = 0; i < WOTS_L; i++) {
      gen_chain(hs, pk, pkOff + i * SPHINCS256Config.HASH_BYTES, pk,
          pkOff + i * SPHINCS256Config.HASH_BYTES, masks, masksOff, WOTS_W - 1);
    }
  }
  ...
  void wots_sign(HashFunctions hs, byte[] sig, int sigOff, byte[] msg, byte[] sk, byte[] masks) {
    ...
    for (i = 0; i < WOTS_L; i++) {
      gen_chain(hs, sig, sigOff + i * SPHINCS256Config.HASH_BYTES, sig,
          sigOff + i * SPHINCS256Config.HASH_BYTES, masks, 0, basew[i]);
    }
  }
  ...
  void wots_verify(HashFunctions hs, byte[] pk, byte[] sig, int sigOff, byte[] msg, byte[] masks) {
    ...
    for (i = 0; i < WOTS_L; i++) {
      gen_chain(hs, pk, i * SPHINCS256Config.HASH_BYTES, sig,
          sigOff + i * SPHINCS256Config.HASH_BYTES, masks, (basew[i] * SPHINCS256Config.HASH_BYTES),
          WOTS_W - 1 - basew[i]);
    }
  }
}