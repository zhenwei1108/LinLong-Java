...
/**
 * This class implements the Fujisaki/Okamoto conversion of the McEliecePKCS. Fujisaki and Okamoto
 * propose hybrid encryption that merges a symmetric encryption scheme which is secure in the
 * find-guess model with an asymmetric one-way encryption scheme which is sufficiently probabilistic
 * to obtain a public key cryptosystem which is CCA2-secure. For details, see D. Engelbert, R.
 * Overbeck, A. Schmidt, "A Summary of McEliece-Type Cryptosystems and their Security", technical
 * report. https://www.degruyter.com/document/doi/10.1515/JMC.2007.009/html
 */
public class McElieceFujisakiCipher
  ...
  public byte[] messageEncrypt(byte[] input) {
    ...
    // compute c1 = E(r, z)
    byte[] c1 = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) key, r,
            z)
        .getEncoded();
  ...
  public byte[] messageDecrypt(byte[] input)
    ...
    GF2Vector[] decC1 = McElieceCCA2Primitives.decryptionPrimitive(
        (McElieceCCA2PrivateKeyParameters) key, hrmVec);
