 ...
 * {@link BigInteger BigInteger} with a few digits on the right of the decimal point. The number of
 * (binary) digits on the right of the decimal point is called the <code>scale</code> of the
 * <code>SimpleBigDecimal</code>. Unlike in {@link java.math.BigDecimal BigDecimal}, the scale is
 * not adjusted automatically, but must be set manually. All <code>SimpleBigDecimal</code>s taking
 * part in the same arithmetic operation must have equal scale. The result of a multiplication of
 * two <code>SimpleBigDecimal</code>s returns a
    ...
    //extends Number   // not in J2ME - add compatibility class?
{
   ...
   * <code>SimpleBigDecimal</code> equals <code>bigInt / 2<sup>scale</sup></code>.
   *
