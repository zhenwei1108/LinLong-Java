...
public class KCCMBlockCipher
   ...
   * <p>
   * Nb is a parameter specified in CCM mode of DSTU7624 standard. This parameter specifies maximum
   * possible length of input. It should be calculated as follows: Nb = 1/8 * (-3 + log[2]Nmax) + 1,
   * where Nmax - length of input message in bits. For practical reasons Nmax usually less than 4Gb,
   * e.g. for Nmax = 2^32 - 1, Nb = 4.
   * </p>
   *
   * @param engine base cipher to use under CCM.
  ...
  public void init(boolean forEncryption, CipherParameters params)
    ...
    if (params instanceof AEADParameters) {
      ...
      if (parameters.getMacSize() > MAX_MAC_BIT_LENGTH
          || parameters.getMacSize() < MIN_MAC_BIT_LENGTH || parameters.getMacSize() % 8 != 0) {
