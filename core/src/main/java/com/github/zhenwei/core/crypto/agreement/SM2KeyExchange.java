...
import com.github.zhenwei.core.crypto.params.SM2KeyExchangePrivateParameters;
import com.github.zhenwei.core.crypto.params.SM2KeyExchangePublicParameters;
...
public class SM2KeyExchange {
  ...
  public byte[][] calculateKeyWithConfirmation(int kLen, byte[] confirmationTag,
  ...
  private ECPoint calculateU(SM2KeyExchangePublicParameters otherPub) {
    ...
    ECPoint p2 = ECAlgorithms.cleanPoint(params.getCurve(),
        otherPub.getEphemeralPublicKey().getQ());
  ...
  private byte[] calculateInnerHash(Digest digest, ECPoint u, byte[] za, byte[] zb, ECPoint p1,
      ECPoint p2) {
    addFieldElement(digest, u.getAffineXCoord());
    digest.update(za, 0, za.length);
    digest.update(zb, 0, zb.length);
