 ...
 * <p/>
 * BLAKE2s offers a built-in keying mechanism to be used directly for authentication ("Prefix-MAC")
 * rather than a HMAC construction.
 * <p/>
 * BLAKE2s offers a built-in support for a salt for randomized hashing and a personal string for
 * defining a unique hash function for each application.
 * <p/>
 * BLAKE2s is optimized for 32-bit platforms and produces digests of any size between 1 and 32
 * bytes.
 */
public class Blake2sDigest
  ...
  /**
   * Whenever this buffer overflows, it will be processed in the compress() function. For
   * performance issues, long messages will not use this buffer.
   */
   ...
   * BLAKE2s for hashing.
   *
   * @param digestBits the desired digest length in bits. Must be a multiple of 8 and less than
   *                   256.
   */
   ...
   * <p/>
   * After calling the doFinal() method, the key will remain to be used for further computations of
   * this instance. The key can be overwritten using the clearKey() method.
   *
   ...
   * <p/>
   * After calling the doFinal() method, the key, the salt and the personal string will remain and
   * might be used for further computations with this instance. The key can be overwritten using the
   * clearKey() method, the salt (pepper) can be overwritten using the clearSalt() method.
   *
   * @param key             a key up to 32 bytes or null
