...
/**
 * This class implements AsymmetricCipherKeyPairGenerator. It is used as a generator for the private
 * and public key of the Rainbow Signature Scheme.
 * <p>
 * Detailed information about the key generation is to be found in the paper of Jintai Ding, Dieter
 * Schmidt: Rainbow, a New Multivariable Polynomial Signature Scheme. ACNS 2005: 164-175
...
public class RainbowKeyPairGenerator
  ...
  /**
   * The standard constructor tries to generate the Rainbow algorithm identifier with the
   * corresponding OID.
   */
  ...
  public AsymmetricCipherKeyPair genKeyPair() {
    ...
    pubKey = new RainbowPublicKeyParameters(vi[vi.length - 1] - vi[0], pub_quadratic, pub_singular,
        pub_scalar);
  ...
  /**
   * This function calls the functions for the random generation of the coefficients and the
   * matrices needed for the private key and the method for computing the public key.
   */
  private void keygen() {
    generateL1();
    generateL2();
   ...
   * <p>
   * The translation part b1, is stored in a separate array. The inverse of the matrix-part of L1
   * A1inv is also computed here.
   * </p><p>
   ...
   * <p>
   * The translation part b2, is stored in a separate array. The inverse of the matrix-part of L2
   * A2inv is also computed here.
   * </p><p>
  ...
  /**
   * This function generates the private map F, which consists of u-1 layers. Each layer consists of
   * oi polynomials where oi = vi[i+1]-vi[i].
   * <p>
   ...
   * <p>
   * The composition of F with L2 is computed, followed by applying L1 to the composition's result.
   * The singular and scalar values constitute to the public key as is, the quadratic terms are
   * compacted in
  ...
  /**
   * The quadratic (or mixed) terms of the public key are compacted from a n x n matrix per
   * polynomial to an upper diagonal matrix stored in one integer array of n (n + 1) / 2 elements
   * per polynomial. The ordering of elements is lexicographic and the result is updating
   * <tt>this.pub_quadratic</tt>, which stores the quadratic elements of the public key.
   *
