 ...
 * cipher followed by a 128 bit {@code r} value used for the polynomial portion of the Mac. <br> The
 * {@code r} value has a specific format with some bits required to be cleared, resulting in an
 * effective 106 bit key. <br> A separately generated 256 bit key can be modified to fit the
 * Poly1305 key format by using the {@link #clamp(byte[])} method to clear the required bits.
 *
 * @see Poly1305
 */
public class Poly1305KeyGenerator
  ...
  /**
   * Initialises the key generator.<br> Poly1305 keys are always 256 bits, so the key length in the
   * provided parameters is ignored.
   */
   ...
   * as per {@link #clamp(byte[])}.
   *
   * @throws IllegalArgumentException if the key is of the wrong length, or has invalid bits set in
   *                                  the <code>r</code> portion of the key.
   */
