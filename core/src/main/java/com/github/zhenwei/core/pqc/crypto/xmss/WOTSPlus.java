...
final class WOTSPlus {
  ...
  WOTSPlusSignature sign(byte[] messageDigest, OTSHashAddress otsHashAddress) {
    ...
    List<Integer> baseWMessage = convertToBaseW(messageDigest, params.getWinternitzParameter(),
        params.getLen1());
    /* create checksum */
    int checksum = 0;
    ...
    for (int i = 0; i < params.getLen(); i++) {
      otsHashAddress = (OTSHashAddress) new OTSHashAddress.Builder()
          .withLayerAddress(otsHashAddress.getLayerAddress())
          .withTreeAddress(otsHashAddress.getTreeAddress())
          .withOTSAddress(otsHashAddress.getOTSAddress()).withChainAddress(i)
          .withHashAddress(otsHashAddress.getHashAddress())
          .withKeyAndMask(otsHashAddress.getKeyAndMask())
          .build();
   ...
   */
  WOTSPlusPublicKeyParameters getPublicKeyFromSignature(byte[] messageDigest,
   ...
   * @param otsHashAddress OTS hash address for randomization.
   * @return Value obtained by iterating F for steps times on input startHash, using the outputs of
   * PRF.
   */
  private byte[] chain(byte[] startHash, int startIndex, int steps, OTSHashAddress otsHashAddress) {
    ...
    otsHashAddress = (OTSHashAddress) new OTSHashAddress.Builder()
        .withLayerAddress(otsHashAddress.getLayerAddress())
        .withTreeAddress(otsHashAddress.getTreeAddress())
        .withOTSAddress(otsHashAddress.getOTSAddress())
        .withChainAddress(otsHashAddress.getChainAddress())
  ...
  /**
   * Calculates a new public key based on the state of secretKeySeed, publicSeed and
   * otsHashAddress.
   *
   * @param otsHashAddress OTS hash address for randomization.
  ...
  WOTSPlusPublicKeyParameters getPublicKey(OTSHashAddress otsHashAddress) {
    ...
    for (int i = 0; i < params.getLen(); i++) {
      otsHashAddress = (OTSHashAddress) new OTSHashAddress.Builder()
          ...
          .withKeyAndMask(otsHashAddress.getKeyAndMask())
          .build();
      publicKey[i] = chain(expandSecretKeySeed(i), 0, params.getWinternitzParameter() - 1,
          otsHashAddress);
    }
    return new WOTSPlusPublicKeyParameters(params, publicKey);
  }
