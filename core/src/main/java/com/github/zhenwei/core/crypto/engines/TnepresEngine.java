...
/**
 * Tnepres is a 128-bit 32-round block cipher with variable key lengths, including 128, 192 and 256
 ...
 * candidate algorithm for the NIST AES Quest. Unfortunately there was an endianness issue with test
 * vectors in the AES submission and the resulting confusion lead to the Tnepres cipher as well,
 * which is a byte swapped version of Serpent.
 * <p>
 * For full details see <a href="https://www.cl.cam.ac.uk/~rja14/serpent.html">The Serpent home
 * page</a>
 */
public final class TnepresEngine
  ...
  protected int[] makeWorkingKey(
    ...
    //
    for (int i = 8; i < 16; i++) {
      kPad[i] = rotateLeft(kPad[i - 8] ^ kPad[i - 5] ^ kPad[i - 3] ^ kPad[i - 1] ^ PHI ^ (i - 8),
          11);
    }
