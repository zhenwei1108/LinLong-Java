 ...
 * change the polynomial, others (like <code>mult</code>) do not but return the result as a new
 * polynomial.
 */
public class BigIntPolynomial {
  ...
  /**
   * Constructs a <code>BigIntPolynomial</code> from a <code>IntegerPolynomial</code>. The two
   ...
   * but returns the result as a new polynomial.<br> Both polynomials must have the same number of
   * coefficients.
   *
  ...
  /**
   * Multiplies each coefficient by a <code>BigInteger</code>. Does not return a new polynomial but
  ...
  /**
   * Divides each coefficient by a <code>BigInteger</code> and rounds the result to the nearest
   * whole number.<br> Does not return a new polynomial but modifies this polynomial.
   *
  ...
  public void div(BigInteger divisor) {
    ...
    for (int i = 0; i < coeffs.length; i++) {
      coeffs[i] = coeffs[i].compareTo(Constants.BIGINT_ZERO) > 0 ? coeffs[i].add(d)
          : coeffs[i].add(d.negate());
      coeffs[i] = coeffs[i].divide(divisor);
    }
  }

  /**
   * Divides each coefficient by a <code>BigDecimal</code> and rounds the result to
   * <code>decimalPlaces</code> places.
   *
   * @param divisor       the number to divide by
  ...
  public BigDecimalPolynomial div(BigDecimal divisor, int decimalPlaces) {
    ...
    {
      p.coeffs[i] = new BigDecimal(coeffs[i]).multiply(factor)
          .setScale(decimalPlaces, BigDecimal.ROUND_HALF_EVEN);
    }
