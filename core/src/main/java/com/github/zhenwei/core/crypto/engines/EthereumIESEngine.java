...
public class EthereumIESEngine {
   ...
   */
  public EthereumIESEngine(BasicAgreement agree, DerivationFunction kdf, Mac mac,
      byte[] commonMac) {
    this.agree = agree;
    this.kdf = kdf;
    this.mac = mac;
   ...
   * @param params                    encoding and derivation parameters, may be wrapped to include
   ...
   * @param params          encoding and derivation parameters, may be wrapped to include an IV for
  ...
  private byte[] decryptBlock(byte[] in_enc, int inOff, int inLen)
    ...
    if (cipher == null) {
      ...
      len = cipher.processBytes(in_enc, inOff + V.length, inLen - V.length - mac.getMacSize(), M,
          0);
    }
  ...
  /**
   * Basic KDF generator for derived keys and ivs as defined by IEEE P1363a/ISO 18033 <br> This
   * implementation is based on ISO 18033/P1363a.
   * <p>
   * This class has been adapted from the <tt>BaseKDFBytesGenerator</tt> implementation of Bouncy
