 ...
 * Internally, those participants call these primitive operations in {@link JPAKEUtil}.
 * <p>
...
public class JPAKEUtil {
  ...
  /**
   * Converts the given password to a {@link BigInteger} for use in arithmetic calculations.
   */
   ...
   * <p>
   * <blockquote>
   * Alice could simply check ga != 1 to ensure it is a generator. In fact, as we will explain in
   * Section 3, (x1 + x3 + x4 ) is random over Zq even in the face of active attacks. Hence, the
   * probability for ga = 1 is extremely small - on the order of 2^160 for 160-bit q.
   * </blockquote>
   *
  ...
  /**
   * Calculates the keying material, which can be done after round 2 has completed. A session key
   * must be derived from this key material using a secure key derivation function (KDF). The KDF
   * used to derive the key is handled externally (i.e. not by {@link JPAKEParticipant}).
   * <pre>
  ...
  /**
   * Validates that the given participant ids are not equal. (For the J-PAKE exchange, each
   * participant must use a unique id.)
   *
  ...
  /**
   * Validates that the given participant ids are equal. This is used to ensure that the payloads
   * received from each round all come from the same participant.
   *
   * @throws CryptoException if the participantId strings are equal.
   */
