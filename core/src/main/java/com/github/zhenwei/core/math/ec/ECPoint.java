...
public abstract class ECPoint {
  ...
  boolean implIsValid(final boolean decompressed, final boolean checkOrder) {
    ...
    ValidityPrecompInfo validity = (ValidityPrecompInfo) getCurve().precompute(this,
        ValidityPrecompInfo.PRECOMP_NAME, new PreCompCallback() {
          public PreCompInfo precompute(PreCompInfo existing) {
            ValidityPrecompInfo info =
                (existing instanceof ValidityPrecompInfo) ? (ValidityPrecompInfo) existing : null;
            if (info == null) {
  ...
  public ECPoint scaleX(ECFieldElement scale) {
    return isInfinity()
        ? this
        : getCurve().createRawPoint(getRawXCoord().multiply(scale), getRawYCoord(),
            getRawZCoords());
  }
  ...
  public ECPoint scaleXNegateY(ECFieldElement scale) {
    return isInfinity()
        ? this
        : getCurve().createRawPoint(getRawXCoord().multiply(scale), getRawYCoord().negate(),
            getRawZCoords());
  }
  ...
  public ECPoint scaleY(ECFieldElement scale) {
    return isInfinity()
        ? this
        : getCurve().createRawPoint(getRawXCoord(), getRawYCoord().multiply(scale),
            getRawZCoords());
  }
  ...
  public ECPoint scaleYNegateX(ECFieldElement scale) {
    return isInfinity()
        ? this
        : getCurve().createRawPoint(getRawXCoord().negate(), getRawYCoord().multiply(scale),
            getRawZCoords());
  }
  ...
  public static abstract class AbstractF2m extends ECPoint {
    ...
    protected boolean satisfiesOrder() {
      ...
      if (ECConstants.FOUR.equals(cofactor)) {
        ...
        ECPoint N = this.normalize();
        ECFieldElement X = N.getAffineXCoord();
        ECFieldElement L = ((ECCurve.AbstractF2m) curve).solveQuadraticEquation(
            X.add(curve.getA()));
        if (null == L) {
          return false;
        }
