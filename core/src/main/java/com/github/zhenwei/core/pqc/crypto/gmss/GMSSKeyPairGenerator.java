...
public class GMSSKeyPairGenerator
  ...
  /**
   * The standard constructor tries to generate the GMSS algorithm identifier with the corresponding
   * OID.
   *
  ...
  private AsymmetricCipherKeyPair genKeyPair() {
    ...
    for (int h = numLayer - 1; h >= 0; h--) {
      ...
      // value
      {
        tree = this.generateCurrentAuthpathAndRoot(currentRoots[h + 1], currentStack[h], seeds[h],
            h);
      }
    ...
    GMSSPrivateKeyParameters privateKey = new GMSSPrivateKeyParameters(currentSeeds, nextNextSeeds,
        currentAuthPaths,
        nextAuthPaths, currentTreehash, nextTreehash, currentStack, nextStack, currentRetain,
  ...
  /**
   * calculates the authpath for tree in layer h which starts with seed[h] additionally computes the
   * rootSignature of underlaying root
   *
   ...
   */
  private GMSSRootCalc generateCurrentAuthpathAndRoot(byte[] lowerRoot, Vector currentStack,
      byte[] seed, int h) {
    ...
    GMSSRootCalc treeToConstruct = new GMSSRootCalc(this.heightOfTrees[h], this.K[h],
        digestProvider);
   ...
   * testing demands.
   * <p>
   * A given <code>keysize</code> of less than 10 creates an amount 2^10 signatures. A keySize
   * between 10 and 20 creates 2^20 signatures. Given an integer greater than 20 the key pair
   * generator creates 2^40 signatures.
   *
   * @param keySize      Assigns the parameters used for the GMSS signatures. There are 3
   *                     choices:<br> 1. keysize &lt;= 10: creates 2^10 signatures using the
   *                     parameterset<br> P = (2, (5, 5), (3, 3), (3, 3))<br> 2. keysize &gt; 10 and
   *                     &lt;= 20: creates 2^20 signatures using the parameterset<br> P = (2, (10,
   *                     10), (5, 4), (2, 2))<br> 3. keysize &gt; 20: creates 2^40 signatures using
   *                     the parameterset<br> P = (2, (10, 10, 10, 10), (9, 9, 9, 3), (2, 2, 2, 2))
   * @param secureRandom not used by GMSS, the SHA1PRNG of the SUN Provider is always used
   */
  public void initialize(int keySize, SecureRandom secureRandom) {
    ...
    if (keySize <= 10) { // create 2^10 keys
      ...
      kgp = new GMSSKeyGenerationParameters(secureRandom,
  ...
  public void initialize(KeyGenerationParameters param) {
    ...
    this.gmssPS = new GMSSParameters(gmssParams.getParameters().getNumOfLayers(),
        gmssParams.getParameters().getHeightOfTrees(),
  ...
  /**
   * This method is called by generateKeyPair() in case that no other initialization method has been
   * called by the user
   */
  private void initializeDefault() {
    ...
    KeyGenerationParameters kgp = new GMSSKeyGenerationParameters(null,
        new GMSSParameters(defh.length, defh, defw, defk));
    this.initialize(kgp);
