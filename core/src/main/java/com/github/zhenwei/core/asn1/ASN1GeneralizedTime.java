 ...
 * </p><p>
 * <h2>X.690</h2>
 * This is what is called "restricted string", and it uses ASCII characters to encode digits and
 * supplemental data.
 *
 ...
 * as described in the ITU-T Rec. X.680 | ISO/IEC 8824-1 clause on GeneralizedTime.
 * </p><p>
 * <b>11.7.2</b> The seconds element shall always be present.
 * </p>
 * <p>
 * <b>11.7.3</b> The fractional-seconds elements, if present,
 * shall omit all trailing zeros; if the elements correspond to 0, they shall be wholly omitted, and
 * the decimal point element also shall be omitted.
 */
public class ASN1GeneralizedTime
    extends ASN1Primitive {

  static final ASN1UniversalType TYPE = new ASN1UniversalType(ASN1GeneralizedTime.class,
      BERTags.GENERALIZED_TIME) {
  ...
  /**
   * The correct format for this is YYYYMMDDHHMMSS[.f]Z, or without the Z for local time, or Z+-HHMM
   * on the end, for difference between local time and UTC time. The fractional second amount f must
   * consist of at least one number with trailing zeroes removed.
   *
  ...
  /**
   * Base constructor from a java.util.date and Locale - you may need to use this if the default
   * locale doesn't use a Gregorian calender so that the GeneralizedTime produced is compatible with
   ...
   * <p>
   * Normally in a certificate we would expect "Z" rather than "GMT", however adding the "GMT" means
   * we can just use:
   * <pre>
