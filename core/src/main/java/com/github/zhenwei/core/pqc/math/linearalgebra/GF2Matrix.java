...
/**
 * This class describes some operations with matrices over finite field GF(2) and is used in ecc and
 * MQ-PKC (also has some specific methods and implementation)
 */
public class GF2Matrix
    extends Matrix {

  /**
   * For the matrix representation the array of type int[][] is used, thus one element of the array
   * keeps 32 elements of the matrix (from one row and 32 columns)
   */
  private int[][] matrix;
  ...
  /**
   * Create the matrix with the contents of the given array. The matrix is not copied. Unused
   * coefficients are masked out.
   *
   ...
   * @param typeOfMatrix the martix type (see {@link Matrix} for predefined constants)
   */
  ...
  /**
   * Compute the full form matrix <tt>(this | Id)</tt> from this matrix in left compact form, where
   * <tt>Id</tt> is the <tt>k x k</tt> identity matrix and <tt>k</tt> is the number of rows of this
   * matrix.
   *
  ...
  /**
   * Compute the full form matrix <tt>(Id | this)</tt> from this matrix in right compact form, where
   * <tt>Id</tt> is the <tt>k x k</tt> identity matrix and <tt>k</tt> is the number of rows of this
   * matrix.
   *
  ...
  /**
   * Compute the product of a permutation matrix (which is generated from an n-permutation) and this
   * matrix.
   *
  ...
  /**
   * Compute the product of the matrix <tt>(this | Id)</tt> and a column vector, where <tt>Id</tt>
   * is a <tt>(numRows x numRows)</tt> unit matrix.
   *
  ...
  /**
   * Compute the product of this matrix and a permutation matrix which is generated from an
   * n-permutation.
   *
   * @param p the permutation
  ...
  /**
   * Compute the product of the matrix <tt>(Id | this)</tt> and a column vector, where <tt>Id</tt>
   * is a <tt>(numRows x numRows)</tt> unit matrix.
   *
   * @param vec the vector over GF(2)
