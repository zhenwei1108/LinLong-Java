...
/**
 * A parser for ASN.1 streams which also returns, where possible, parsers for the objects it
 * encounters.
 */
public class ASN1StreamParser {
    ...
    switch (univTagNo) {
      ...
      default:
        throw new ASN1Exception(
            "unknown DL object encountered: 0x" + Integer.toHexString(univTagNo));
    }
  }
  ...
  ASN1Encodable parseImplicitConstructedIL(int univTagNo) throws IOException {
    switch (univTagNo) {
      ...
      default:
        throw new ASN1Exception(
            "unknown BER object encountered: 0x" + Integer.toHexString(univTagNo));
    }
  }
  ...
  ASN1Encodable parseImplicitPrimitive(int univTagNo, DefiniteLengthInputStream defIn)
      throws IOException {
