...
/**
 * Implements the Counter with Cipher Block Chaining mode (CCM) detailed in NIST Special Publication
 * 800-38C.
 * <p>
...
public class CCMBlockCipher
  ...
  /**
   * Returns a byte array containing the mac calculated as part of the last encrypt or decrypt
   * operation.
   *
  ...
  private int getMacSize(boolean forEncryption, int requestedMacBits) {
    if (forEncryption && (requestedMacBits < 32 || requestedMacBits > 128 || 0 != (requestedMacBits
        & 15))) {
  ...
  private int getAssociatedTextLength() {
    return associatedText.size() + ((initialAssociatedText == null) ? 0
