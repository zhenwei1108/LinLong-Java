...
public class ArrayEncoder {
  ...
  /**
   * Bit string to coefficient conversion table from P1363.1. Also found at {@link
   * https://stackoverflow.com/questions/1562548/how-to-make-a-message-into-a-polynomial}
   * <p>
   * Convert each three-bit quantity to two ternary coefficients as follows, and concatenate the
   * resulting ternary quantities to obtain [the output].
   * </p><p>
   * <code>
   * {0, 0, 0} -> {0, 0}<br/> {0, 0, 1} -> {0, 1}<br/> {0, 1, 0} -> {0, -1}<br/> {0, 1, 1} -> {1,
   * 0}<br/> {1, 0, 0} -> {1, 1}<br/> {1, 0, 1} -> {1, -1}<br/> {1, 1, 0} -> {-1, 0}<br/> {1, 1, 1}
   * -> {-1, 1}<br/>
   * </code>
   * </p>
   */
  ...
  /**
   * Coefficient to bit string conversion table from P1363.1. Also found at {@link
   * https://stackoverflow.com/questions/1562548/how-to-make-a-message-into-a-polynomial}
   * <p>
   * Convert each set of two ternary coefficients to three bits as follows, and concatenate the
   * resulting bit quantities to obtain [the output]:
   * </p><p>
   * <code>
   * {-1, -1} -> set "fail" to 1 and set bit string to {1, 1, 1} {-1, 0} -> {1, 1, 0}<br/> {-1, 1}
   * -> {1, 1, 1}<br/> {0, -1} -> {0, 1, 0}<br/> {0, 0} -> {0, 0, 0}<br/> {0, 1} -> {0, 0, 1}<br/>
   * {1, -1} -> {1, 0, 1}<br/> {1, 0} -> {0, 1, 1}<br/> {1, 1} -> {1, 0, 0}<br/>
   * </code>   \
   * </p>
   */
  ...
  /**
   * Encodes an int array whose elements are between 0 and <code>q</code>, to a byte array leaving
   * no gaps between bits.<br>
  ...
  /**
   * Decodes a <code>byte</code> array encoded with {@link #encodeModQ(int[], int)} back to an
   * <code>int</code> array.<br>
   * <code>N</code> is the number of coefficients. <code>q</code> must be a power of
   * <code>2</code>.<br>
   ...
   * @param q
   * @return an array containing <code>N</code> coefficients between <code>0</code> and
   * <code>q-1</code>
   */
  ...
  /**
   * Decodes data encoded with {@link #encodeModQ(int[], int)} back to an <code>int</code>
   * array.<br>
   * <code>N</code> is the number of coefficients. <code>q</code> must be a power of
   * <code>2</code>.<br>
   * Ignores any excess bytes.
  ...
  /**
   * Decodes a <code>byte</code> array encoded with {@link #encodeMod3Sves(int[])} back to an
   * <code>int</code> array with <code>N</code> coefficients between <code>-1</code> and
   * <code>1</code>.<br> Ignores any excess bytes.<br> See P1363.1 section 9.2.2.
   *
   * @param data an encoded ternary polynomial
   * @param N    number of coefficients
  ...
  /**
   * Encodes an <code>int</code> array whose elements are between <code>-1</code> and
   * <code>1</code>, to a byte array.
   * <code>coeffs[2*i]</code> and <code>coeffs[2*i+1]</code> must not both equal -1 for any integer
   * <code>i</code>,
   * so this method is only safe to use with arrays produced by {@link #decodeMod3Sves(byte[],
   * int)}.<br> See P1363.1 section 9.2.3.
   *
   * @param arr
   * @return the encoded array
   */
  public static byte[] encodeMod3Sves(int[] arr) {
    ...
    int byteIndex = 0;
    for (int i = 0;
        i < arr.length / 2 * 2; ) {   // if length is an odd number, throw away the highest coeff
      int coeff1 = arr[i++] + 1;
      int coeff2 = arr[i++] + 1;
      ...
      int[] bits = new int[]{BIT1_TABLE[bitTableIndex], BIT2_TABLE[bitTableIndex],
          BIT3_TABLE[bitTableIndex]};
      for (int j = 0; j < 3; j++) {
  ...
  /**
   * Converts a byte array produced by {@link #encodeMod3Tight(int[])} back to an <code>int</code>
   * array.
   *
