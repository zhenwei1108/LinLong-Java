...
/**
 * GCM-SIV Mode.
 * <p>It should be noted that the specified limit of 2<sup>36</sup> bytes is not supported. This is
 * because all bytes are
 * cached in a <b>ByteArrayOutputStream</b> object (which has a limit of a little less than
 * 2<sup>31</sup> bytes), and are output on the <b>doFinal</b>() call (which can only process a
 * maximum of 2<sup>31</sup> bytes).</p>
 * <p>The practical limit of 2<sup>31</sup> - 24 bytes is policed, and attempts to breach the limit
 * will be rejected</p>
 * <p>In order to properly support the higher limit, an extended form of
 * <b>ByteArrayOutputStream</b> would be needed
 * which would use multiple arrays to store the data. In addition, a new <b>doOutput</b> method
 * would be required (similar to that in <b>XOF</b> digests), which would allow the data to be
 * output over multiple calls. Alternatively an extended form of <b>ByteArrayInputStream</b> could
 * be used to deliver the data.</p>
 */
public class GCMSIVBlockCipher
  ...
  /**
   * The maximum data length (AEAD/PlainText). Due to implementation constraints this is restricted
   * to the maximum array length (https://programming.guide/java/array-maximum-length.html) minus
   * the BUFLEN to allow for the MAC
   */
