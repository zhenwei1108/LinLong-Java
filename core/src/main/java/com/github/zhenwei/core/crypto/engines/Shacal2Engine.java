 ...
 * <p>
 * A description of Shacal can be found at: https://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.3.4066
 * Best known cryptanalytic (Wikipedia 11.2013): Related-key rectangle attack on 44-rounds (Jiqiang
 * Lu, Jongsung Kim). Comments are related to SHA-256-Naming as described in FIPS PUB 180-2
 * </p>
 */
public class Shacal2Engine
  ...
  public void setKey(byte[] kb) {
    ...
    for (int i = 16; i < 64;
        i++) { // Key-Expansion, implicitly Zero-Padding for 16 > i > kb.length/4
      workingKey[i] =
          ((workingKey[i - 2] >>> 17 | workingKey[i - 2] << -17)
              // corresponds to ROTL n(x) of FIPS PUB 180-2
              ^ (workingKey[i - 2] >>> 19 | workingKey[i - 2] << -19)
              ^ (workingKey[i - 2] >>> 10))
              // corresponds to sigma1(x)-Function of FIPS PUB 180-2
              + workingKey[i - 7]
              + ((workingKey[i - 15] >>> 7 | workingKey[i - 15] << -7)
              ^ (workingKey[i - 15] >>> 18 | workingKey[i - 15] << -18)
              ^ (workingKey[i - 15] >>> 3))
              // corresponds to sigma0(x)-Function of FIPS PUB 180-2
              + workingKey[i - 16];
    }
  }
  ...
  private void encryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
    int[] block = new int[BLOCK_SIZE
        / 4];// corresponds to working variables a,b,c,d,e,f,g,h of FIPS PUB 180-2
