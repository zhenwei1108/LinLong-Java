...
public class OpenSSHPrivateKeyUtil {
   ...
   **/
  static final byte[] AUTH_MAGIC = Strings.toByteArray(
      "openssh-key-v1\0"); // C string so null terminated
  ...
  public static byte[] encodePrivateKey(AsymmetricKeyParameter params)
    ...
    if (params instanceof RSAPrivateCrtKeyParameters) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "unable to encode DSAPrivateKeyParameters " + ex.getMessage());
      }
...

      {
        ...
        pkBuild.writeBlock(
            Arrays.concatenate(((Ed25519PrivateKeyParameters) params).getEncoded(), pubKeyEncoded));
    ...
    throw new IllegalArgumentException(
        "unable to convert " + params.getClass().getName() + " to openssh private key");
  ...
  public static AsymmetricKeyParameter parsePrivateKeyBlob(byte[] blob) {
    ...
    if (blob[0] == 0x30) {
      ...
      if (sequence.size() == 6) {
        if (allIntegers(sequence) && ((ASN1Integer) sequence.getObjectAt(0)).getPositiveValue()
            .equals(BigIntegers.ZERO)) {
        ...
        if (sequence.getObjectAt(3) instanceof ASN1TaggedObject
          ...
          ASN1ObjectIdentifier curveOID = ASN1ObjectIdentifier.getInstance(
      ...
      if ("ssh-ed25519".equals(keyType)) {
        ...
        if (edPrivateKey.length
            != Ed25519PrivateKeyParameters.KEY_SIZE + Ed25519PublicKeyParameters.KEY_SIZE) {
        ...
        ASN1ObjectIdentifier oid = SSHNamedCurves.getByName(
            Strings.fromByteArray(pkIn.readBlock()));
        if (oid == null) {
