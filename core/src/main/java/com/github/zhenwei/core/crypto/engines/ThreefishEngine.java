...
import com.github.zhenwei.core.crypto.params.TweakableBlockCipherParameters;
 ...
 * to speed up key schedule injection. <br> 2 x block size state is retained by each cipher
 * instance.
 */
public class ThreefishEngine
   ...
   *
   * @param blocksizeBits the block size in bits, one of {@link #BLOCKSIZE_256}, {@link
   *                      #BLOCKSIZE_512}, {@link #BLOCKSIZE_1024}.
   */
  ...
  public void init(boolean forEncryption, CipherParameters params)
    ...
    if (keyBytes != null) {
      if (keyBytes.length != this.blocksizeBytes) {
        throw new IllegalArgumentException(
            "Threefish key must be same size as block (" + blocksizeBytes
                + " bytes)");
      }
   ...
   * @param tweak         the 2 word (128 bit) tweak, or <code>null</code> to use the current
   *                      tweak.
   */
