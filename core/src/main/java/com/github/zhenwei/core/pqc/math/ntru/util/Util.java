...
public class Util {
  ...
  /**
   * Generates a "sparse" or "dense" polynomial containing numOnes ints equal to 1, numNegOnes int
   * equal to -1, and the rest equal to 0.
   *
   * @param N
   * @param numOnes
   * @param numNegOnes
   * @param sparse     whether to create a {@link SparseTernaryPolynomial} or {@link
   *                   DenseTernaryPolynomial}
   * @return a ternary polynomial
   */
  public static TernaryPolynomial generateRandomTernary(int N, int numOnes, int numNegOnes,
      boolean sparse, SecureRandom random) {
    if (sparse) {
  ...
  /**
   * Generates an array containing numOnes ints equal to 1, numNegOnes int equal to -1, and the rest
   * equal to 0.
   *
   * @param N
   * @param numOnes
  ...
  public static boolean is64BitJVM() {
    if (!IS_64_BITNESS_KNOWN) {
      ...
      IS_64_BIT_JVM =
          "amd64".equals(arch) || "x86_64".equals(arch) || "ppc64".equals(arch) || "64".equals(
              sunModel);
  ...
  /**
   * Reads a given number of bytes from an <code>InputStream</code>. If there are not enough bytes
   * in the stream, an <code>IOException</code> is thrown.
   *
   * @param is
   * @param length
