...
public class GMSSPrivateKeyParameters
   ...
   *
   * @param currentSeed      seed for the generation of private OTS keys for the current subtrees
   * @param nextNextSeed     seed for the generation of private OTS keys for the next subtrees
    ...
    if (numLayer > 1) {
      ...
      if (nextNextLeaf == null) {
        ...
        for (int i = 0; i < numLayer - 2; i++) {
          this.nextNextLeaf[i] = new GMSSLeaf(digestProvider.get(), otsIndex[i + 1],
              numLeafs[i + 2], this.nextNextSeeds[i]);
        }
      } else {
  ...
  /**
   * This method computes the authpath (AUTH) for the current tree, Additionally the root signature
   * for the next tree (SIG+), the authpath (AUTH++) and root (ROOT++) for the tree after next in
   * layer
  ...
  private int getMinTreehashIndex(int layer) {
    ...
    for (int h = 0; h < heightOfTrees[layer] - K[layer]; h++) {
      if (currentTreehash[layer][h].wasInitialized()
        ...
        if (minTreehash == -1) {
          minTreehash = h;
        } else if (currentTreehash[layer][h].getLowestNodeHeight()
            < currentTreehash[layer][minTreehash]
            .getLowestNodeHeight()) {
          minTreehash = h;
        }
      }
