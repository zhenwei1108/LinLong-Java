...
class Tree {
  ...
  static void l_tree(HashFunctions hs, byte[] leaf, int leafOff, byte[] wots_pk, int pkOff,
  ...
  static void treehash(HashFunctions hs, byte[] node, int nodeOff, int height, byte[] sk,
    ...
    for (; a.subleaf < lastnode; a.subleaf++) {
      ...
      while (stackoffset > 1 && stacklevels[stackoffset - 1] == stacklevels[stackoffset - 2]) {
        //MASKS
        int maskoffset =
            2 * (stacklevels[stackoffset - 1] + Wots.WOTS_LOG_L) * SPHINCS256Config.HASH_BYTES;

        hs.hash_2n_n_mask(stack, (stackoffset - 2) * SPHINCS256Config.HASH_BYTES, stack,
            (stackoffset - 2) * SPHINCS256Config.HASH_BYTES,
