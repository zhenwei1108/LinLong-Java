...
class HSS {
  ...
  /**
   * Increments an HSS private key without doing any work on it. HSS private keys are automatically
   * incremented when when used to create signatures.
   * <p>
   * The HSS private key is ranged tested before this incrementation is applied. LMS keys will be
  ...
  public static boolean verifySignature(HSSPublicKeyParameters publicKey, HSSSignature signature,
      byte[] message) {
  ...
  static class PlaceholderLMSPrivateKey
    ...
    public PlaceholderLMSPrivateKey(LMSigParameters lmsParameter, LMOtsParameters otsParameters,
        int q, byte[] I, int maxQ, byte[] masterSecret) {
