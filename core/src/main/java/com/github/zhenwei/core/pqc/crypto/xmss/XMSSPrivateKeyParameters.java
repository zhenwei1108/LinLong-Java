...
public final class XMSSPrivateKeyParameters
  ...
  private XMSSPrivateKeyParameters(Builder builder) {
    ...
    if (privateKey != null) {
      ...
      position += rootSize;
      /* import BDS state */
      byte[] bdsStateBinary = XMSSUtil.extractBytesAtOffset(privateKey, position,
          privateKey.length - position);
      try {
      ...
      if (tmpBDSState != null) {
        ...
        if (builder.index < ((1 << params.getHeight()) - 2) && tmpPublicSeed != null
            && tmpSecretKeySeed != null) {
          bdsState = new BDS(params, tmpPublicSeed, tmpSecretKeySeed,
              (OTSHashAddress) new OTSHashAddress.Builder().build(), builder.index);
        } else {
  ...
  XMSSPrivateKeyParameters rollKey() {
    synchronized (this) {
      ...
      if (bdsState.getIndex() < bdsState.getMaxIndex()) {
        bdsState = bdsState.getNextState(publicSeed, secretKeySeed,
            (OTSHashAddress) new OTSHashAddress.Builder().build());
      } else {
        bdsState = new BDS(params, bdsState.getMaxIndex(),
            bdsState.getMaxIndex() + 1); // no more nodes left.
      }
  ...
  public XMSSPrivateKeyParameters extractKeyShard(int usageCount) {
    ...
    synchronized (this) {
      ...
      if (usageCount <= this.getUsagesRemaining()) {
        ...
        if (usageCount == this.getUsagesRemaining()) {
          this.bdsState = new BDS(params, bdsState.getMaxIndex(),
              getIndex() + usageCount);   // we're finished.
        } else {
