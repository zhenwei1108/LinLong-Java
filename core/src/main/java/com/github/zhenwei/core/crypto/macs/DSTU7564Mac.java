...
public class DSTU7564Mac
  ...
  private byte[] padKey(byte[] in) {
    int paddedLen = ((in.length + engine.getByteLength() - 1) / engine.getByteLength())
    ...
    Pack.intToLittleEndian(in.length * BITS_IN_BYTE, padded,
        padded.length - 12); // Defined in standard;
