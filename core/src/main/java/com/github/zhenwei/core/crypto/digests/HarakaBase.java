...
public abstract class HarakaBase
  ...
  static private byte[] mixColumns(byte[] s) {
    ...
    int j = 0;
    for (int i = 0; i < 4; i++) {
      out[j++] = (byte) (xTime(s[4 * i]) ^ xTime(s[4 * i + 1]) ^ s[4 * i + 1] ^ s[4 * i + 2] ^ s[
          4 * i + 3]);
      out[j++] = (byte) (s[4 * i] ^ xTime(s[4 * i + 1]) ^ xTime(s[4 * i + 2]) ^ s[4 * i + 2] ^ s[
          4 * i + 3]);
      out[j++] = (byte) (s[4 * i] ^ s[4 * i + 1] ^ xTime(s[4 * i + 2]) ^ xTime(s[4 * i + 3]) ^ s[
          4 * i + 3]);
      out[j++] = (byte) (xTime(s[4 * i]) ^ s[4 * i] ^ s[4 * i + 1] ^ s[4 * i + 2] ^ xTime(
          s[4 * i + 3]));
    }
