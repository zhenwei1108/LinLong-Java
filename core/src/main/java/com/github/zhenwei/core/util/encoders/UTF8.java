...
//  private static final byte C_ASC = 11;           //- 00..7F          ASCII leading byte range
  ...
  static {
    ...
    byte[] firstUnitTransitions = {S_ERR, S_ERR, S_ERR, S_ERR, S_CS1, S_P3A, S_CS2, S_P3B, S_P4A,
        S_CS3, S_P4B};
  ...
  /**
   * Transcode a UTF-8 encoding into a UTF-16 representation. In the general case the output {@code
   * utf16} array should be at least as long as the input {@code utf8} one to handle arbitrary
   * inputs. The number of output UTF-16 code units is returned, or -1 if any errors are encountered
   * (in which case an arbitrary amount of data may have been written into the output array). Errors
   * that will be detected are malformed UTF-8, including incomplete, truncated or "overlong"
   * encodings, and unmappable code points. In particular, no unmatched surrogates will be produced.
   * An error will also result if {@code utf16} is found to be too small to store the complete
   * output.
   *
