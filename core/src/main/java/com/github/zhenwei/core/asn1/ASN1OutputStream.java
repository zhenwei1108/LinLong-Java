...
public class ASN1OutputStream {
  ...
  final void writeEncodingDL(boolean withID, int identifier, byte[] contents, int contentsOff,
      int contentsLen)
      throws IOException {
    writeIdentifier(withID, identifier);
    writeDL(contentsLen);
  ...
  final void writeEncodingDL(boolean withID, int identifier, byte contentsPrefix, byte[] contents,
      int contentsOff,
  ...
  final void writeEncodingDL(boolean withID, int flags, int tag, byte[] contents)
      throws IOException {
  ...
  final void writeEncodingIL(boolean withID, int flags, int tag, byte[] contents)
      throws IOException {
    writeIdentifier(withID, flags, tag);
    write(0x80);
    write(contents, 0, contents.length);
  ...
  final void writeEncodingIL(boolean withID, int identifier, ASN1Encodable[] elements)
      throws IOException {
    writeIdentifier(withID, identifier);
    write(0x80);
  ...
  static int getLengthOfEncodingDL(boolean withID, int tag, int contentsLength) {
    return (withID ? getLengthOfIdentifier(tag) : 0) + getLengthOfDL(contentsLength)
        + contentsLength;
  }
