...
public class GOST3410ParametersGenerator {
    ...
    for (int i = 0; i < s; i++) {
      ...
      for (; ; ) {
        ...
        for (; ; ) {
          ...
          //step13
          if ((TWO.modPow(p[m + 1].multiply(N.add(BigInteger.valueOf(k))), p[m]).compareTo(ONE)
              == 0) &&
  ...
  private void procedure_B(int x0, int c, BigInteger[] pq) {
    ...
    for (; ; ) {
      ...
      for (; ; ) {
        ...
        //step10
        if ((TWO.modPow(q.multiply(Q).multiply(N.add(BigInteger.valueOf(k))), p).compareTo(ONE)
            == 0) &&
  ...
  /**
   * which generates the p , q and a values from the given parameters, returning the
   * GOST3410Parameters object.
   *
