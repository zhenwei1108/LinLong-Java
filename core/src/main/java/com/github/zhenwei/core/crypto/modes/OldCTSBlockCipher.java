...
/**
 * A Cipher Text Stealing (CTS) mode cipher. CTS allows block ciphers to be used to produce cipher
 * text which is the same length as the plain text.
 * <p>
 * This version applies the CTS algorithm from one block up, rather than following the errata update
 * issued in 2004, where CTS mode is applied from greater than 1 block up and the first block is
 * processed using CBC mode.
 * </p>
 */
public class OldCTSBlockCipher
  ...
  /**
   * return the size of the output buffer required for an update plus a doFinal with an input of len
   * bytes.
   *
   * @param len the length of the input.
   ...
   * @throws com.github.zhenwei.core.crypto.DataLengthException if there isn't enough space in out.
