 ...
 * SPHINCS-256 signer.
 * <p>
 * This implementation is heavily based on the reference implementation in SUPERCOP, the main
 * difference being the digests used for message hashing and tree construction are now configurable
 * (within limits...) and that the implementation produces detached signatures.
 * </p>
 * <p>
 * The SPHINCS reference implementation is public domain, as per the statement in the second last
 * paragraph of section 1 in https://eprint.iacr.org/2014/795.pdf
 * </p>
 */
public class SPHINCS256Signer
   ...
   * @param twoNDigest the "2n-digest" must produce 64 bytes of output - used for initial
  ...
  static void validate_authpath(HashFunctions hs, byte[] root, byte[] leaf, int leafidx,
      byte[] authpath, int auOff, byte[] masks, int height) {
    int i, j;
    ...
    for (i = 0; i < height - 1; i++) {
      ...
      if ((leafidx & 1) != 0) {
        ...
        hs.hash_2n_n_mask(buffer, 0, buffer, 0, masks,
            2 * (Wots.WOTS_LOG_L + i) * SPHINCS256Config.HASH_BYTES);
        for (j = 0; j < SPHINCS256Config.HASH_BYTES; j++) {
    ...
    hs.hash_2n_n_mask(root, 0, buffer, 0, masks,
        2 * (Wots.WOTS_LOG_L + height - 1) * SPHINCS256Config.HASH_BYTES);
  }


  static void compute_authpath_wots(HashFunctions hs, byte[] root, byte[] authpath, int authOff,
      Tree.leafaddr a, byte[] sk, byte[] masks, int height) {
    int i, idx, j;
    Tree.leafaddr ta = new Tree.leafaddr(a);

    byte[] tree = new byte[2 * (1 << SPHINCS256Config.SUBTREE_HEIGHT)
        * SPHINCS256Config.HASH_BYTES];
    byte[] seed = new byte[(1 << SPHINCS256Config.SUBTREE_HEIGHT) * SPHINCS256Config.SEED_BYTES];
    byte[] pk = new byte[(1 << SPHINCS256Config.SUBTREE_HEIGHT) * Wots.WOTS_L
        * SPHINCS256Config.HASH_BYTES];
    ...
    for (i = (1 << SPHINCS256Config.SUBTREE_HEIGHT); i > 0; i >>>= 1) {
      for (j = 0; j < i; j += 2) {
        hs.hash_2n_n_mask(tree,
            (i >>> 1) * SPHINCS256Config.HASH_BYTES + (j >>> 1) * SPHINCS256Config.HASH_BYTES,
    ...
    for (i = 0; i < height; i++) {
      System.arraycopy(tree,
          ((1 << SPHINCS256Config.SUBTREE_HEIGHT) >>> i) * SPHINCS256Config.HASH_BYTES
              + ((idx >>> i) ^ 1) * SPHINCS256Config.HASH_BYTES, authpath,
          authOff + i * SPHINCS256Config.HASH_BYTES, SPHINCS256Config.HASH_BYTES);
    }
  ...
  byte[] crypto_sign(HashFunctions hs, byte[] m, byte[] sk) {
    ...
    // create leafidx deterministically
    {
      ...
      System.arraycopy(tsk,
          SPHINCS256Config.CRYPTO_SECRETKEYBYTES - SPHINCS256Config.SK_RAND_SEED_BYTES, sm, scratch,
          SPHINCS256Config.SK_RAND_SEED_BYTES);
      ...
      System.arraycopy(tsk, SPHINCS256Config.SEED_BYTES, sm, pk,
          Horst.N_MASKS * SPHINCS256Config.HASH_BYTES);

      Tree.treehash(hs, sm, pk + (Horst.N_MASKS * SPHINCS256Config.HASH_BYTES),
          SPHINCS256Config.SUBTREE_HEIGHT, tsk, b, sm, pk);

      d = hs.getMessageHash();

      d.update(sm, scratch,
          SPHINCS256Config.MESSAGE_HASH_SEED_BYTES + SPHINCS256Config.CRYPTO_PUBLICKEYBYTES);
      d.update(m, 0, m.length);
      d.doFinal(m_h, 0);
    }
    ...
    System.arraycopy(tsk, SPHINCS256Config.SEED_BYTES, masks, 0,
        Horst.N_MASKS * SPHINCS256Config.HASH_BYTES);
