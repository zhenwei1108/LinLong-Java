...
public class DSTU7624WrapEngine
  ...
  public byte[] wrap(byte[] in, int inOff, int inLen) {
    ...
    if ((inLen % engine.getBlockSize()) != 0) {
      ...
      throw new DataLengthException(
          "wrap data must be a multiple of " + engine.getBlockSize() + " bytes");
    }
    ...
    for (int j = 0; j < V; j++) {
      ...
      System.arraycopy(Btemp.get(0), 0, wrappedBuffer, engine.getBlockSize() / 2,
          engine.getBlockSize() / 2);
  ...
  public byte[] unwrap(byte[] in, int inOff, int inLen)
    ...
    if ((inLen % engine.getBlockSize()) != 0) {
      //Partial blocks not supported
      throw new DataLengthException(
          "unwrap data must be a multiple of " + engine.getBlockSize() + " bytes");
    }
    ...
    for (int j = 0; j < V; j++) {
      ...
      System.arraycopy(buffer, engine.getBlockSize() / 2, Btemp.get(0), 0,
          engine.getBlockSize() / 2);
    }
    ...
    System.arraycopy(buffer, buffer.length - engine.getBlockSize(), checkSumArray, 0,
        engine.getBlockSize());
