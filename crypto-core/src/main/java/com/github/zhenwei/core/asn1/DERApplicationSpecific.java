 ...
 *
 * @deprecated Will be removed. See comments for {@link ASN1ApplicationSpecific}.
 */
public class DERApplicationSpecific
    extends ASN1ApplicationSpecific {

  /**
   * Create an application specific object from the passed in data. This will assume the data does
   * not represent a constructed object.
   *
  ...
  public DERApplicationSpecific(int tagNo, byte[] contentsOctets) {
    super(
        new DERTaggedObject(false, BERTags.APPLICATION, tagNo, new DEROctetString(contentsOctets)));
  }
   ...
   * @param baseEncodable the object to be contained.
   */
  public DERApplicationSpecific(boolean explicit, int tagNo, ASN1Encodable baseEncodable)
      throws IOException {
  ...
  public DERApplicationSpecific(int tagNo, ASN1EncodableVector contentsElements) {
    super(new DERTaggedObject(false, BERTags.APPLICATION, tagNo,
        DERFactory.createSequence(contentsElements)));
  }
