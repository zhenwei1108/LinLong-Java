...
public class LMSPublicKeyParameters
  ...
  public LMSPublicKeyParameters(LMSigParameters parameterSet, LMOtsParameters lmOtsType, byte[] T1,
      byte[] I) {
    super(false);
  ...
  public static LMSPublicKeyParameters getInstance(Object src)
    ...
    if (src instanceof LMSPublicKeyParameters) {
      ...
      LMOtsParameters ostTypeCode = LMOtsParameters.getParametersForType(
  ...
  LMSContext generateOtsContext(LMSSignature S) {
    ...
    return new LMOtsPublicKey(LMOtsParameters.getParametersForType(ots_typecode), I, S.getQ(),
        null).createOtsContext(S);
  }
