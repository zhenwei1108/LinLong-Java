...
public class RFC5649WrapEngine
  ...
  public byte[] wrap(byte[] in, int inOff, int inLen) {
    ...
    if (paddedPlaintext.length == 8) {
      ...
      System.arraycopy(paddedPlaintext, 0, paddedPlainTextWithIV, iv.length,
  ...
  public byte[] unwrap(byte[] in, int inOff, int inLen)
    ...
    System.arraycopy(paddedPlaintext, paddedPlaintext.length - expectedZeros, pad, 0,
        expectedZeros);
  ...
  /**
   * Performs steps 1 and 2 of the unwrap process defined in RFC 3394. This code is duplicated from
   * RFC3394WrapEngine because that class will throw an error during unwrap because the IV won't
   * match up.
   *
   * @param in
   * @param inOff
