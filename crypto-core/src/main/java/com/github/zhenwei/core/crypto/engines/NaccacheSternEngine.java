...
public class NaccacheSternEngine
   ...
   *
   * @see com.github.zhenwei.core.crypto.AsymmetricBlockCipher#init(boolean,
   * com.github.zhenwei.core.crypto.CipherParameters)
   */
   ...
   *
   * @see com.github.zhenwei.core.crypto.AsymmetricBlockCipher#getInputBlockSize()
   */
   ...
   *
   * @see com.github.zhenwei.core.crypto.AsymmetricBlockCipher#getOutputBlockSize()
   */
    ...
    if (forEncryption) {
      ...
      for (int i = 0; i < primes.size(); i++) {
        ...
        if (lookup[i].size() != ((BigInteger) primes.elementAt(i)).intValue()) {
          if (debug) {
            System.out.println(
                "Prime is " + primes.elementAt(i) + ", lookup table has size " + al.size());
          }
        ...
        if (lookedup == -1) {
          if (debug) {
            ...
            System.out.println(
                "LookupList for " + primes.elementAt(i) + " with size " + lookup[i].size()
                    + " is: ");
   ...
   * @return The byte[] representation of the encrypted BigInteger (i.e. crypted.toByteArray())
   */
  ...
  /**
   * Computes the integer x that is expressed through the given primes and the congruences with the
