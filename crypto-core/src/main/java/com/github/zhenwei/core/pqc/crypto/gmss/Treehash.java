...
/**
 * This class implements a treehash instance for the Merkle tree traversal algorithm. The first node
 * of the stack is stored in this instance itself, additional tail nodes are stored on a tailstack.
 */
public class Treehash {
   ...
   *
   * @param name     an array of strings, containing the name of the used hash function and PRNG and
   ...
   * @param digest    an array of strings, containing the name of the used hash function and PRNG
   *                  and the name of the corresponding provider
   */
  ...
  /**
   * Method to initialize the seeds needed for the precomputation of right nodes. Should be
   * initialized with index 3*2^i for treehash_i
   *
   * @param seedIn
   */
  ...
  /**
   * initializes the treehash instance. The seeds must already have been initialized to work
   * correctly.
   */
  ...
  /**
   * Returns the height of the lowest node stored either in treehash or on the stack. It must not be
   * set to infinity (as mentioned in the paper) because this cases are considered in the
   * computeAuthPaths method of JDKGMSSPrivateKey
   *
