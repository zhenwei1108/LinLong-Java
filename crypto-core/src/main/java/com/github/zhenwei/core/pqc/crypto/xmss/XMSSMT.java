...
public final class XMSSMT {
  ...
  private void importState(XMSSMTPrivateKeyParameters privateKey,
      XMSSMTPublicKeyParameters publicKey) {
    /* import to xmss */
    xmssParams.getWOTSPlus()
        .importKeys(new byte[params.getTreeDigestSize()], this.privateKey.getPublicSeed());
  ...
  public void importState(byte[] privateKey, byte[] publicKey) {
    ...
    /* import to xmss */
    xmssParams.getWOTSPlus()
        .importKeys(new byte[params.getTreeDigestSize()], xmssMTPrivateKey.getPublicSeed());
  ...
  public boolean verifySignature(byte[] message, byte[] signature, byte[] publicKey)
    ...
    signer.init(false,
        new XMSSMTPublicKeyParameters.Builder(getParams()).withPublicKey(publicKey).build());
