...
/**
 * A polynomial with <code>int</code> coefficients.<br> Some methods (like <code>add</code>) change
 * the polynomial, others (like <code>mult</code>) do not but return the result as a new
 * polynomial.
 */
public class IntegerPolynomial
  ...
  /**
   * Prime numbers &gt; 4500 for resultant computation. Starting them below ~4400 causes incorrect
   * results occasionally. Fortunately, 4500 is about the optimum number for performance.<br/> This
   * array contains enough prime numbers so primes never have to be computed on-line for any
   * standard {@link com.github.zhenwei.core.pqc.crypto.ntru.NTRUSigningParameters}.
   */
  ...
  /**
   * Constructs a <code>IntegerPolynomial</code> from a <code>BigIntPolynomial</code>. The two
  ...
  /**
   * Decodes a byte array to a polynomial with <code>N</code> ternary coefficients<br> Ignores any
   * excess bytes.
   *
  ...
  /**
   * Reads data produced by {@link #toBinary3Tight()} from an input stream and converts it to a
   * polynomial.
   *
   ...
   * <code>coeffs[2*i]</code> and <code>coeffs[2*i+1]</code> must not both equal -1 for any integer
   * <code>i</code>,
  ...
  /**
   * Encodes a polynomial whose coefficients are between 0 and q, to binary. q must be a power of
   * 2.
   *
   * @param q
   * @return the encoded polynomial
  ...
  /**
   * Computes the inverse mod <code>q; q</code> must be a power of 2.<br> Returns <code>null</code>
  ...
  /**
   * Computes the inverse mod 3. Returns <code>null</code> if the polynomial is not invertible.
   *
   * @return a new polynomial
   */
  ...
  public Resultant resultantMultiThread() {
    ...
    while (!resultantTasks.isEmpty()) {
      try {
        ...
        Future<ModularResultant> newTask = executor.submit(
            new CombineTask(modRes1.get(), modRes2.get()));
   ...
   *
   * @return <code>(rho, res)</code> satisfying <code>res = rho*this + t*(x^n-1) mod p</code> for
   * some integer <code>t</code>.
   */
  public ModularResultant resultant(int p) {
    ...
    return new ModularResultant(new BigIntPolynomial(v2), BigInteger.valueOf(r),
        BigInteger.valueOf(p));
  }

  /**
   * Computes <code>this-b*c*(x^k) mod p</code> and stores the result in this polynomial.<br/> See
   * steps 4a,4b in EESS algorithm 2.2.7.1.
   *
   * @param b
   * @param c
   * @param k
  ...
  /**
   * Subtracts another polynomial which can have a different number of coefficients, and takes the
   * coefficient values mod <code>modulus</code>.
   *
   * @param b another polynomial
   */
  ...
  /**
   * Subtracts a <code>int</code> from each coefficient. Does not return a new polynomial but
  ...
  /**
   * Multiplies each coefficient by a <code>int</code>. Does not return a new polynomial but
   * modifies this polynomial.
   *
   * @param factor
   */
  ...
  /**
   * Multiplies each coefficient by a 2 and applies a modulus. Does not return a new polynomial but
   * modifies this polynomial.
   *
  ...
  /**
   * Divides each coefficient by <code>k</code> and rounds to the nearest integer. Does not return a
   ...
   *
   * @return true iff all coefficients are equal to zero, except for the lowest coefficient which
   * must equal 1
   */
