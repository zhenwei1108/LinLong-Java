...
public class ASN1ObjectIdentifier
  ...
  static final ASN1UniversalType TYPE = new ASN1UniversalType(ASN1ObjectIdentifier.class,
  ...
  public boolean on(ASN1ObjectIdentifier stem) {
    ...
    return id.length() > stemId.length() && id.charAt(stemId.length()) == '.' && id.startsWith(
        stemId);
  }
  ...
  /**
   * Intern will return a reference to a pooled version of this object, unless it is not present in
   * which case intern will add it.
   * <p>
   * The pool is also used by the ASN.1 parsers to limit the number of duplicated OID objects in
   * circulation.
   * </p>
   *
