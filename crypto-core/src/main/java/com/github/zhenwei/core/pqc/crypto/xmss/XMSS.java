...
//    }
  ...
  public void importState(byte[] privateKey, byte[] publicKey) {
    ...
    XMSSPublicKeyParameters tmpPublicKey = new XMSSPublicKeyParameters.Builder(
        params).withPublicKey(publicKey)
        .build();
  ...
  public boolean verifySignature(byte[] message, byte[] signature, byte[] publicKey)
    ...
    signer.init(false,
        new XMSSPublicKeyParameters.Builder(getParams()).withPublicKey(publicKey).build());
  ...
  protected void setRoot(byte[] root) {
    privateKey = new XMSSPrivateKeyParameters.Builder(params)
        ...
        .withPublicSeed(getPublicSeed()).withRoot(root).withBDSState(privateKey.getBDSState())
        .build();
    publicKey = new XMSSPublicKeyParameters.Builder(params).withRoot(root)
        .withPublicSeed(getPublicSeed())
        .build();
  }
  ...
  protected void setPublicSeed(byte[] publicSeed) {
    privateKey = new XMSSPrivateKeyParameters.Builder(params)
        .withSecretKeySeed(privateKey.getSecretKeySeed())
        .withSecretKeyPRF(privateKey.getSecretKeyPRF())
        .withPublicSeed(publicSeed).withRoot(getRoot()).withBDSState(privateKey.getBDSState())
        .build();
    publicKey = new XMSSPublicKeyParameters.Builder(params).withRoot(getRoot())
