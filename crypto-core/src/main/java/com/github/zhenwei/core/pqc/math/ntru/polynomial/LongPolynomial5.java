 ...
 * multiplication by a ternary polynomial.<br> Coefficients can be between 0 and 2047 and are stored
 * in bits 0..11, 12..23, ..., 48..59 of a <code>long</code> number.
 */
public class LongPolynomial5 {
  ...
  /**
   * Constructs a <code>LongPolynomial5</code> from a <code>IntegerPolynomial</code>. The two
  ...
  /**
   * Multiplies the polynomial with a <code>TernaryPolynomial</code>, taking the indices mod N and
   * the values mod 2048.
   */
  public LongPolynomial5 mult(TernaryPolynomial poly2) {
    long[][] prod = new long[5][coeffs.length + (poly2.size() + 4) / 5
        - 1];   // intermediate results, the subarrays are shifted by 0,...,4 coefficients
