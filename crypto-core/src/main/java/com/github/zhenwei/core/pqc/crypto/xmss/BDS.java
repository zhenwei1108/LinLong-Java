...
public final class BDS
   ...
   */
  BDS(XMSSParameters params, byte[] publicSeed, byte[] secretKeySeed,
  ...
  private void initialize(byte[] publicSeed, byte[] secretSeed, OTSHashAddress otsHashAddress) {
    ...
    for (int indexLeaf = 0; indexLeaf < (1 << treeHeight); indexLeaf++) {
      ...
      lTreeAddress = (LTreeAddress) new LTreeAddress.Builder().withLayerAddress(
              lTreeAddress.getLayerAddress())
      ...
      while (!stack.isEmpty() && stack.peek().getHeight() == node.getHeight()) {
        ...
        hashTreeAddress = (HashTreeAddress) new HashTreeAddress.Builder()
            ...
            .withKeyAndMask(hashTreeAddress.getKeyAndMask())
            .build();
      }
      /* push to stack */
      stack.push(node);
  ...
  private void nextAuthenticationPath(byte[] publicSeed, byte[] secretSeed,
      OTSHashAddress otsHashAddress) {
    if (otsHashAddress == null) {
      throw new NullPointerException("otsHashAddress == null");
    }
    ...
    if (tau == 0) {
       ...
       */
      wotsPlus.importKeys(wotsPlus.getWOTSPlusSecretKey(secretSeed, otsHashAddress), publicSeed);
      XMSSNode node = XMSSNodeUtil.randomizeHash(wotsPlus, authenticationPath.get(tau - 1),
          keep.get(tau - 1), hashTreeAddress);
      node = new XMSSNode(node.getHeight() + 1, node.getValue());
