 ...
 * 字典表，规定了输出范围。
 * radix，字典表数量，对于数字，0-9十个数字即可，对于字母则26*2（大小写）。
 * key，密钥，长度一般为 16，24，32 （貌似描述的是AES密钥）
 * tweak，相当于第二密钥，防止强度不够。解决因局部加密而导致结果冲突问题。
 */
public abstract class FPEEngine {
  ...
  protected abstract int encryptBlock(byte[] inBuf, int inOff, int length, byte[] outBuf,
      int outOff);

  protected abstract int decryptBlock(byte[] inBuf, int inOff, int length, byte[] outBuf,
      int outOff);
}