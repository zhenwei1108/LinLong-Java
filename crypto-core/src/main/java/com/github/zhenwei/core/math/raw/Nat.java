...
//        }
  ...
  public static int csub(int len, int mask, int[] x, int xOff, int[] y, int yOff, int[] z,
      int zOff) {
    long MASK = -(mask & 1) & M;
    long c = 0;
    for (int i = 0; i < len; ++i) {
...
//        assert c == 0L || c == -1L;
  ...
  public static void mul(int[] x, int xOff, int xLen, int[] y, int yOff, int yLen, int[] zz,
      int zzOff) {
...
//        assert bits > 0 && bits < 32;
  ...
  public static long shiftUpBits64(int len, long[] x, int xOff, int bits, long c, long[] z,
      int zOff) {
//        assert bits > 0 && bits < 64;
    for (int i = 0; i < len; ++i) {
      long next = x[xOff + i];
