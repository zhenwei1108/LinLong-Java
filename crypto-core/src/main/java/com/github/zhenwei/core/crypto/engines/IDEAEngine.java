 ...
 * implementation summary by Fauzan Mirza (F.U.Mirza@sheffield.ac.uk). (barring 1 typo at the end of
 * the mulinv function!).
 * <p>
 * It can be found at ftp://ftp.funet.fi/pub/crypt/cryptography/symmetric/idea/
 * <p>
 * Note: This algorithm was patented in the USA, Japan and Europe. These patents expired in
 * 2011/2012.
 */
public class IDEAEngine
  ...
  public void init(
    ...
    throw new IllegalArgumentException(
        "invalid parameter passed to IDEA init - " + params.getClass().getName());
  }
  ...
  /**
   * return x = x * y where the multiplication is done modulo 65537 (0x10001) (as defined in the
   * IDEA specification) and a zero input is taken to be 65536 (0x10000).
   *
   * @param x the x value
   * @param y the y value
   * @return x = x * y
  ...
  /**
   * The following function is used to expand the user key to the encryption subkey. The first 16
   * bytes are the user key, and the rest of the subkey is calculated by rotating the previous 16
   * bytes by 25 bits to the left, and so on until the subkey is completed.
   */
  ...
  /**
   * The function to invert the encryption subkey to the decryption subkey. It also involves the
