...
/**
 * Class holding methods for point multiplication based on the window &tau;-adic nonadjacent form
 * (WTNAF). The algorithms are based on the paper "Improved Algorithms for Arithmetic on Anomalous
 * Binary Curves" by Jerome A. Solinas. The paper first appeared in the Proceedings of Crypto 1997.
 */
class Tnaf {
   ...
   * time, but keeps space requirements for precomputation low. For typical curves, a value of 5 or
   * 6 results in a better running time. When changing this value, the
   * <code>&alpha;<sub>u</sub></code>'s must be computed differently, see
   * e.g. "Guide to Elliptic Curve Cryptography", Darrel Hankerson, Alfred Menezes, Scott Vanstone,
   * Springer-Verlag New York Inc., 2004, p. 121-122
   */
  ...
  public static byte[] tauAdicNaf(byte mu, ZTauElement lambda) {
    ...
    while (!((r0.equals(ECConstants.ZERO)) && (r1.equals(ECConstants.ZERO)))) {
      // If r0 is odd
      if (r0.testBit(0)) {
        u[i] = (byte) ECConstants.TWO.subtract((r0.subtract(r1.shiftLeft(1))).mod(ECConstants.FOUR))
            .intValue();
   ...
   *
   * @param curve The elliptic curve from which to obtain <code>&mu;</code>. The curve must be a
   *              Koblitz curve, i.e. <code>a</code> equals
...
//            System.out.println();
  ...
  /**
   * Computes the auxiliary value <code>t<sub>w</sub></code>. If the width is 4, then for <code>mu =
   * 1</code>, <code>t<sub>w</sub> = 6</code> and for
...
//            System.out.println("tw = " + tw);
   ...
   * @throws IllegalArgumentException if <code>curve</code> is not a Koblitz curve (Anomalous Binary
   *                                  Curve, ABC).
   */
  ...
  /**
   * Multiplies a {@link com.github.zhenwei.core.math.ec.ECPoint.AbstractF2m ECPoint.AbstractF2m} by
   * a <code>BigInteger</code> using the reduced <code>&tau;</code>-adic NAF (RTNAF) method.
   *
  ...
  /**
   * Multiplies a {@link com.github.zhenwei.core.math.ec.ECPoint.AbstractF2m ECPoint.AbstractF2m} by
   * an element <code>&lambda;</code> of <code><b>Z</b>[&tau;]</code> using the
