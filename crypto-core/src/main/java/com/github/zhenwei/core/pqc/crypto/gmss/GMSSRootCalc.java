...
/**
 * This class computes a whole Merkle tree and saves the needed values for AuthPath computation. It
 * is used for precomputation of the root of a following tree. After initialization, 2^H updates are
 * required to complete the root. Every update requires one leaf value as parameter. While computing
 * the root all initial values for the authentication path algorithm (treehash, auth, retain) are
...
public class GMSSRootCalc {
  ...
  /**
   * An array of strings containing the name of the hash function used to construct the
   * authentication trees and used by the OTS.
   */
  ...
  /**
   * temporary integer that stores the height of the next treehash instance that gets initialized
   * with a seed
   */
   ...
   * @param digestProvider an array of strings, containing the name of the used hash function and
   *                       PRNG and the name of the corresponding provider
   */
   ...
   * Additionally counts the seeds used. This method is used when performing the updates for
   * TREE++.
   *
  ...
  public byte[][] getStatByte() {
    ...
    byte[][] statByte = new byte[1 + heightOfTree
        + tailLength][64]; //FIXME: messDigestTree.getByteLength()
    statByte[0] = root;
