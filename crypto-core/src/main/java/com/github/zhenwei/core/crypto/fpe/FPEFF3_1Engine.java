...
public class FPEFF3_1Engine
  ...
  public void init(boolean forEncryption, CipherParameters parameters) {
    ...
    baseCipher.init(!fpeParameters.isUsingInverseFunction(),
        new KeyParameter(Arrays.reverse(fpeParameters.getKey().getKey())));
  ...
  protected int encryptBlock(byte[] inBuf, int inOff, int length, byte[] outBuf, int outOff) {
    ...
    if (fpeParameters.getRadix() > 256) {
      enc = toByteArray(
          SP80038G.encryptFF3_1w(baseCipher, fpeParameters.getRadix(), fpeParameters.getTweak(),
              toShortArray(inBuf), inOff, length / 2));
    } else {
      enc = SP80038G.encryptFF3_1(baseCipher, fpeParameters.getRadix(), fpeParameters.getTweak(),
          inBuf, inOff, length);
    }
  ...
  protected int decryptBlock(byte[] inBuf, int inOff, int length, byte[] outBuf, int outOff) {
    ...
    if (fpeParameters.getRadix() > 256) {
      dec = toByteArray(
          SP80038G.decryptFF3_1w(baseCipher, fpeParameters.getRadix(), fpeParameters.getTweak(),
              toShortArray(inBuf), inOff, length / 2));
    } else {
      dec = SP80038G.decryptFF3_1(baseCipher, fpeParameters.getRadix(), fpeParameters.getTweak(),
          inBuf, inOff, length);
    }
