...
/**
 * A set of parameters for NtruSign. Several predefined parameter sets are available and new ones
...
public class NTRUSigningKeyGenerationParameters
   ...
   */
  public static final NTRUSigningKeyGenerationParameters APR2011_439 = new NTRUSigningKeyGenerationParameters(
      439, 2048, 146, 1, BASIS_TYPE_TRANSPOSE, 0.165, 490, 280, false, true, KEY_GEN_ALG_RESULTANT,
      new SHA256Digest());
  ...
  public static final NTRUSigningKeyGenerationParameters APR2011_439_PROD = new NTRUSigningKeyGenerationParameters(
      439, 2048, 9, 8, 5, 1, BASIS_TYPE_TRANSPOSE, 0.165, 490, 280, false, true,
      KEY_GEN_ALG_RESULTANT, new SHA256Digest());

  /**
   * Gives 256 bits of security
   */
  public static final NTRUSigningKeyGenerationParameters APR2011_743 = new NTRUSigningKeyGenerationParameters(
      743, 2048, 248, 1, BASIS_TYPE_TRANSPOSE, 0.127, 560, 360, true, false, KEY_GEN_ALG_RESULTANT,
      new SHA512Digest());

  /**
   * Like <code>APR2011_439</code>, this parameter set gives 256 bits of security but uses
   * product-form polynomials
   */
  public static final NTRUSigningKeyGenerationParameters APR2011_743_PROD = new NTRUSigningKeyGenerationParameters(
      743, 2048, 11, 11, 15, 1, BASIS_TYPE_TRANSPOSE, 0.127, 560, 360, true, false,
      KEY_GEN_ALG_RESULTANT, new SHA512Digest());

  /**
   * Generates key pairs quickly. Use for testing only.
   */
  public static final NTRUSigningKeyGenerationParameters TEST157 = new NTRUSigningKeyGenerationParameters(
      157, 256, 29, 1, BASIS_TYPE_TRANSPOSE, 0.38, 200, 80, false, false, KEY_GEN_ALG_RESULTANT,
      new SHA256Digest());
  /**
   * Generates key pairs quickly. Use for testing only.
   */
  public static final NTRUSigningKeyGenerationParameters TEST157_PROD = new NTRUSigningKeyGenerationParameters(
      157, 256, 5, 5, 8, 1, BASIS_TYPE_TRANSPOSE, 0.38, 200, 80, false, false,
      KEY_GEN_ALG_RESULTANT, new SHA256Digest());
   ...
   * @param q            modulus
   * @param d            number of -1's in the private polynomials <code>f</code> and
   *                     <code>g</code>
   ...
   * @param keyGenAlg    <code>RESULTANT</code> produces better bases, <code>FLOAT</code> is
   *                     slightly faster. <code>RESULTANT</code> follows the EESS standard while
   *                     <code>FLOAT</code> is described in Hoffstein et al: An Introduction to
   ...
   * @param N            number of polynomial coefficients
   * @param q            modulus
   * @param d1           number of -1's in the private polynomials <code>f</code> and
   *                     <code>g</code>
   * @param d2           number of -1's in the private polynomials <code>f</code> and
   *                     <code>g</code>
   * @param d3           number of -1's in the private polynomials <code>f</code> and
   *                     <code>g</code>
   * @param B            number of perturbations
   * @param basisType    whether to use the standard or transpose lattice
   * @param beta         balancing factor for the transpose lattice
   ...
   *                     the <code>getDigestLength()</code> method.
   */
  public NTRUSigningKeyGenerationParameters(int N, int q, int d1, int d2, int d3, int B,
      int basisType, double beta, double normBound, double keyNormBound, boolean primeCheck,
  ...
  public NTRUSigningKeyGenerationParameters clone() {
    if (polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_SIMPLE) {
      return new NTRUSigningKeyGenerationParameters(N, q, d, B, basisType, beta, normBound,
          keyNormBound, primeCheck, sparse, keyGenAlg, hashAlg);
    } else {
      return new NTRUSigningKeyGenerationParameters(N, q, d1, d2, d3, B, basisType, beta, normBound,
          keyNormBound, primeCheck, sparse, keyGenAlg, hashAlg);
    }
  }
  ...
  public String toString() {
    ...
    output.append(" B=" + B + " basisType=" + basisType + " beta=" + format.format(beta) +
        ...
        " prime=" + primeCheck + " sparse=" + sparse + " keyGenAlg=" + keyGenAlg + " hashAlg="
        + hashAlg + ")");
