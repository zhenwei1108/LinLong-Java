...
/**
 * ASN.1 TaggedObject - in ASN.1 notation this is any object preceded by a [n] where n is some
 * number - these are assumed to follow the construction rules (as with sequences).
 */
...
//      else if (obj instanceof ASN1TaggedObjectParser)
   ...
   * <p>
   * If the object implements ASN1Choice the tag style will always be changed to explicit in
   * accordance with the ASN.1 encoding rules.
   * </p>
   *
  ...
  public int hashCode() {
    return (tagClass * 7919) ^ tagNo ^ (isExplicit() ? 0x0F : 0xF0) ^ obj.toASN1Primitive()
        .hashCode();
  }
   ...
   * <p>
   * Note: if the object has been read from an input stream, the only time you can be sure if
   * isExplicit is returning the true state of affairs is if it returns false. An implicitly tagged
   * object may appear to be explicitly tagged, so you need to understand the context under which
   * the reading was done as well, see getObject below.
   */
   ...
   * <p>
   * Note: tagged objects are generally context dependent. If you're trying to extract a tagged
   * object you should be going via the appropriate getInstance method.
   *
   * @deprecated Tagged objects now include the {@link #getTagClass() tag class}. This method will
   * raise an exception if it is not {@link BERTags#CONTEXT_SPECIFIC}. Use {@link
   * #getBaseUniversal(boolean, int)} only after confirming the expected tag class.
   */
  ...
  /**
   * Needed for open types, until we have better type-guided parsing support. Use sparingly for
   * other purposes, and prefer {@link #getExplicitBaseTagged()}, {@link #getImplicitBaseTagged(int,
   * int)} or {@link #getBaseUniversal(boolean, int)} where possible. Before using, check for
   * matching tag {@link #getTagClass() class} and {@link #getTagNo() number}.
   */
  ...
  /**
   * Needed for open types, until we have better type-guided parsing support. Use sparingly for
   * other purposes, and prefer {@link #getExplicitBaseTagged()} or {@link
   * #getBaseUniversal(boolean, int)} where possible. Before using, check for matching tag {@link
   * #getTagClass() class} and {@link #getTagNo() number}.
   */
  ...
  /**
   * Note: tagged objects are generally context dependent. Before trying to extract a tagged object
   * this way, make sure you have checked that both the {@link #getTagClass() tag class} and {@link
   * #getTagNo() tag number} match what you are looking for.
   *
   * @param declaredExplicit Whether the tagged type for this object was declared EXPLICIT.
   * @param tagNo            The universal {@link BERTags tag number} of the expected base object.
   */
  ...
  static ASN1Primitive createConstructedDL(int tagClass, int tagNo,
    ...
    ASN1TaggedObject taggedObject = maybeExplicit
        ...
        : new DLTaggedObject(PARSED_IMPLICIT, tagClass, tagNo,
            DLFactory.createSequence(contentsElements));
  ...
  static ASN1Primitive createConstructedIL(int tagClass, int tagNo,
      ASN1EncodableVector contentsElements) {
    boolean maybeExplicit = (contentsElements.size() == 1);

    ASN1TaggedObject taggedObject = maybeExplicit
        ? new BERTaggedObject(PARSED_EXPLICIT, tagClass, tagNo, contentsElements.get(0))
        : new BERTaggedObject(PARSED_IMPLICIT, tagClass, tagNo,
            BERFactory.createSequence(contentsElements));
