 ...
 * <p>
 * Blake2b offers a built-in support for a salt for randomized hashing and a personal string for
...
public class Blake2bDigest
  ...
  /**
   * Blake2b for authentication ("Prefix-MAC mode"). After calling the doFinal() method, the key
   * will remain to be used for further computations of this instance. The key can be overwritten
   * using the clearKey() method.
   *
   ...
   * the doFinal() method, the key, the salt and the personal string will remain and might be used
  ...
  public void update(byte[] message, int offset, int len) {
    ...
    for (messagePos = offset + remainingLength; messagePos < blockWiseLastPos;
        messagePos += BLOCK_LENGTH_BYTES) { // block wise 128 bytes
      // without buffer:
      t0 += BLOCK_LENGTH_BYTES;
      if (t0 == 0) {
        t1++;
  ...
  private void G(long m1, long m2, int posA, int posB, int posC, int posD) {
    ...
    internalState[posB] = Longs.rotateRight(internalState[posB] ^ internalState[posC],
