...
/**
 * A polynomial with {@link BigDecimal} coefficients. Some methods (like <code>add</code>) change
...
public class BigDecimalPolynomial {
  ...
  /**
   * Constructs a <code>BigDecimalPolynomial</code> from a <code>BigIntPolynomial</code>. The two
  ...
  /**
   * Multiplies the polynomial by another. Does not change this polynomial but returns the result as
   * a new polynomial.
   *
  ...
  private BigDecimal[] copyOfRange(BigDecimal[] a, int from, int to) {
    ...
    System.arraycopy(a, from, tmp, 0,
        (a.length - from) < newLength ? (a.length - from) : newLength);
