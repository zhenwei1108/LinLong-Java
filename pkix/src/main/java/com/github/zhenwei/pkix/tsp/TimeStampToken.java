...
import com.github.zhenwei.pkix.cms.CMSProcessable;
import com.github.zhenwei.pkix.cms.CMSSignedData;
import com.github.zhenwei.pkix.cms.SignerId;
import com.github.zhenwei.pkix.cms.SignerInformation;
...
public class TimeStampToken {
  ...
  public TimeStampToken(CMSSignedData signedData)
    ...
    if (!this.tsToken.getSignedContentTypeOID()
        .equals(PKCSObjectIdentifiers.id_ct_TSTInfo.getId())) {
    ...
    try {
      ...
      if (attr != null) {
        ...
        attr = tsaSignerInfo.getSignedAttributes()
    ...
    try {
      ...
      if (certID.getIssuerSerial() != null) {
        ...
        for (int i = 0; i != names.length; i++) {
          if (names[i].getTagNo() == 4 && X500Name.getInstance(names[i].getName())
              .equals(X500Name.getInstance(issuerSerial.getName()))) {
            found = true;
            break;
          }
        }
   ...
   * @throws TSPException if the signature cannot be processed or the provider cannot match the
   *                      algorithm.
   */
