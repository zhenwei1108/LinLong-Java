...
import com.github.zhenwei.pkix.util.asn1.cmp.InfoTypeAndValue;
...
import com.github.zhenwei.pkix.util.asn1.cmp.PKIHeaderBuilder;
  ...
  private ProtectedPKIMessage finaliseMessage(PKIHeader header, DERBitString protection) {
    if (!extraCerts.isEmpty()) {
      ...
      for (int i = 0; i != cmpCerts.length; i++) {
        cmpCerts[i] = new CMPCertificate(
            ((X509CertificateHolder) extraCerts.get(i)).toASN1Structure());
      }
