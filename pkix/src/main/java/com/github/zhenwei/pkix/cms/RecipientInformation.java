...
public abstract class RecipientInformation {
   ...
   * This will only happen if we are dealing with authenticated data and authenticated attributes
   * are present.
   *
  ...
  /**
   * Return the MAC calculated for the recipient. Note: this call is only meaningful once all the
  ...
  /**
   * Return a CMSTypedStream representing the content in the EnvelopedData after recovering the
   * content encryption/MAC key using the passed in Recipient.
   *
   * @param recipient recipient object to use to recover content encryption key
   * @return the content inside the EnvelopedData this RecipientInformation is associated with.
   * @throws CMSException if the content-encryption/MAC key cannot be recovered.
  ...
  public CMSTypedStream getContentStream(Recipient recipient)
    ...
    if (additionalData != null) {
      if (additionalData.isAead()) {
        ...
        operator.getAADStream()
            .write(additionalData.getAuthAttributes().getEncoded(ASN1Encoding.DER));

        return new CMSTypedStream(secureReadable.getContentType(),
            operator.getInputStream(secureReadable.getInputStream()));
      } else {
