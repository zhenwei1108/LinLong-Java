...
import com.github.zhenwei.pkix.cms.KeyAgreeRecipientInfoGenerator;
...
import com.github.zhenwei.pkix.util.asn1.cms.KeyAgreeRecipientIdentifier;
import com.github.zhenwei.pkix.util.asn1.cms.OriginatorPublicKey;
import com.github.zhenwei.pkix.util.asn1.cms.RecipientEncryptedKey;
...
public class JceKeyAgreeRecipientInfoGenerator
  ...
  public JceKeyAgreeRecipientInfoGenerator(ASN1ObjectIdentifier keyAgreementOID,
    ...
    super(keyAgreementOID, SubjectPublicKeyInfo.getInstance(senderPublicKey.getEncoded()),
        keyEncryptionOID);
  ...
  public ASN1Sequence generateRecipientEncryptedKeys(AlgorithmIdentifier keyAgreeAlgorithm,
      AlgorithmIdentifier keyEncryptionAlgorithm, GenericKey contentEncryptionKey)
      throws CMSException {
      ...
      try {
        ...
        if (CMSUtils.isMQV(keyAgreementOID)) {
          ...
          byte[] ukmKeyingMaterial = ecc_cms_Generator.generateKDFMaterial(keyEncryptionAlgorithm,
              keySizeProvider.getKeySize(keyEncAlg), userKeyingMaterial);
        ...
        if (keyEncAlg.equals(CryptoProObjectIdentifiers.id_Gost28147_89_None_KeyWrap)
          ...
          keyEncryptionCipher.init(Cipher.WRAP_MODE, keyEncryptionKey,
              new GOST28147WrapParameterSpec(
          ...
          byte[] encryptedKeyBytes = keyEncryptionCipher.wrap(
              helper.getJceKey(contentEncryptionKey));
  ...
  protected byte[] getUserKeyingMaterial(AlgorithmIdentifier keyAgreeAlg)
    ...
    if (ephemeralKP != null) {
      ...
      try {
        if (userKeyingMaterial != null) {
          return new MQVuserKeyingMaterial(originatorPublicKey,
              new DEROctetString(userKeyingMaterial)).getEncoded();
        } else {
