...
public class JcaContentVerifierProviderBuilder {
  ...
  private ContentVerifier createCompositeVerifier(AlgorithmIdentifier compAlgId,
      PublicKey publicKey)
      throws OperatorCreationException {
    if (publicKey instanceof CompositePublicKey) {
  ...
  private class RawSigVerifier
    ...
    public boolean verify(byte[] digest, byte[] expected) {
      try {
        ...
        throw new RuntimeOperatorException("exception obtaining raw signature: " + e.getMessage(),
            e);
      } finally {
        // we need to do this as in some PKCS11 implementations the session associated with the init of the
