...
import com.github.zhenwei.core.asn1.x509.TBSCertList;
import com.github.zhenwei.core.asn1.x509.Time;
import com.github.zhenwei.core.asn1.x509.V2TBSCertListGenerator;
...
public class X509v2CRLBuilder {
   ...
   * doesn't use a Gregorian calender so that the Time produced is compatible with other ASN.1
   * implementations.
   *
   * @param issuer     the issuer this CRL is associated with.
   * @param thisUpdate the date of this update.
   ...
   * @param reason                the reason code, as indicated in CRLReason, i.e
   *                              CRLReason.keyCompromise, or 0 if not to be used.
   * @return the current builder.
   */
  public X509v2CRLBuilder addCRLEntry(BigInteger userCertificateSerial, Date revocationDate,
      int reason) {
  ...
  /**
   * Add a CRL entry with an invalidityDate extension as well as a reasonCode extension. This is
   * used where the date of revocation might be after issues with the certificate may have
   * occurred.
   *
   * @param userCertificateSerial serial number of revoked certificate.
   * @param revocationDate        date of certificate revocation.
   * @param reason                the reason code, as indicated in CRLReason, i.e
   *                              CRLReason.keyCompromise, or 0 if not to be used.
   * @param invalidityDate        the date on which the private key for the certificate became
   *                              compromised or the certificate otherwise became invalid.
   * @return the current builder.
   */
  public X509v2CRLBuilder addCRLEntry(BigInteger userCertificateSerial, Date revocationDate,
  ...
  public X509v2CRLBuilder addCRLEntry(BigInteger userCertificateSerial, Date revocationDate,
    ...
    tbsGen.addCRLEntry(new ASN1Integer(userCertificateSerial), new Time(revocationDate),
        extensions);
  ...
  public X509v2CRLBuilder addCRL(X509CRLHolder other) {
    ...
    if (revocations != null) {
      for (Enumeration en = revocations.getRevokedCertificateEnumeration();
          en.hasMoreElements(); ) {
        tbsGen.addCRLEntry(
            ASN1Sequence.getInstance(((ASN1Encodable) en.nextElement()).toASN1Primitive()));
      }
    }
  ...
  /**
   * Add a given extension field for the standard extensions tag (tag 3) using a byte encoding of
   * the extension value.
   *
   * @param oid          the OID defining the extension type.
   * @param isCritical   true if the extension is critical, false otherwise.
  ...
  /**
   * Replace a given extension field for the standard extensions tag (tag 3) with the passed in byte
  ...
  public X509v2CRLBuilder replaceExtension(
      ...
      throws CertIOException {
    extGenerator = CertUtils.doReplaceExtension(extGenerator,
        new Extension(oid, isCritical, encodedValue));
