...
public class CMSPatchKit {
  ...
  /**
   * Create a SignerInformation based on original which uses definite-length rather than DER
   * encoding for verifying the signature on the signed attributes.
   *
  ...
  /**
   * Create a SignerInformation based on original has it's signatureAlgorithm replaced with the
   * passed in AlgorithmIdentifier.
   *
   * @param original the source SignerInformation
   */
  ...
  private static class ModEncAlgSignerInformation
    ...
    private static SignerInfo editEncAlg(SignerInfo info, AlgorithmIdentifier signatureAlgorithm) {
      return new SignerInfo(info.getSID(), info.getDigestAlgorithm(),
          info.getAuthenticatedAttributes(),
