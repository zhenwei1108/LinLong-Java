...
import com.github.zhenwei.core.asn1.cryptopro.GostR3410KeyTransport;
import com.github.zhenwei.core.asn1.cryptopro.GostR3410TransportParameters;
...
public abstract class JceKeyTransRecipient
   ...
   */
  public JceKeyTransRecipient setAlgorithmMapping(ASN1ObjectIdentifier algorithm,
   ...
   * based provider for decrypting the content. Default value: false.
   *
   * @param unwrappedKeyMustBeEncodable true if getEncoded() should return key bytes, false if not
   *                                    necessary.
   * @return this recipient.
   */
  public JceKeyTransRecipient setMustProduceEncodableUnwrappedKey(
  ...
  /**
   * Set validation of retrieved key sizes against the algorithm parameters for the encrypted key
   * where possible - default is off.
   * <p>
   * This setting will not have any affect if the encryption algorithm in the recipient does not
   * specify a particular key size, or if the unwrapper is a HSM and the byte encoding of the
   * unwrapped secret key is not available.
   * </p>
   *
   * @param doValidate true if unwrapped key's should be validated against the content encryption
    ...
    if (CMSUtils.isGOST(keyEncryptionAlgorithm.getAlgorithm())) {
      try {
        ...
        PublicKey pubKey = keyFactory.generatePublic(
            new X509EncodedKeySpec(transParams.getEphemeralPublicKey().getEncoded()));
        ...
        SecretKey key = agreement.generateSecret(
            CryptoProObjectIdentifiers.id_Gost28147_89_CryptoPro_KeyWrap.getId());

        Cipher keyCipher = helper.createCipher(
            CryptoProObjectIdentifiers.id_Gost28147_89_CryptoPro_KeyWrap);

        keyCipher.init(Cipher.UNWRAP_MODE, key,
            new GOST28147WrapParameterSpec(transParams.getEncryptionParamSet(),
                transParams.getUkm()));
        ...
        return keyCipher.unwrap(Arrays.concatenate(encKey.getEncryptedKey(), encKey.getMacKey()),
            helper.getBaseCipherName(encryptedKeyAlgorithm.getAlgorithm()), Cipher.SECRET_KEY);
      } catch (Exception e) {
        throw new CMSException("exception unwrapping key: " + e.getMessage(), e);
      }
    } else {
      JceAsymmetricKeyUnwrapper unwrapper = helper.createAsymmetricUnwrapper(keyEncryptionAlgorithm,
          recipientKey).setMustProduceEncodableUnwrappedKey(unwrappedKeyMustBeEncodable);
      ...
      try {
        Key key = helper.getJceKey(encryptedKeyAlgorithm.getAlgorithm(),
