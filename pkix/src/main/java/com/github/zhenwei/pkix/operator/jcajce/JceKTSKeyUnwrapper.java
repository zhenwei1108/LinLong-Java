...
public class JceKTSKeyUnwrapper
  ...
  public JceKTSKeyUnwrapper(AlgorithmIdentifier algorithmIdentifier, PrivateKey privKey,
  ...
  public GenericKey generateUnwrappedKey(AlgorithmIdentifier encryptedKeyAlgorithm,
    ...
    GenericHybridParameters params = GenericHybridParameters.getInstance(
        this.getAlgorithmIdentifier().getParameters());
    ...
    try {
      DEROtherInfo otherInfo = new DEROtherInfo.Builder(params.getDem(), partyUInfo,
          partyVInfo).build();
      KTSParameterSpec ktsSpec = new KTSParameterSpec.Builder(symmetricWrappingAlg, keySizeInBits,
          otherInfo.getEncoded()).withKdfAlgorithm(kemParameters.getKeyDerivationFunction())
          .build();
