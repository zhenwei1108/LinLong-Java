...
public class PKCS12PfxPdu {
  ...
  public boolean isMacValid(PKCS12MacCalculatorBuilderProvider macCalcProviderBuilder,
    ...
    if (hasMac()) {
      ...
      MacDataGenerator mdGen = new MacDataGenerator(macCalcProviderBuilder.get(
          new AlgorithmIdentifier(pfxmData.getMac().getAlgorithmId().getAlgorithm(),
  ...
  /**
   * Return a Pfx with the outer wrapper encoded as asked for. For example, Pfx is a usually a BER
   * encoded object, to get one with DefiniteLength encoding use:
   * <pre>
