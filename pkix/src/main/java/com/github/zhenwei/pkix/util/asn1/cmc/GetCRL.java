...
public class GetCRL
  ...
  public GetCRL(X500Name issuerName, GeneralName cRLName, ASN1GeneralizedTime time,
      ReasonFlags reasons) {
  ...
  private GetCRL(ASN1Sequence seq) {
    ...
    int index = 1;
    if (seq.size() > index && seq.getObjectAt(index)
        .toASN1Primitive() instanceof ASN1TaggedObject) {
      this.cRLName = GeneralName.getInstance(seq.getObjectAt(index++));
    }
    if (seq.size() > index && seq.getObjectAt(index)
        .toASN1Primitive() instanceof ASN1GeneralizedTime) {
