...
import com.github.zhenwei.pkix.cert.CertException;
    ...
    if (workingPublicKey != null) {
      try {
        ...
        if (workingPublicKey.getAlgorithm().equals(workingAlgId)) {
          ...
          validatingKeyInfo = new SubjectPublicKeyInfo(workingAlgId,
              workingPublicKey.parsePublicKey());
        }

        if (!certificate.isSignatureValid(contentVerifierProvider.build(validatingKeyInfo))) {
          throw new CertPathValidationException(
              "Certificate signature not for public key in parent");
        }
