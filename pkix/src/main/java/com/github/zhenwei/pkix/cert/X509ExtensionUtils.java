...
public class X509ExtensionUtils {
  ...
  public AuthorityKeyIdentifier createAuthorityKeyIdentifier(
    ...
    return new AuthorityKeyIdentifier(
        getSubjectKeyIdentifier(certHolder), new GeneralNames(genName),
   ...
   * @return the key identifier.
   */
  public SubjectKeyIdentifier createTruncatedSubjectKeyIdentifier(
      SubjectPublicKeyInfo publicKeyInfo) {
  ...
  private byte[] calculateIdentifier(SubjectPublicKeyInfo publicKeyInfo) {
    ...
    try {
      ...
      cOut.close();
    } catch (IOException e) {   // it's hard to imagine this happening, but yes it does!
