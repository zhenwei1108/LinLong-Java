...
public class JcaJceUtils {
  ...
  public static X509TrustManager[] getCertPathTrustManager(final Set<TrustAnchor> anchors,
      final CRL[] revocationLists) {
      ...
      public void checkServerTrusted(X509Certificate[] x509Certificates, String s)
        ...
        try {
          CertStore certStore = CertStore.getInstance("Collection",
              new CollectionCertStoreParameters(Arrays.asList(x509Certificates)), "LL");

          CertPathBuilder pathBuilder = CertPathBuilder.getInstance("PKIX", "LL");
          ...
          PKIXCertPathValidatorResult result = (PKIXCertPathValidatorResult) pathBuilder.build(
              param);
  ...
  public static void validateServerCertUsage(X509Certificate x509Certificate)
    ...
    try {
      ...
      if (keyUsage != null) {
        ...
        if (!(keyUsage.hasUsages(KeyUsage.digitalSignature) || keyUsage.hasUsages(
            KeyUsage.keyEncipherment))) {
          throw new CertificateException(
              "Key usage must be none, digitalSignature or keyEncipherment");
        }
      }
