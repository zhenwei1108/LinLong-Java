...
import com.github.zhenwei.pkix.cmc.CMCException;
import com.github.zhenwei.pkix.cmc.SimplePKIResponse;
...
public class ESTService {
  ...
  /**
   * Utility method to extract all the X509Certificates from a store using a filter and to return
   * them as an array.
   *
   * @param store    The store.
  ...
  public CACertsResponse getCACerts()
      ...
      if (resp.getStatusCode() == 200) {
        ...
        if (contentType == null || !contentType.startsWith("application/pkcs7-mime")) {
          ...
          throw new ESTException(
              ("Response : " + url.toString() + "Expecting application/pkcs7-mime ") + j, null,
        ...
        try {
          if (resp.getContentLength() != null && resp.getContentLength() > 0) {
            ...
            SimplePKIResponse spkr = new SimplePKIResponse(
                ContentInfo.getInstance((ASN1Sequence) ain.readObject()));
            caCerts = spkr.getCertificates();
            crlHolderStore = spkr.getCRLs();
          }
        } catch (Throwable ex) {
          throw new ESTException("Decoding CACerts: " + url.toString() + " " + ex.getMessage(), ex,
              resp.getStatusCode(), resp.getInputStream());
        }

      } else if (resp.getStatusCode() != 204) // 204 are No Content
      {
        throw new ESTException("Get CACerts: " + url.toString(), null, resp.getStatusCode(),
            resp.getInputStream());
      }

      caCertsResponse = new CACertsResponse(caCerts, crlHolderStore, req, resp.getSource(),
          clientProvider.isTrusted());
    ...
    if (finalThrowable != null) {
      ...
      throw new ESTException("Get CACerts: " + url.toString(), finalThrowable, resp.getStatusCode(),
          null);
    }
  ...
  public EnrollmentResponse simpleEnroll(EnrollmentResponse priorResponse)
    ...
    try {
      ESTClient client = clientProvider.makeClient();
      resp = client.doRequest(
          new ESTRequestBuilder(priorResponse.getRequestToRetry()).withClient(client).build());
   ...
   * <p>
   * If authentication is to be performed as part of TLS then this instances client keystore and
   * their keystore password need to be specified.
   *
  ...
  public EnrollmentResponse simpleEnroll(boolean reenroll,
      PKCS10CertificationRequest certificationRequest, ESTAuth auth)
      throws IOException {
    if (!clientProvider.isTrusted()) {
      throw new IllegalStateException("No trust anchors.");
    }
  ...
  public EnrollmentResponse simpleEnrollPoP(boolean reEnroll,
      final PKCS10CertificationRequestBuilder builder, final ContentSigner contentSigner,
      ESTAuth auth)
      throws IOException {
    if (!clientProvider.isTrusted()) {
      throw new IllegalStateException("No trust anchors.");
    ...
    try {
      ...
      //
      ESTRequestBuilder reqBldr = new ESTRequestBuilder("POST", url).withClient(client)
          .withConnectionListener(new ESTSourceConnectionListener() {
            public ESTRequest onConnection(Source source, ESTRequest request)
              ...
              if (source instanceof TLSUniqueProvider
                  && ((TLSUniqueProvider) source).isTLSUniqueAvailable()) {
                PKCS10CertificationRequestBuilder localBuilder = new PKCS10CertificationRequestBuilder(
                    builder);

                ByteArrayOutputStream bos = new ByteArrayOutputStream();
                byte[] tlsUnique = ((TLSUniqueProvider) source).getTLSUnique();

                localBuilder.setAttribute(PKCSObjectIdentifiers.pkcs_9_at_challengePassword,
                    new DERPrintableString(Base64.toBase64String(tlsUnique)));
                bos.write(
                    annotateRequest(localBuilder.build(contentSigner).getEncoded()).getBytes());
                bos.flush();

                ESTRequestBuilder reqBuilder = new ESTRequestBuilder(request).withData(
                    bos.toByteArray());
  ...
  protected EnrollmentResponse handleEnrollResponse(ESTResponse resp)
    ...
    if (resp.getStatusCode() == 202) {
      ...
      if (rt == null) {
        throw new ESTException(
            "Got Status 202 but not Retry-After header from: " + req.getURL().toString());
      }
      ...
      try {
        ...
        try {
          ...
          throw new ESTException(
              "Unable to parse Retry-After header:" + req.getURL().toString() + " "
                  + ex.getMessage(), null,
              resp.getStatusCode(), resp.getInputStream());
        }
      }
  ...
  public CSRRequestResponse getCSRAttributes()
    ...
    try {
      ...
      ESTRequest req = new ESTRequestBuilder("GET", url).withClient(client)
          .build(); //    new ESTRequest("GET", url, null);
      resp = client.doRequest(req);
      ...
      switch (resp.getStatusCode()) {
        case 200:
          try {
            ...
            throw new ESTException("Decoding CACerts: " + url.toString() + " " + ex.getMessage(),
    ...
    if (finalThrowable != null) {
      ...
      throw new ESTException(finalThrowable.getMessage(), finalThrowable, resp.getStatusCode(),
          null);
    }
  ...
  private String verifyLabel(String label) {
    ...
    if (label.length() == 0) {
      throw new IllegalArgumentException(
          "Label set but after trimming '/' is not zero length string.");
    }
  ...
  private String verifyServer(String server) {
    try {
      ...
      if (server.contains("://")) {
        throw new IllegalArgumentException(
            "Server contains scheme, must only be <dnsname/ipaddress>:port, https:// will be added arbitrarily.");
      }
