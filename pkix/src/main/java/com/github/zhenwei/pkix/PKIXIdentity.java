package com.github.zhenwei.pkix;
...
import com.github.zhenwei.pkix.cms.RecipientId;
...
public class PKIXIdentity {
  ...
  /**
   * Base constructor - a private key and its associated certificate chain. The chain should be
   * ordered so that certificateHolders[0] is the matching public key for privKey.
   *
  ...
  public RecipientId getRecipientId() {
    ...
    return new KeyTransRecipientId(certificateHolders[0].getIssuer(),
        certificateHolders[0].getSerialNumber(), getSubjectKeyIdentifier());
  }

  private byte[] getSubjectKeyIdentifier() {
    SubjectKeyIdentifier subId = SubjectKeyIdentifier.fromExtensions(
        certificateHolders[0].getExtensions());
