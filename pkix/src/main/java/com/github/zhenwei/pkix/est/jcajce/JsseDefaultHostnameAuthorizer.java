...
public class JsseDefaultHostnameAuthorizer
   ...
   * Base constructor.
   * <p>
   * The authorizer attempts to perform matching (including the use of the wildcard) in accordance
   * with RFC 6125.
   * </p>
   * <p>
   * Known suffixes is a list of public domain suffixes that can't be used as wild cards for example
   * *.com, or c*c.com, as a dns wildcard could match every/most .com domains if a registrar were
   * issue it. If *.com is in the known suffixes list will not be allowed to match.
   * </p>
   *
   * @param knownSuffixes a set of suffixes that cannot be wild-carded, e.g. { ".com", ".net",
   *                      ".org" }
   */
  ...
  public static boolean isValidNameMatch(String name, String dnsName, Set<String> suffixes)
    ...
    if (dnsName.contains("*")) {
      ...
      if (wildIndex == dnsName.lastIndexOf("*")) {
        ...
        if (suffixes != null && suffixes.contains(
            Strings.toLowerCase(dnsName.substring(dnsDotIndex)))) {
        ...
        if (wildIndex > 0) {
          if (loweredName.startsWith(dnsName.substring(0, wildIndex)) && loweredName.endsWith(
              end)) {
            return
                loweredName.substring(wildIndex, loweredName.length() - end.length()).indexOf('.')
                    < 0;
          } else {
            return false;
          }
