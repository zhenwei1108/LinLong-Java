...
public abstract class JceKTSKeyTransRecipient
   ...
   */
  public JceKTSKeyTransRecipient setAlgorithmMapping(ASN1ObjectIdentifier algorithm,
  ...
  protected Key extractSecretKey(AlgorithmIdentifier keyEncryptionAlgorithm,
      ...
      throws CMSException {
    JceKTSKeyUnwrapper unwrapper = helper.createAsymmetricUnwrapper(keyEncryptionAlgorithm,
        recipientKey, ANONYMOUS_SENDER, partyVInfo);
  ...
  protected static byte[] getPartyVInfoFromRID(KeyTransRecipientId recipientId)
    ...
    if (recipientId.getSerialNumber() != null) {
      return new IssuerAndSerialNumber(recipientId.getIssuer(),
          recipientId.getSerialNumber()).getEncoded(ASN1Encoding.DER);
    } else {
