 ...
 * method setDigestCalculatorProvider <b>must</b> be called once to configure the class to do the
...
public class AttributeCertificateHolder
  ...
  /**
   * Create a holder using the baseCertificateID option based on the passed in associated
   * certificate,
   *
  ...
  public AttributeCertificateHolder(int digestedObjectType,
      ASN1ObjectIdentifier digestAlgorithm, ASN1ObjectIdentifier otherObjectTypeID,
      byte[] objectDigest) {
    ...
    if (holder.getObjectDigestInfo() != null) {
      try {
        DigestCalculator digCalc = digestCalculatorProvider.get(
            holder.getObjectDigestInfo().getDigestAlgorithm());
