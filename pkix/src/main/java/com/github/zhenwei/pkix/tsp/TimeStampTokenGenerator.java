...
import com.github.zhenwei.pkix.cms.CMSAttributeTableGenerationException;
...
import com.github.zhenwei.pkix.util.asn1.ess.ESSCertID;
import com.github.zhenwei.pkix.util.asn1.ess.ESSCertIDv2;
import com.github.zhenwei.pkix.util.asn1.ess.SigningCertificate;
import com.github.zhenwei.pkix.util.asn1.ess.SigningCertificateV2;
...
/**
 * Currently the class supports ESSCertID by if a digest calculator based on SHA1 is passed in,
 * otherwise it uses ESSCertIDv2. In the event you need to pass both types, you will need to
 * override the SignedAttributeGenerator for the SignerInfoGeneratorBuilder you are using. For the
 * default for ESSCertIDv2 the code will look something like the following:
 * <pre>
...
public class TimeStampTokenGenerator {
  ...
  public TimeStampTokenGenerator(
    ...
    try {
      ...
      if (digestCalculator.getAlgorithmIdentifier().getAlgorithm()
          .equals(OIWObjectIdentifiers.idSHA1)) {
        final ESSCertID essCertid = new ESSCertID(digestCalculator.getDigest(),
            isIssuerSerialIncluded ? new IssuerSerial(
                new GeneralNames(new GeneralName(assocCert.getIssuer())),
                assocCert.getSerialNumber())
                : null);

        this.signerInfoGen = new SignerInfoGenerator(signerInfoGen,
            new CMSAttributeTableGenerator() {
              public AttributeTable getAttributes(Map parameters)
                  throws CMSAttributeTableGenerationException {
                AttributeTable table = signerInfoGen.getSignedAttributeTableGenerator()
        ...
        AlgorithmIdentifier digAlgID = new AlgorithmIdentifier(
            digestCalculator.getAlgorithmIdentifier().getAlgorithm());
  ...
  /**
   * Add a Store of otherRevocationData to the CRL set to be included with the generated
   * TimeStampToken.
   *
   * @param otherRevocationInfoFormat the OID specifying the format of the otherRevocationInfo
   *                                  data.
  ...
  /**
   * Set the resolution of the time stamp - R_SECONDS (the default), R_TENTH_OF_SECONDS,
   * R_MICROSECONDS, R_MILLISECONDS
   *
  ...
  /**
   * Set a Locale for time creation - you may need to use this if the default locale doesn't use a
   * Gregorian calender so that the GeneralizedTime produced is compatible with other ASN.1
   * implementations.
   *
  ...
  public TimeStampToken generate(
    ...
    if (resolution == R_SECONDS) {
      timeStampTime = (locale == null) ? new ASN1GeneralizedTime(genTime)
          : new ASN1GeneralizedTime(genTime, locale);
    } else {
    ...
    try {
      ...
      if (!otherRevoc.isEmpty()) {
        for (Iterator it = otherRevoc.keySet().iterator(); it.hasNext(); ) {
          ...
          signedDataGenerator.addOtherRevocationInfo(format,
              new CollectionStore((Collection) otherRevoc.get(format)));
        }
      }
  ...
  private ASN1GeneralizedTime createGeneralizedTime(Date time)
    ...
    SimpleDateFormat dateF =
        (locale == null) ? new SimpleDateFormat(format) : new SimpleDateFormat(format, locale);
