...
import com.github.zhenwei.provider.x509.extension.X509ExtensionUtil;
 ...
 * @see com.github.zhenwei.provider.jce.provider.X509StoreCRLCollection
 */
public class X509CRLStoreSelector
  ...
  /**
   * Returns if the issuing distribution point criteria should be applied. Defaults to
   * <code>false</code>.
   * <p>
   * You may also set the issuing distribution point criteria if not a missing issuing distribution
  ...
  /**
   * Sets the attribute certificate being checked. This is not a criterion. Rather, it is optional
   * information that may help a {@link X509Store} find CRLs that would be relevant when checking
   * revocation for the specified attribute certificate. If <code>null</code> is specified, then no
  ...
  /**
   * Returns if this selector must match CRLs with the delta CRL indicator extension set. Defaults
   * to <code>false</code>.
   *
   * @return Returns <code>true</code> if only CRLs with the delta CRL indicator extension are
   * selected.
   */
  ...
  /**
   * If this is set to <code>true</code> the CRL reported contains the delta CRL indicator CRL
   * extension.
   * <p>
   * {@link #setCompleteCRLEnabled(boolean)} and {@link #setDeltaCRLIndicatorEnabled(boolean)}
   * excluded each other.
   *
  ...
  /**
   * Sets the maximum base CRL number. Setting to <code>null</code> disables this cheack.
   * <p>
   * This is only meaningful for delta CRLs. Complete CRLs must have a CRL number which is greater
   * or equal than the base number of the corresponding CRL.
   *
  ...
  /**
   * Returns the issuing distribution point. Defaults to <code>null</code>, which is a missing
   * issuing distribution point extension.
   * <p>
   * The internal byte array is cloned before it is returned.
   * <p>
   * The criteria must be enable with {@link #setIssuingDistributionPointEnabled(boolean)}.
   *
   ...
   * <p>
   * The issuing distribution point extension is a CRL extension which identifies the scope and the
   * distribution point of a CRL. The scope contains among others information about revocation
   * reasons contained in the CRL. Delta CRLs and complete CRLs must have matching issuing
   * distribution points.
   * <p>
   * The byte array is cloned to protect against subsequent modifications.
   * <p>
   * You must also enable or disable this criteria with {@link #setIssuingDistributionPointEnabled(boolean)}.
   *
   * @param issuingDistributionPoint The issuing distribution point to set. This is the DER encoded
