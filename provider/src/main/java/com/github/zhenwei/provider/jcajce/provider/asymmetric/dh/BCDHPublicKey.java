...
import com.github.zhenwei.provider.jcajce.spec.DHExtendedPublicKeySpec;
...
public class BCDHPublicKey
  ...
  BCDHPublicKey(
    ...
    if (dhSpec instanceof DHDomainParameterSpec) {
      ...
      this.dhPublicKey = new DHPublicKeyParameters(y,
          new DHParameters(dhSpec.getP(), dhSpec.getG()));
    }
  }
  ...
  BCDHPublicKey(
    ...
    if (dhSpec instanceof DHDomainParameterSpec) {
      this.dhPublicKey = new DHPublicKeyParameters(y,
          ((DHDomainParameterSpec) dhSpec).getDomainParameters());
    } else {
      this.dhPublicKey = new DHPublicKeyParameters(y,
          new DHParameters(dhSpec.getP(), dhSpec.getG()));
    }
      ...
      return KeyUtil.getEncodedSubjectPublicKeyInfo(
          new AlgorithmIdentifier(X9ObjectIdentifiers.dhpublicnumber,
              new DomainParameters(params.getP(), params.getG(), params.getQ(), params.getJ(),
                  vParams).toASN1Primitive()), new ASN1Integer(y));
    }
    return KeyUtil.getEncodedSubjectPublicKeyInfo(
