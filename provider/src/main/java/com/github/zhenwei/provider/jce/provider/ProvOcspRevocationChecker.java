...
class ProvOcspRevocationChecker
  ...
  static {
    ...
    oids.put(RosstandartObjectIdentifiers.id_tc26_signwithdigest_gost_3410_12_256,
        "GOST3411-2012-256WITHECGOST3410-2012-256");
    oids.put(RosstandartObjectIdentifiers.id_tc26_signwithdigest_gost_3410_12_512,
  ...
  public void check(Certificate certificate)
    ...
    if (ocspResponses.get(cert) == null && ocspUri != null) {
      ...
      CertID id = createCertID(new AlgorithmIdentifier(OIWObjectIdentifiers.idSHA1), issuer,
          new ASN1Integer(cert.getSerialNumber()));

      OCSPResponse response = OcspCache.getOcspResponse(id, parameters, ocspUri,
          parent.getOcspResponderCert(), parent.getOcspExtensions(), helper);
    ...
    if (!ocspResponses.isEmpty()) {
      ...
      if (ocspResponse != null) {
          ...
          if (respBytes.getResponseType().equals(OCSPObjectIdentifiers.id_pkix_ocsp_basic)) {
            try {
              BasicOCSPResponse basicResp = BasicOCSPResponse.getInstance(
                  respBytes.getResponse().getOctets());

              if (preValidated || validatedOcspResponse(basicResp, parameters, nonce,
                  parent.getOcspResponderCert(), helper)) {
                ResponseData responseData = ResponseData.getInstance(
                    basicResp.getTbsResponseData());
                ...
                for (int i = 0; i != s.size(); i++) {
                  ...
                  if (serialNumber.equals(resp.getCertID().getSerialNumber())) {
                    ...
                    if (certID == null || !certID.getHashAlgorithm()
                        .equals(resp.getCertID().getHashAlgorithm())) {
                      com.github.zhenwei.core.asn1.x509.Certificate issuer = extractCert();
                    ...
                    if (certID.equals(resp.getCertID())) {
                      ...
                      if (resp.getCertStatus().getTagNo() == 1) {
                        RevokedInfo info = RevokedInfo.getInstance(
                            resp.getCertStatus().getStatus());
                        CRLReason reason = info.getRevocationReason();
                        throw new CertPathValidatorException(
                            "certificate revoked, reason=(" + reason + "), date="
                                + info.getRevocationTime().getDate(),
                            null, parameters.getCertPath(), parameters.getIndex());
                      }
                      throw new CertPathValidatorException(
              ...
              throw new CertPathValidatorException(
                  "unable to process OCSP response", e, parameters.getCertPath(),
                  parameters.getIndex());
            }
          }
        } else {
        ...
        throw new RecoverableCertPathValidatorException(
            "no OCSP response found for certificate", null, parameters.getCertPath(),
            parameters.getIndex());
      }
    } else {
      throw new RecoverableCertPathValidatorException(
          "no OCSP response found for any certificate", null, parameters.getCertPath(),
          parameters.getIndex());
    }
  }
  ...
  static URI getOcspResponderURI(X509Certificate cert) {
    byte[] extValue = cert.getExtensionValue(
        com.github.zhenwei.core.asn1.x509.Extension.authorityInfoAccess.getId());
    if (extValue == null) {
      return null;
    } else {
  ...
  static boolean validatedOcspResponse(BasicOCSPResponse basicResp,
      PKIXCertRevocationCheckerParameters parameters, byte[] nonce, X509Certificate responderCert,
      JcaJceHelper helper)
      throws CertPathValidatorException {
    try {
      ...
      X509Certificate sigCert = getSignerCert(basicResp, parameters.getSigningCert(), responderCert,
          helper);
      ...
      if (sigCert != null) {
        ...
        X509Certificate ocspCert = (X509Certificate) cf.generateCertificate(
        ...
        if (!responderMatches(basicResp.getTbsResponseData().getResponderID(), ocspCert, helper)) {
          throw new CertPathValidatorException("responder certificate does not match responderID",
              null,
        ...
        if (extendedKeyUsage == null || !extendedKeyUsage.contains(
            KeyPurposeId.id_kp_OCSPSigning.getId())) {
          throw new CertPathValidatorException(
              "responder certificate not valid for signing OCSP responses", null,
              parameters.getCertPath(), parameters.getIndex());
        }
      ...
      if (sig.verify(basicResp.getSignature().getBytes())) {
        if (nonce != null) {
          ...
          com.github.zhenwei.core.asn1.x509.Extension ext = exts.getExtension(
              OCSPObjectIdentifiers.id_pkix_ocsp_nonce);

          if (!Arrays.areEqual(nonce, ext.getExtnValue().getOctets())) {
            throw new CertPathValidatorException("nonce mismatch in OCSP response", null,
                parameters.getCertPath(), parameters.getIndex());
          }
        }
        return true;
      ...
      throw new CertPathValidatorException("OCSP response failure: " + e.getMessage(), e,
          parameters.getCertPath(), parameters.getIndex());
    } catch (IOException e) {
      throw new CertPathValidatorException("OCSP response failure: " + e.getMessage(), e,
          parameters.getCertPath(), parameters.getIndex());
  ...
  private static X509Certificate getSignerCert(BasicOCSPResponse basicResp,
      X509Certificate signingCert, X509Certificate responderCert, JcaJceHelper helper)
    ...
    if (keyHash != null) {
      ...
      if (sigCert != null) {
        if (name.equals(X500Name.getInstance(BCStrictStyle.INSTANCE,
            sigCert.getSubjectX500Principal().getEncoded()))) {
          return sigCert;
        }
      }
  ...
  private static boolean responderMatches(ResponderID responderID, X509Certificate certificate,
      JcaJceHelper helper)
      throws NoSuchProviderException, NoSuchAlgorithmException {
    byte[] keyHash = responderID.getKeyHash();
    if (keyHash != null) {
      ...
      return name.equals(X500Name.getInstance(BCStrictStyle.INSTANCE,
  ...
  private com.github.zhenwei.core.asn1.x509.Certificate extractCert()
      throws CertPathValidatorException {
    try {
      return com.github.zhenwei.core.asn1.x509.Certificate.getInstance(
          parameters.getSigningCert().getEncoded());
    } catch (Exception e) {
      throw new CertPathValidatorException("cannot process signing cert: " + e.getMessage(), e,
          parameters.getCertPath(), parameters.getIndex());
    }
  }

  private CertID createCertID(CertID base, com.github.zhenwei.core.asn1.x509.Certificate issuer,
      ASN1Integer serialNumber)
      throws CertPathValidatorException {
    return createCertID(base.getHashAlgorithm(), issuer, serialNumber);
  }

  private CertID createCertID(AlgorithmIdentifier digestAlg,
      com.github.zhenwei.core.asn1.x509.Certificate issuer, ASN1Integer serialNumber)
      throws CertPathValidatorException {
    try {
      MessageDigest digest = helper.createMessageDigest(
          MessageDigestUtils.getDigestName(digestAlg.getAlgorithm()));

      ASN1OctetString issuerNameHash = new DEROctetString(
          digest.digest(issuer.getSubject().getEncoded(ASN1Encoding.DER)));
