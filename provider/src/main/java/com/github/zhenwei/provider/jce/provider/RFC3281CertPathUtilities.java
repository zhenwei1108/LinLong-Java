...
import com.github.zhenwei.provider.x509.PKIXAttrCertChecker;
...
class RFC3281CertPathUtilities {
  ...
  protected static void checkCRLs(X509AttributeCertificate attrCert,
      ...
      Date currentDate, Date validityDate, X509Certificate issuerCert, List certPathCerts,
      JcaJceHelper helper)
    ...
    if (paramsPKIX.isRevocationEnabled()) {
      ...
      if (attrCert.getExtensionValue(NO_REV_AVAIL) == null) {
        ...
        try {
          crlStores.addAll(
              CertPathValidatorUtilities.getAdditionalStoresFromCRLDistributionPoint(crldp,
        ...
        if (certStatus.getCertStatus() == CertStatus.UNREVOKED
          ...
          try {
            ...
            checkCRL(dp, attrCert, paramsPKIXClone, currentDate, validityDate, issuerCert,
                certStatus,
  ...
  protected static void processAttrCert3(X509Certificate acIssuerCert,
    ...
    if (keyUsage != null && !((keyUsage.length > 0 && keyUsage[0]) || (keyUsage.length > 1
        && keyUsage[1]))) {
      throw new CertPathValidatorException(
  ...
  protected static CertPathValidatorResult processAttrCert2(
    ...
    try {
      validator = CertPathValidator.getInstance("PKIX", LinLongProvider.PROVIDER_NAME);
  ...
  protected static CertPath processAttrCert1(
    ...
    if (attrCert.getHolder().getIssuer() != null) {
      ...
      for (int i = 0; i < principals.length; i++) {
        try {
          ...
          CertPathValidatorUtilities.findCertificates(holderPKCs, certSelect,
    ...
    for (Iterator it = holderPKCs.iterator(); it.hasNext(); ) {
      ...
      try {
        builder = CertPathBuilder.getInstance("PKIX", LinLongProvider.PROVIDER_NAME);
      } catch (NoSuchProviderException e) {
        throw new ExtCertPathValidatorException(
            "Support class could not be created.", e);
      } catch (NoSuchAlgorithmException e) {
      ...
      try {
        result = builder.build(
            new PKIXExtendedBuilderParameters.Builder(paramsBldr.build()).build());
  ...
  private static void checkCRL(DistributionPoint dp, X509AttributeCertificate attrCert,
      PKIXExtendedParameters paramsPKIX, Date currentDate, Date validityDate,
      X509Certificate issuerCert, CertStatus certStatus,
    ...
    PKIXCertRevocationCheckerParameters params = new PKIXCertRevocationCheckerParameters(paramsPKIX,
        validityDate,
        null, -1, issuerCert, null);
    Set crls = CertPathValidatorUtilities.getCompleteCRLs(params, dp, attrCert, paramsPKIX,
        validityDate);
    boolean validCrlFound = false;
    AnnotatedException lastException = null;
    ...
    while (crl_iter.hasNext()
      ...
      try {
        ...
        // (f)
        Set keys = RFC3280CertPathUtilities.processCRLF(crl, attrCert, null, null, paramsPKIX,
