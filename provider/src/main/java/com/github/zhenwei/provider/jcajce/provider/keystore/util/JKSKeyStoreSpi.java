...
public class JKSKeyStoreSpi
  ...
  public String engineGetCertificateAlias(Certificate cert) {
    synchronized (certificateEntries) {
      for (Iterator<Map.Entry<String, BCJKSTrustedCertEntry>> it = certificateEntries.entrySet()
          .iterator(); it.hasNext(); ) {
          ...
          for (int t = 0; t < numEntries; t++) {
            ...
            switch (tag) {
              ...
              case 2: // certificate
                ...
                ErasableByteStream certStream = new ErasableByteStream(certData, 0,
                    certData.length);
                Certificate cert;
                try {
  ...
  private ErasableByteStream validateStream(InputStream inputStream, char[] password)
    ...
    if (password != null) {
      ...
      System.arraycopy(rawStore, rawStore.length - checksum.length, streamChecksum, 0,
          checksum.length);
    ...
    return new ErasableByteStream(rawStore, 0,
        rawStore.length - checksumCalculator.getDigestSize());
  }
