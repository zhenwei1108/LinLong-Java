...
public class BCDSTU4145PrivateKey
    implements ECPrivateKey, com.github.zhenwei.provider.jce.interfaces.ECPrivateKey,
  ...
  private void populateFromPrivKeyInfo(PrivateKeyInfo info)
    ...
    if (params.isNamedCurve()) {
      ...
      if (seq.getObjectAt(0) instanceof ASN1Integer) {
        ...
        if (dstuParams.isNamedCurve()) {
          ...
          spec = new ECNamedCurveParameterSpec(curveOid.getId(), ecP.getCurve(), ecP.getG(),
          ...
          ECCurve curve = new ECCurve.F2m(field.getM(), field.getK1(), field.getK2(), field.getK3(),
              binary.getA(), new BigInteger(1, b_bytes));
          byte[] g_bytes = binary.getG();
          if (info.getPrivateKeyAlgorithm().getAlgorithm().equals(UAObjectIdentifiers.dstu4145le)) {
            reverseBytes(g_bytes);
          }
          spec = new com.github.zhenwei.provider.jce.spec.ECParameterSpec(curve,
    ...
    if (privKey instanceof ASN1Integer) {
      ...
      com.github.zhenwei.core.asn1.sec.ECPrivateKey ec = com.github.zhenwei.core.asn1.sec.ECPrivateKey.getInstance(
          privKey);
  ...
  public byte[] getEncoded() {
    ...
    if (ecSpec instanceof ECNamedCurveSpec) {
      ASN1ObjectIdentifier curveOid = ECUtil.getNamedCurveOid(
      ...
      orderBitLength = ECUtil.getOrderBitLength(BouncyCastleProvider.CONFIGURATION,
          ecSpec.getOrder(), this.getS());
    } else if (ecSpec == null) {
      params = new X962Parameters(DERNull.INSTANCE);
      orderBitLength = ECUtil.getOrderBitLength(BouncyCastleProvider.CONFIGURATION, null,
          this.getS());
    } else {
    ...
    if (publicKey != null) {
      keyStructure = new com.github.zhenwei.core.asn1.sec.ECPrivateKey(orderBitLength, this.getS(),
          publicKey, params);
    } else {
      keyStructure = new com.github.zhenwei.core.asn1.sec.ECPrivateKey(orderBitLength, this.getS(),
          params);
  ...
  private ASN1BitString getPublicKeyDetails(BCDSTU4145PublicKey pub) {
    try {
      SubjectPublicKeyInfo info = SubjectPublicKeyInfo.getInstance(
          ASN1Primitive.fromByteArray(pub.getEncoded()));
