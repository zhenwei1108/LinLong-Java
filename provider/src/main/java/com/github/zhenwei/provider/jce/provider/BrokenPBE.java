...
import com.github.zhenwei.core.crypto.generators.PKCS5S1ParametersGenerator;
...
/**
 * Generator for PBE derived keys and ivs as defined by PKCS 12 V1.0, with a bug affecting 180 bit
 * plus keys - this class is only here to allow smooth migration of the version 0 keystore to
 * version 1. Don't use it (it won't be staying around).
 * <p>
...
class OldPKCS12ParametersGenerator
  ...
  /**
   * Construct a PKCS 12 Parameters generator. This constructor will accept MD5, SHA1, and
   * RIPEMD160.
   *
  ...
  /**
   * add a + b + 1, returning the result in a. The a value is treated as a BigInteger of length
   * (b.length * 8) bits. The result is modulo 2^b.length in case of overflow.
   */
  private void adjust(
      byte[] a,
      int aOff,
  ...
  /**
   * Generate a key parameter for use with a MAC derived from the password, salt, and iteration
