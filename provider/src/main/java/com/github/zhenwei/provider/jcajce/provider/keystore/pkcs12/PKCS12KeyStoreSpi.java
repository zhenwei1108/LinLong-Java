package com.github.zhenwei.provider.jcajce.provider.keystore.pkcs12;
...
import com.github.zhenwei.core.asn1.pkcs.EncryptedData;
...
import com.github.zhenwei.provider.jcajce.PKCS12StoreParameter;
...
public class PKCS12KeyStoreSpi
  ...
  static final String PKCS12_MAX_IT_COUNT_PROPERTY = "com.github.zhenwei.pkix.pkcs12.max_it_count";
  ...
  private AlgorithmIdentifier macAlgorithm = new AlgorithmIdentifier(OIWObjectIdentifiers.idSHA1,
      DERNull.INSTANCE);
  ...
  private Cipher createCipher(int mode, char[] password, AlgorithmIdentifier algId)
    ...
    if (func.isDefaultPrf()) {
      key = keyFact.generateSecret(
          new PBEKeySpec(password, func.getSalt(), validateIterationCount(func.getIterationCount()),
              keySizeProvider.getKeySize(encScheme)));
    } else {
      key = keyFact.generateSecret(new PBKDF2KeySpec(password, func.getSalt(),
          validateIterationCount(func.getIterationCount()), keySizeProvider.getKeySize(encScheme),
          func.getPrf()));
    }
    ...
    if (encParams instanceof ASN1OctetString) {
      ...
      cipher.init(mode, key,
          new GOST28147ParameterSpec(gParams.getEncryptionParamSet(), gParams.getIV()));
    }
    return cipher;
  }
  ...
  public void engineLoad(
    ...
    if (bag.getMacData() != null)           // check the mac code
    {
      ...
      try {
        byte[] res = calculatePbeMac(macAlgorithm.getAlgorithm(), salt, itCount, password, false,
            data);
        ...
        if (c[i].getContentType().equals(data)) {
          ...
          for (int j = 0; j != seq.size(); j++) {
            ...
            if (b.getBagId().equals(pkcs8ShroudedKeyBag)) {
              com.github.zhenwei.core.asn1.pkcs.EncryptedPrivateKeyInfo eIn = com.github.zhenwei.core.asn1.pkcs.EncryptedPrivateKeyInfo.getInstance(
                  b.getBagValue());
          ...
          for (int j = 0; j != seq.size(); j++) {
            ...
            if (b.getBagId().equals(certBag)) {
              ...
              com.github.zhenwei.core.asn1.pkcs.PrivateKeyInfo kInfo = com.github.zhenwei.core.asn1.pkcs.PrivateKeyInfo.getInstance(
                  b.getBagValue());
              PrivateKey privKey = LinLongProvider.getPrivateKey(kInfo);
      ...
      if (unmarkedKey) {
        if (keyCerts.isEmpty()) {
          String name = new String(
              Hex.encode(createSubjectKeyId(cert.getPublicKey()).getKeyIdentifier()));
  ...
  private void doStore(OutputStream stream, char[] password, boolean useDEREncoding)
    ...
    if (keys.size() == 0) {
      if (password == null) {
        ...
        if (useDEREncoding) {
          ContentInfo bagInfo = new ContentInfo(PKCSObjectIdentifiers.data,
              new DEROctetString(new DERSequence(certSeq).getEncoded()));

          Pfx pfx = new Pfx(new ContentInfo(PKCSObjectIdentifiers.data,
              new DEROctetString(new DERSequence(bagInfo).getEncoded())), null);

          pfx.encodeTo(stream, ASN1Encoding.DER);
        } else {
          ContentInfo bagInfo = new ContentInfo(PKCSObjectIdentifiers.data,
              new BEROctetString(new BERSequence(certSeq).getEncoded()));

          Pfx pfx = new Pfx(new ContentInfo(PKCSObjectIdentifiers.data,
              new BEROctetString(new BERSequence(bagInfo).getEncoded())), null);
    ...
    while (ks.hasMoreElements()) {
      ...
      com.github.zhenwei.core.asn1.pkcs.EncryptedPrivateKeyInfo kInfo = new com.github.zhenwei.core.asn1.pkcs.EncryptedPrivateKeyInfo(
          kAlgId, kBytes);
    ...
    try {
      byte[] res = calculatePbeMac(macAlgorithm.getAlgorithm(), mSalt, itCount, password, false,
          data);
  ...
  public static class BCPKCS12KeyStore
    ...
    public BCPKCS12KeyStore() {
      super(new BCJcaJceHelper(),
          new PKCS12KeyStoreSpi(new BCJcaJceHelper(), pbeWithSHAAnd3_KeyTripleDES_CBC,
  ...
  public static class DefPKCS12KeyStore
    ...
    public DefPKCS12KeyStore() {
      super(new DefaultJcaJceHelper(),
          new PKCS12KeyStoreSpi(new DefaultJcaJceHelper(), pbeWithSHAAnd3_KeyTripleDES_CBC,
              pbeWithSHAAnd40BitRC2_CBC));
    }
  }
