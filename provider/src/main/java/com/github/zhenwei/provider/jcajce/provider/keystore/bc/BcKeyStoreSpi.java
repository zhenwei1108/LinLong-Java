...
import com.github.zhenwei.core.crypto.io.DigestInputStream;
import com.github.zhenwei.core.crypto.io.DigestOutputStream;
import com.github.zhenwei.core.crypto.io.MacInputStream;
import com.github.zhenwei.core.crypto.io.MacOutputStream;
...
public class BcKeyStoreSpi
  ...
  private class StoreEntry {
    ...
    StoreEntry(
      ...
      Cipher cipher = makePBECipher(KEY_CIPHER, Cipher.ENCRYPT_MODE, password, salt,
          iterationCount);
    ...
    Object getObject(
      ...
      if (type == SEALED) {
        ...
        try {
          ...
          Cipher cipher = makePBECipher(KEY_CIPHER, Cipher.DECRYPT_MODE, password, salt,
              iterationCount);
          ...
          try {
            ...
            if (k != null) {
              ...
              Cipher out = makePBECipher(KEY_CIPHER, Cipher.ENCRYPT_MODE, password, salt,
                  iterationCount);
  ...
  private Key decodeKey(
    ...
    try {
      switch (keyType) {
        case KEY_PRIVATE:
          return LinLongProvider.getPrivateKey(PrivateKeyInfo.getInstance(enc));
        case KEY_PUBLIC:
          return LinLongProvider.getPublicKey(SubjectPublicKeyInfo.getInstance(enc));
        case KEY_SECRET:
  ...
  /**
   * the BouncyCastle store. This wont work with the key tool as the store is stored encrypted on
   * disk, so the password is mandatory, however if you hard drive is in a bad part of town and you
   * absolutely, positively, don't want nobody peeking at your things, this is the one to use, no
   * problem! After all in a Bouncy Castle nothing can touch you.
   * <p>
  ...
  public static class BouncyCastleStore
    ...
    public void engineLoad(
      ...
      Cipher cipher = this.makePBECipher(cipherAlg, Cipher.DECRYPT_MODE, password, salt,
          iterationCount);
      CipherInputStream cIn = new CipherInputStream(dIn, cipher);
    ...
    public void engineStore(OutputStream stream, char[] password)
      ...
      cipher = this.makePBECipher(STORE_CIPHER, Cipher.ENCRYPT_MODE, password, salt,
          iterationCount);
