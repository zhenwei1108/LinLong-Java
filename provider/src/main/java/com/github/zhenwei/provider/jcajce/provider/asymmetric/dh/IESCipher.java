...
import com.github.zhenwei.core.crypto.agreement.DHBasicAgreement;
...
import com.github.zhenwei.core.crypto.generators.DHKeyPairGenerator;
...
import com.github.zhenwei.core.crypto.params.DHKeyParameters;
...
import com.github.zhenwei.core.crypto.parsers.DHIESPublicKeyParser;
    ...
    if (state == Cipher.ENCRYPT_MODE || state == Cipher.WRAP_MODE) {
      ...
      EphemeralKeyPairGenerator kGen = new EphemeralKeyPairGenerator(gen, new KeyEncoder() {
        public byte[] getEncoded(AsymmetricKeyParameter keyParameter) {
          byte[] Vloc = new byte[
              (((DHKeyParameters) keyParameter).getParameters().getP().bitLength() + 7) / 8];
          byte[] Vtmp = BigIntegers.asUnsignedByteArray(
              ((DHPublicKeyParameters) keyParameter).getY());
