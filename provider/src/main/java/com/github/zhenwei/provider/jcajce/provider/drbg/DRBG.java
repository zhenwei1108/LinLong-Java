...
import com.github.zhenwei.core.crypto.prng.SP800SecureRandom;
import com.github.zhenwei.core.crypto.prng.SP800SecureRandomBuilder;
 ...
 * <p>org.bouncycastle.drbg.gather_pause_secs - is to stop the entropy collection thread from
 * grabbing all
 * available entropy on the system. The original motivation for the hybrid infrastructure was
 * virtual machines sometimes produce very few bits of entropy a second, the original approach
 * (which "worked" at least for BC) was to just read on the second thread and allow things to
 * progress around it, but it did tend to hog the system if other processes were using /dev/random.
 * By default the thread will pause for 5 seconds between 64 bit reads, increasing this time will
 * reduce the demands on the system entropy pool. Ideally the pause will be set to large enough to
 * allow everyone to work together, but small enough to ensure the provider's DRBG is being
 * regularly reseeded.
 * </p>
 * <p>org.bouncycastle.drbg.entropysource - is the class name for an implementation of
 * EntropySourceProvider.
 * For example, one could be provided which just reads directly from /dev/random and the extra
 * infrastructure used here could be avoided.</p>
 */
public class DRBG {
  ...
  private static class CoreSecureRandom
    ...
    CoreSecureRandom(Object[] initialEntropySourceAndSpi) {
      super((SecureRandomSpi) initialEntropySourceAndSpi[1],
          (Provider) initialEntropySourceAndSpi[0]);
    }
  }
      ...
      public EntropySourceProvider run() {
        try {
          ...
          throw new IllegalStateException(
              "entropy source " + sourceClass + " not created: " + e.getMessage(), e);
        }
      }
    });
  }
  ...
  private static SecureRandom createBaseRandom(boolean isPredictionResistant) {
    if (Properties.getPropertyValue("org.bouncycastle.drbg.entropysource") != null) {
      ...
      byte[] personalisationString =
          isPredictionResistant ? generateDefaultPersonalizationString(initSource.getEntropy())
      ...
      return new SP800SecureRandomBuilder(entropyProvider)
          ...
          .buildHash(new SHA512Digest(),
              Arrays.concatenate(initSource.getEntropy(), initSource.getEntropy()),
              isPredictionResistant);
    } else {
      SecureRandom randomSource = new HybridSecureRandom();   // needs to be done late, can't use static

      byte[] personalisationString = isPredictionResistant ? generateDefaultPersonalizationString(
          randomSource.generateSeed(16))
  ...
  private static byte[] generateDefaultPersonalizationString(byte[] seed) {
    return Arrays.concatenate(Strings.toByteArray("Default"), seed,
        Pack.longToBigEndian(Thread.currentThread().getId()),
    ...
    HybridSecureRandom() {
      ...
      drbg = new SP800SecureRandomBuilder(new EntropySourceProvider() {
        public EntropySource get(final int bitsRequired) {
          ...
          .buildHMAC(new HMac(new SHA512Digest()), baseRandom.generateSeed(32),
              false);     // 32 byte nonce
    }
