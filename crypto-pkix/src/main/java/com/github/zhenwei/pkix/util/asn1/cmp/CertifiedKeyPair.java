...
import com.github.zhenwei.pkix.util.asn1.crmf.PKIPublicationInfo;
  ...
  private CertifiedKeyPair(ASN1Sequence seq) {
    ...
    if (seq.size() >= 2) {
      if (seq.size() == 2) {
        ...
        privateKey = EncryptedKey.getInstance(
            ASN1TaggedObject.getInstance(seq.getObjectAt(1)).getObject());
        publicationInfo = PKIPublicationInfo.getInstance(
            ASN1TaggedObject.getInstance(seq.getObjectAt(2)).getObject());
      }
    }
  }
