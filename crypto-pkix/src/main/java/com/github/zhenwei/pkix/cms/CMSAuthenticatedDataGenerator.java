...
public class CMSAuthenticatedDataGenerator
  ...
  public CMSAuthenticatedData generate(CMSTypedData typedData, MacCalculator macCalculator,
    ...
    if (digestCalculator != null) {
      ...
      Map parameters = Collections.unmodifiableMap(
          getBaseParameters(typedData.getContentType(), digestCalculator.getAlgorithmIdentifier(),
              macCalculator.getAlgorithmIdentifier(), digestCalculator.getDigest()));
      ...
      ASN1Set unauthed = (unauthGen != null) ? new BERSet(
          unauthGen.getAttributes(parameters).toASN1EncodableVector()) : null;

      ContentInfo eci = new ContentInfo(
          typedData.getContentType(),
          encContent);

      authData = new AuthenticatedData(originatorInfo, new DERSet(recipientInfos),
          macCalculator.getAlgorithmIdentifier(), digestCalculator.getAlgorithmIdentifier(), eci,
          authed, macResult, unauthed);
    } else {
      try {
        ByteArrayOutputStream bOut = new ByteArrayOutputStream();
      ...
      ASN1Set unauthed = (unauthGen != null) ? new BERSet(
          unauthGen.getAttributes(Collections.EMPTY_MAP).toASN1EncodableVector()) : null;
      ...
      authData = new AuthenticatedData(originatorInfo, new DERSet(recipientInfos),
          macCalculator.getAlgorithmIdentifier(), null, eci, null, macResult, unauthed);
    }
