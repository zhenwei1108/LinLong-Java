...
import com.github.zhenwei.pkix.tsp.TimeStampTokenInfo;
...
import com.github.zhenwei.pkix.util.asn1.cms.TimeStampedDataParser;
...
class TimeStampDataUtil {
  ...
  void validate(DigestCalculatorProvider calculatorProvider, byte[] dataDigest,
    ...
    throw new ImprintDigestInvalidException(
        "passed in token not associated with timestamps present", timeStampToken);
  }
  ...
  private void compareDigest(TimeStampToken timeStampToken, byte[] digest)
    ...
    if (!Arrays.areEqual(digest, tsrMessageDigest)) {
      throw new ImprintDigestInvalidException(
          "hash calculated is different from MessageImprintDigest found in TimeStampToken",
          timeStampToken);
    }
  }
