...
/**
 * Builder for the content encryptor in EnvelopedData - used to encrypt the actual transmitted
 * content.
 */
public class JceCMSContentEncryptorBuilder {
  ...
  /**
   * Constructor for a content encryptor builder based on an algorithm identifier and its contained
   * parameters.
   *
  ...
  public JceCMSContentEncryptorBuilder(AlgorithmIdentifier encryptionAlgId) {
    this(encryptionAlgId.getAlgorithm(),
        KEY_SIZE_PROVIDER.getKeySize(encryptionAlgId.getAlgorithm()));
   ...
   *
   * @param providerName the name of the provider to use for cipher and default parameters
   *                     creation.
   * @return the current builder instance.
   */
   ...
   * @return the current builder instance.
   */
  public JceCMSContentEncryptorBuilder setAlgorithmParameters(
  ...
  public OutputEncryptor build()
    ...
    if (algorithmIdentifier != null) {
      ...
      if (params != null && !params.equals(DERNull.INSTANCE)) {
        try {
          algorithmParameters = helper.createAlgorithmParameters(
              algorithmIdentifier.getAlgorithm());

          algorithmParameters.init(params.toASN1Primitive().getEncoded());
        } catch (Exception e) {
          throw new CMSException("unable to process provided algorithmIdentifier: " + e.toString(),
              e);
        }
      }
    }
  ...
  private class CMSOutputEncryptor
    ...
    CMSOutputEncryptor(ASN1ObjectIdentifier encryptionOID, int keySize, AlgorithmParameters params,
        SecureRandom random)
        throws CMSException {
  ...
  private class CMSAuthOutputEncryptor
    ...
    CMSAuthOutputEncryptor(ASN1ObjectIdentifier encryptionOID, int keySize,
