...
public class OERDefinition {
  ...
  private static final BigInteger[][] sIntRange = new BigInteger[][]{
      ...
      new BigInteger[]{new BigInteger("-9223372036854775808"),
  ...
  public static class Element {
    ...
    public boolean isUnsignedWithRange() {
      return isLowerRangeZero() && (upperBound != null
          && BigInteger.ZERO.compareTo(upperBound) < 0);
    }
    ...
    /**
     * Determine the number of integer bytes for a range, ints, signed or unsigned that can fit into
     * 1 to 8 octets use a fixed with encoding. Returns a negative number if the value is signed and
     * the absolute value is the number of bytes.
     */
      ...
      if (lowerBound != null && upperBound != null) {
        if (BigInteger.ZERO.equals(lowerBound)) {
          ...
          for (int i = 0, j = 1; i < sIntRange.length; i++, j *= 2) {
            if (lowerBound.compareTo(sIntRange[i][0]) >= 0
                && upperBound.compareTo(sIntRange[i][1]) < 0) {
              return -j;
            }
          }
        }
  ...
  public static Builder octets(int lowerBound, int upperBound) {
    return new Builder(BaseType.OCTET_STRING).range(BigInteger.valueOf(lowerBound),
  ...
  public static Builder utf8String(int lowerBound, int upperBound) {
    return new Builder(BaseType.UTF8_STRING).range(BigInteger.valueOf(lowerBound),
        BigInteger.valueOf(upperBound));
  }
