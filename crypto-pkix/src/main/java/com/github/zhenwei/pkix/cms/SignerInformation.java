...
import com.github.zhenwei.pkix.operator.RawContentVerifier;
...
public class SignerInformation {
  ...
  /**
   * Protected constructor. In some cases clients have their own idea about how to encode the signed
   * attributes and calculate the signature. This constructor is to allow developers to deal with
   * that by extending off the class and overriding methods like getSignedAttributes().
   *
  ...
  /**
   * Protected constructor. In some cases clients also have their own ideas about what goes in
   * various SignerInfo fields. This constructor is to allow developers to deal with that by also
   * tweaking the SignerInfo so that these issues can be dealt with.
   *
   * @param baseInfo the SignerInformation to base this one on.
        ...
        if (contentVerifier instanceof RawContentVerifier) {
          ...
          if (encName.equals("RSA")) {
            DigestInfo digInfo = new DigestInfo(
                new AlgorithmIdentifier(digestAlgorithm.getAlgorithm(), DERNull.INSTANCE),
                resultDigest);
  ...
  private void verifyAlgorithmIdentifierProtectionAttribute(AttributeTable signedAttrTable)
    ...
    if (signedAttrTable != null) {
      ASN1EncodableVector protectionAttributes = signedAttrTable.getAll(
      ...
      if (protectionAttributes.size() > 0) {
        ...
        CMSAlgorithmProtection algorithmProtection = CMSAlgorithmProtection.getInstance(
            attr.getAttributeValues()[0]);

        if (!CMSUtils.isEquivalent(algorithmProtection.getDigestAlgorithm(),
            info.getDigestAlgorithm())) {
          throw new CMSException(
  ...
  /**
   * Verify that the given verifier can successfully verify the signature on this SignerInformation
   * object.
   *
   ...
   * @throws com.github.zhenwei.pkix.cms.CMSVerifierCertificateNotValidException if the provider has
   *                                                                             an associated
   *                                                                             certificate and the
   *                                                                             certificate is not
   *                                                                             valid at the time
   *                                                                             given as the SignerInfo's
   *                                                                             signing time.
   * @throws com.github.zhenwei.pkix.cms.CMSException                            if the verifier is
   *                                                                             unable to create a
   *                                                                             ContentVerifiers or
   *                                                                             DigestCalculators.
   */
