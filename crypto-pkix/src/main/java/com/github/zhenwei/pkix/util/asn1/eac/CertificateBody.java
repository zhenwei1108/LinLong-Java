...
public class CertificateBody
  ...
  private void setIso7816CertificateBody(ASN1ApplicationSpecific appSpe)
    ...
    while ((obj = aIS.readObject()) != null) {
      ...
      if (obj instanceof ASN1ApplicationSpecific) {
        ...
        throw new IOException("Not a valid iso7816 content : not a ASN1ApplicationSpecific Object :"
      ...
      switch (aSpe.getApplicationTag()) {
        ...
        default:
          ...
          throw new IOException(
              "Not a valid iso7816 ASN1ApplicationSpecific tag " + aSpe.getApplicationTag());
      }
    }
    aIS.close();
  }
  ...
  private void setCertificateProfileIdentifier(ASN1ApplicationSpecific certificateProfileIdentifier)
    ...
    if (certificateProfileIdentifier.getApplicationTag() == EACTags.INTERCHANGE_PROFILE) {
      ...
      throw new IllegalArgumentException(
          "Not an Iso7816Tags.INTERCHANGE_PROFILE tag :" + EACTags.encodeTag(
   ...
   *
   * @param certificationAuthorityReference the ASN1ApplicationSpecific containing the
  ...
  private void setCertificationAuthorityReference(
    ...
    if (certificationAuthorityReference.getApplicationTag()
        == EACTags.ISSUER_IDENTIFICATION_NUMBER) {
  ...
  /**
   * gives the type of the certificate (value should be profileType or requestType if all data are
   * set).
   *
  ...
  /**
   * the Iso7816CertificateHolderAuthorization encodes the role of the holder (i.e. CVCA, DV, IS)
   * and assigns read/write access rights to data groups storing sensitive data. This functions
  ...
  /**
   * certificateHolderReference : associates the public key contained in the certificate with a
   * unique name
   *
  ...
  /**
   * get the certificationAuthorityReference certificationAuthorityReference : uniquely identifies
   * the issuinng CA's signature key pair
   *
