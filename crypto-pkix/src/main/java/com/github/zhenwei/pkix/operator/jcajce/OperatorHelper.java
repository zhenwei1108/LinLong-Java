...
import com.github.zhenwei.pkix.operator.DefaultSignatureNameFinder;
import com.github.zhenwei.pkix.operator.OperatorCreationException;
import com.github.zhenwei.provider.jcajce.util.AlgorithmParametersUtils;
...
class OperatorHelper {
  ...
  static {
    ...
    symmetricWrapperKeySizes.put(KISAObjectIdentifiers.id_npki_app_cmsSeed_wrap,
  ...
  AlgorithmParameters createAlgorithmParameters(AlgorithmIdentifier cipherAlgId)
    ...
    try {
      ...
      throw new OperatorCreationException("cannot create algorithm parameters: " + e.getMessage(),
          e);
    }
  ...
  MessageDigest createDigest(AlgorithmIdentifier digAlgId)
    ...
    try {
      if (digAlgId.getAlgorithm().equals(NISTObjectIdentifiers.id_shake256_len)) {
        dig = helper.createMessageDigest(
        ...
        dig = helper.createMessageDigest(
            "SHAKE128-" + ASN1Integer.getInstance(digAlgId.getParameters()).getValue());
      } else {
  ...
  public X509Certificate convertCertificate(X509CertificateHolder certHolder)
    ...
    try {
      ...
      return (X509Certificate) certFact.generateCertificate(
          new ByteArrayInputStream(certHolder.getEncoded()));
    } catch (IOException e) {
      throw new OpCertificateException("cannot get encoded form of certificate: " + e.getMessage(),
          e);
    } catch (NoSuchProviderException e) {
  ...
  public PublicKey convertPublicKey(SubjectPublicKeyInfo publicKeyInfo)
      throws OperatorCreationException {
    try {
      KeyFactory keyFact = helper.createKeyFactory(
  ...
  private boolean notDefaultPSSParams(ASN1Sequence seq)
    ...
    if (!pssParams.getHashAlgorithm()
        .equals(AlgorithmIdentifier.getInstance(pssParams.getMaskGenAlgorithm().getParameters()))) {
      return true;
    }
