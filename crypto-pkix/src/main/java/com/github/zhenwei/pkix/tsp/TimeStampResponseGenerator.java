 ...
 * <p>
 * New generate methods have been introduced to give people more control over what ends up in the
 * message. Unfortunately it turns out that in some cases fields like statusString must be left out
 * otherwise a an otherwise valid timestamp will be rejected.
 * </p>
 ...
 * <p>
 * It should be pointed out that generateRejectedResponse() may also, on very rare occasions throw a
 * TSPException. In the event that happens, there's a serious internal problem with your responder.
 * </p>
 */
public class TimeStampResponseGenerator {
   ...
   * <p>
   * If genTime is null a timeNotAvailable error response will be returned. Calling generate() is
   * the equivalent of:
   * <pre>
   *    TimeStampResponse tsResp;
   *
  ...
  public TimeStampResponse generateGrantedResponse(
    ...
    try {
      tstTokenContentInfo = tokenGenerator.generate(request, serialNumber, genTime,
          additionalExtensions).toCMSSignedData().toASN1Structure();
    ...
    try {
      return new TimeStampResponse(new DLSequence(
          new ASN1Encodable[]{pkiStatusInfo.toASN1Primitive(),
              tstTokenContentInfo.toASN1Primitive()}));
    } catch (IOException e) {
   ...
   * Exceptions which are not an instance of TSPValidationException will be treated as
   * systemFailure. The return value of exception.getMessage() will be used as the status string for
   * the response.
   *
  ...
  public TimeStampResponse generateRejectedResponse(Exception exception)
    ...
    if (exception instanceof TSPValidationException) {
      return generateFailResponse(PKIStatus.REJECTION,
          ((TSPValidationException) exception).getFailureCode(), exception.getMessage());
    } else {
      return generateFailResponse(PKIStatus.REJECTION, PKIFailureInfo.systemFailure,
