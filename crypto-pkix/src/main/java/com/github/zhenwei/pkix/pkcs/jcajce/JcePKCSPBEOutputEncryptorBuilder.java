...
import com.github.zhenwei.pkix.operator.AlgorithmNameFinder;
import com.github.zhenwei.pkix.operator.DefaultAlgorithmNameFinder;
...
public class JcePKCSPBEOutputEncryptorBuilder {
   ...
   */
  public JcePKCSPBEOutputEncryptorBuilder setKeySizeProvider(
    ...
    try {
      if (isPKCS12(algorithm)) {
        ...
        cipher.init(Cipher.ENCRYPT_MODE,
            new PKCS12KeyWithParameters(password, salt, iterationCount));

        encryptionAlg = new AlgorithmIdentifier(algorithm,
            new PKCS12PBEParams(salt, iterationCount));
        ...
        if (MiscObjectIdentifiers.id_scrypt.equals(pbkDef.getAlgorithm())) {
          ...
          key = keyFact.generateSecret(new ScryptKeySpec(password,
              salt, skdf.getCostParameter(), skdf.getBlockSize(),
              skdf.getParallelizationParameter(),
          ...
          SecretKeyFactory keyFact = helper.createSecretKeyFactory(
              JceUtils.getAlgorithm(pkdf.getPRF().getAlgorithm()));
          ...
          if (algP != null) {
            algParams = new PBES2Parameters(
                new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2,
                    new PBKDF2Params(salt, pkdf.getIterationCount(), pkdf.getPRF())),
                new EncryptionScheme(keyEncAlgorithm,
                    ASN1Primitive.fromByteArray(cipher.getParameters().getEncoded())));
          } else {
            algParams = new PBES2Parameters(
  ...
  /**
   * converts a password to a byte array according to the scheme in PKCS12 (unicode, big endian, 2
   * zero pad bytes at the end).
   *
   * @param password a character array representing the password.
   * @return a byte array representing the password.
   */
