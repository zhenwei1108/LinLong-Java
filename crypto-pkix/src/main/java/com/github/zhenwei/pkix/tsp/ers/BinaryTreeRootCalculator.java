...
/**
 * Calculator based on the use of a left weighted binary Merkle tree created on top of the partial
 * hash tree objects provided.
 */
  ...
  public byte[] computeRootHash(DigestCalculator digCalc, PartialHashtree[] nodes) {
    ...
    do {
      ...
      for (int i = 0; i <= hashes.size() - 2; i += 2) {
        newHashes.add(ERSUtil.calculateBranchHash(digCalc, (byte[]) hashes.get(i),
            (byte[]) hashes.get(i + 1)));
      }
