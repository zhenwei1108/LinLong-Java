...
public class DANEEntrySelectorFactory {
   ...
   * or some equivalent.
   *
   * @param digestCalculator a calculator for the message digest to filter email addresses currently
   *                         truncated SHA-256 (originally SHA-224).
   */
  ...
  public DANEEntrySelector createSelector(String emailAddress)
    ...
    final byte[] enc = Strings.toUTF8ByteArray(
    ...
    final String domainName =
        Strings.fromByteArray(Hex.encode(hash)) + "._smimecert." + emailAddress.substring(
