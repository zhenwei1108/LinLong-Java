...
class RFC3280CertPathUtilities {
    ...
    if (idp != null) {
      if (idp.getDistributionPoint() != null) {
        ...
        if (dpName.getType() == DistributionPointName.NAME_RELATIVE_TO_CRL_ISSUER) {
          ...
          try {
            Enumeration e = ASN1Sequence.getInstance(crl.getIssuerX500Principal().getEncoded())
                .getObjects();
      ...
      try {
        bc = BasicConstraints.getInstance(
            RevocationUtilities.getExtensionValue((X509Extension) cert,
  ...
  protected static void processCRLB1(
      ...
      X509CRL crl)
      throws AnnotatedException {
    ASN1Primitive idp = RevocationUtilities.getExtensionValue(crl,
        Extension.issuingDistributionPoint);
  ...
  protected static Set processCRLA1i(
    ...
    if (paramsPKIX.isUseDeltasEnabled()) {
      ...
      try {
        ...
        throw new AnnotatedException(
            "Freshest CRL extension could not be decoded from certificate.", e);
      }
      if (freshestCRL == null) {
        try {
      ...
      if (freshestCRL != null) {
        ...
        try {
          crlStores.addAll(
              RevocationUtilities.getAdditionalStoresFromCRLDistributionPoint(freshestCRL,
        ...
        // get delta CRL(s)
        try {
          set.addAll(RevocationUtilities.getDeltaCRLs(currentDate, crl, paramsPKIX.getCertStores(),
              crlStores));
        } catch (AnnotatedException e) {
  ...
  protected static Set[] processCRLA1ii(
    ...
    PKIXCRLStoreSelector extSelect = new PKIXCRLStoreSelector.Builder(
        crlselect).setCompleteCRLEnabled(true).build();
    ...
    if (paramsPKIX.isUseDeltasEnabled()) {
      ...
      try {
        deltaSet.addAll(
            RevocationUtilities.getDeltaCRLs(validityDate, crl, paramsPKIX.getCertStores(),
                paramsPKIX.getCRLStores()));
      } catch (AnnotatedException e) {
        throw new AnnotatedException("Exception obtaining delta CRLs.", e);
      }
  ...
  static void checkCRL(
    ...
    Set crls = RevocationUtilities.getCompleteCRLs(dp, cert, validityDate,
    ...
    while (crl_iter.hasNext() && certStatus.getCertStatus() == CertStatus.UNREVOKED
      ...
      try {
        ...
        if (paramsPKIX.isUseDeltasEnabled()) {
          // get delta CRLs
          Set deltaCRLs = RevocationUtilities.getDeltaCRLs(validityDate, crl,
              paramsPKIX.getCertStores(), paramsPKIX.getCRLStores());
