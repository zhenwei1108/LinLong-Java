...
import com.github.zhenwei.pkix.util.asn1.cmp.CMPCertificate;
...
public class ProtectedPKIMessage {
  ...
  /**
   * Determine whether the message is protected by a password based MAC. Use verify(PKMACBuilder,
   * char[]) to verify the message if this method returns true.
   *
   * @return true if protection MAC PBE based, false otherwise.
   */
  public boolean hasPasswordBasedMacProtection() {
    return pkiMessage.getHeader().getProtectionAlg().getAlgorithm()
        .equals(CMPObjectIdentifiers.passwordBasedMac);
  }
   ...
   * @return true if the provider is able to create a verifier that validates the signature, false
   * otherwise.
  ...
  public boolean verify(PKMACBuilder pkMacBuilder, char[] password)
      throws CMPException {
    if (!CMPObjectIdentifiers.passwordBasedMac.equals(
        pkiMessage.getHeader().getProtectionAlg().getAlgorithm())) {
      throw new CMPException("protection algorithm not mac based");
    }

    try {
      pkMacBuilder.setParameters(
