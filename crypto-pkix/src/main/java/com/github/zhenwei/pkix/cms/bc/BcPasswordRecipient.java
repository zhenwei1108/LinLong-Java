...
public abstract class BcPasswordRecipient
  ...
  protected KeyParameter extractSecretKey(AlgorithmIdentifier keyEncryptionAlgorithm,
    ...
    keyEncryptionCipher.init(false, new ParametersWithIV(new KeyParameter(derivedKey),
        ASN1OctetString.getInstance(keyEncryptionAlgorithm.getParameters()).getOctets()));

    try {
      return new KeyParameter(keyEncryptionCipher.unwrap(encryptedContentEncryptionKey, 0,
