...
//    }
  ...
  public ASN1Object parse(OERDefinition.Element element)
    ...
    switch (element.baseType) {
      ...
      case SEQ: {
        Sequence sequence = sequence(countOptionalChildTypes(element), element.hasDefaultChildren(),
            element.extensionsInDefinition);
...
//                    debugPrint(child.appendLabel("E[" + t + "]"));
      ...
      case ENUM: {
        ...
        debugPrint(
            element.appendLabel("ENUM(" + bi + ") = " + element.children.get(bi.intValue()).label));
      ...
      case OCTET_STRING: {
        ...
        if (debugOutput != null) {
          debugPrint(element.appendLabel(
              "OCTET STRING (" + data.length + ") = " + Hex.toHexString(data, 0,
                  Math.min(data.length, 32))));
        }
  ...
  private byte[] allocateArray(int requiredSize) {
    if (requiredSize > maxByteAllocation) {
      throw new IllegalArgumentException(
          "required byte array size " + requiredSize + " was greater than " + maxByteAllocation);
    }
