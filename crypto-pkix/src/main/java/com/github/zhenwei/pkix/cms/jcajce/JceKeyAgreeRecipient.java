...
import com.github.zhenwei.core.asn1.cryptopro.Gost2814789EncryptedKey;
import com.github.zhenwei.core.asn1.cryptopro.Gost2814789KeyWrapParameters;
...
import com.github.zhenwei.pkix.cms.KeyAgreeRecipient;
...
import com.github.zhenwei.pkix.util.asn1.cms.ecc.MQVuserKeyingMaterial;
import com.github.zhenwei.provider.jcajce.spec.GOST28147WrapParameterSpec;
import com.github.zhenwei.provider.jcajce.spec.MQVParameterSpec;
...
public abstract class JceKeyAgreeRecipient
  ...
  /**
   * Set the provider to use for content processing.  If providerName is null a "no provider" search
   * will be used to satisfy getInstance calls.
   *
  ...
  /**
   * Set the algorithm identifier for the private key. You'll want to use this if you are dealing
   * with a HSM and it is not possible to get the encoding of the private key.
   *
  ...
  private SecretKey calculateAgreedWrapKey(AlgorithmIdentifier keyEncAlg,
    ...
    if (CMSUtils.isMQV(keyEncAlg.getAlgorithm())) {
      ...
      if (kmGen == old_ecc_cms_Generator) {
        ukmKeyingMaterial = old_ecc_cms_Generator.generateKDFMaterial(wrapAlg,
            keySizeProvider.getKeySize(wrapAlg), ukmKeyingMaterial);
      }

      agreement.init(receiverPrivateKey,
          new MQVParameterSpec(receiverPrivateKey, ephemeralKey, ukmKeyingMaterial));
      ...
      if (CMSUtils.isEC(keyEncAlg.getAlgorithm())) {
        if (userKeyingMaterial != null) {
          byte[] ukmKeyingMaterial = kmGen.generateKDFMaterial(wrapAlg,
              keySizeProvider.getKeySize(wrapAlg), userKeyingMaterial.getOctets());

          userKeyingMaterialSpec = new UserKeyingMaterialSpec(ukmKeyingMaterial);
        } else {
          byte[] ukmKeyingMaterial = kmGen.generateKDFMaterial(wrapAlg,
              keySizeProvider.getKeySize(wrapAlg), null);
  ...
  protected Key unwrapSessionKey(ASN1ObjectIdentifier wrapAlg, SecretKey agreedKey,
      ASN1ObjectIdentifier contentEncryptionAlgorithm, byte[] encryptedContentEncryptionKey)
    ...
    return keyCipher.unwrap(encryptedContentEncryptionKey,
        helper.getBaseCipherName(contentEncryptionAlgorithm), Cipher.SECRET_KEY);
  }

  protected Key extractSecretKey(AlgorithmIdentifier keyEncryptionAlgorithm,
      AlgorithmIdentifier contentEncryptionAlgorithm, SubjectPublicKeyInfo senderKey,
      ASN1OctetString userKeyingMaterial, byte[] encryptedContentEncryptionKey)
      throws CMSException {
    try {
      AlgorithmIdentifier wrapAlg =
      ...
      try {
        ...
        if (wrapAlg.getAlgorithm().equals(CryptoProObjectIdentifiers.id_Gost28147_89_None_KeyWrap)
            || wrapAlg.getAlgorithm()
            .equals(CryptoProObjectIdentifiers.id_Gost28147_89_CryptoPro_KeyWrap)) {
          Gost2814789EncryptedKey encKey = Gost2814789EncryptedKey.getInstance(
              encryptedContentEncryptionKey);
          Gost2814789KeyWrapParameters wrapParams = Gost2814789KeyWrapParameters.getInstance(
              wrapAlg.getParameters());

          Cipher keyCipher = helper.createCipher(wrapAlg.getAlgorithm());

          keyCipher.init(Cipher.UNWRAP_MODE, agreedWrapKey,
              new GOST28147WrapParameterSpec(wrapParams.getEncryptionParamSet(),
                  userKeyingMaterial.getOctets()));

          return keyCipher.unwrap(Arrays.concatenate(encKey.getEncryptedKey(), encKey.getMacKey()),
              helper.getBaseCipherName(contentEncryptionAlgorithm.getAlgorithm()),
              Cipher.SECRET_KEY);
        }

        return unwrapSessionKey(wrapAlg.getAlgorithm(), agreedWrapKey,
            contentEncryptionAlgorithm.getAlgorithm(), encryptedContentEncryptionKey);
  ...
  private static KeyMaterialGenerator old_ecc_cms_Generator = new KeyMaterialGenerator() {
    public byte[] generateKDFMaterial(AlgorithmIdentifier keyAlgorithm, int keySize,
      ...
      eccInfo = new ECCCMSSharedInfo(
          new AlgorithmIdentifier(keyAlgorithm.getAlgorithm(), DERNull.INSTANCE),
          userKeyMaterialParameters, Pack.intToBigEndian(keySize));
