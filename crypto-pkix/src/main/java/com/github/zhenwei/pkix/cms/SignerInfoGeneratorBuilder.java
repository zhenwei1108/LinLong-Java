...
public class SignerInfoGeneratorBuilder {
   ...
   * Base constructor.
   *
   * @param digestProvider a provider of digest calculators for the algorithms required in the
   *                       signature and attribute calculations.
   */
   ...
   * @param digestProvider  a provider of digest calculators for the algorithms required in the
   *                        signature and attribute calculations.
   * @param sigEncAlgFinder finder for algorithm IDs to store for the signature encryption/signature
   *                        algorithm field.
   */
  public SignerInfoGeneratorBuilder(DigestCalculatorProvider digestProvider,
  ...
  /**
   * Build a generator with the passed in subjectKeyIdentifier as the signerIdentifier. If used  you
   * should try to follow the calculation described in RFC 5280 section 4.2.1.2.
   *
   * @param contentSigner        operator for generating the final signature in the SignerInfo
   *                             with.
   * @param subjectKeyIdentifier key identifier to identify the public key for verifying the
   *                             signature.
   * @return a SignerInfoGenerator
   * @throws OperatorCreationException if the generator cannot be built.
   */
  ...
  private SignerInfoGenerator createGenerator(ContentSigner contentSigner, SignerIdentifier sigId)
    ...
    if (directSignature) {
      return new SignerInfoGenerator(sigId, contentSigner, digester.getAlgorithmIdentifier(),
          sigEncAlgFinder);
    }
    ...
    if (signedGen != null || unsignedGen != null) {
      ...
      return new SignerInfoGenerator(sigId, contentSigner, digester, sigEncAlgFinder, signedGen,
          unsignedGen);
    }

    return new SignerInfoGenerator(sigId, contentSigner, digester, sigEncAlgFinder,
