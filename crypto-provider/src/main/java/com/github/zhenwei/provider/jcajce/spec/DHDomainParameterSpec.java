...
/**
 * Extension class for DHParameterSpec that wraps a DHDomainParameters object and provides the q
 * domain parameter.
 */
public class DHDomainParameterSpec
  ...
  public DHDomainParameterSpec(DHParameters domainParameters) {
    this(domainParameters.getP(), domainParameters.getQ(), domainParameters.getG(),
        domainParameters.getJ(), domainParameters.getM(), domainParameters.getL());
  ...
  /**
   * Minimal constructor for parameters able to be used to verify a public key, or use with MQV, and
   * a private value length.
   *
   * @param p the prime p defining the Galois field.
   * @param g the generator of the multiplicative subgroup of order g.
   * @param q specifies the prime factor of p - 1
   ...
   * @param l the maximum bit length for the private value.
   */
  public DHDomainParameterSpec(BigInteger p, BigInteger q, BigInteger g, BigInteger j, int m,
      int l) {
    super(p, g, l);
    this.q = q;
    this.j = j;
  ...
  /**
   * Return the minimum bitlength for a private value to be generated from these parameters, 0 if
   * not set.
   *
