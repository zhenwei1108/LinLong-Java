...
import com.github.zhenwei.core.asn1.bc.EncryptedObjectStoreData;
import com.github.zhenwei.core.asn1.bc.EncryptedPrivateKeyData;
import com.github.zhenwei.core.asn1.bc.EncryptedSecretKeyData;
import com.github.zhenwei.core.asn1.bc.ObjectData;
import com.github.zhenwei.core.asn1.bc.ObjectDataSequence;
import com.github.zhenwei.core.asn1.bc.ObjectStore;
import com.github.zhenwei.core.asn1.bc.ObjectStoreData;
import com.github.zhenwei.core.asn1.bc.ObjectStoreIntegrityCheck;
import com.github.zhenwei.core.asn1.bc.PbkdMacIntegrityCheck;
import com.github.zhenwei.core.asn1.bc.SecretKeyData;
import com.github.zhenwei.core.asn1.bc.SignatureCheck;
...
import com.github.zhenwei.core.asn1.misc.ScryptParams;
...
import com.github.zhenwei.core.crypto.util.PBKDF2Config;
import com.github.zhenwei.core.crypto.util.PBKDFConfig;
import com.github.zhenwei.core.crypto.util.ScryptConfig;
import com.github.zhenwei.core.internal.asn1.cms.CCMParameters;
import com.github.zhenwei.core.util.Arrays;
import com.github.zhenwei.core.util.Strings;
import com.github.zhenwei.provider.jcajce.BCFKSLoadStoreParameter;
import com.github.zhenwei.provider.jcajce.BCFKSStoreParameter;
...
class BcFKSKeyStoreSpi
  ...
  public Key engineGetKey(String alias, char[] password)
    ...
    if (ent != null) {
      if (ent.getType().equals(PRIVATE_KEY) || ent.getType().equals(PROTECTED_PRIVATE_KEY)) {
        ...
        EncryptedPrivateKeyInfo encInfo = EncryptedPrivateKeyInfo.getInstance(
            encPrivData.getEncryptedPrivateKeyInfo());

        try {
          PrivateKeyInfo pInfo = PrivateKeyInfo.getInstance(
              decryptData("PRIVATE_KEY_ENCRYPTION", encInfo.getEncryptionAlgorithm(), password,
                  encInfo.getEncryptedData()));

          KeyFactory kFact = helper.createKeyFactory(
              getPublicKeyAlg(pInfo.getPrivateKeyAlgorithm().getAlgorithm()));

          PrivateKey privateKey = kFact.generatePrivate(
        ...
        try {
          SecretKeyData keyData = SecretKeyData.getInstance(
              decryptData("SECRET_KEY_ENCRYPTION", encKeyData.getKeyEncryptionAlgorithm(), password,
    ...
    if (helper != null) {
      try {
        ...
        return certFact.generateCertificate(new ByteArrayInputStream(
            com.github.zhenwei.core.asn1.x509.Certificate.getInstance(cert).getEncoded()));
      } catch (Exception e) {
        return null;
      }
    } else {
    ...
    if (key instanceof PrivateKey) {
      if (chain == null) {
        throw new KeyStoreException(
            "BCFKS KeyStore requires a certificate chain for private key storage.");
      }
      ...
      try {
        ...
        KeyDerivationFunc pbkdAlgId = generatePkbdAlgorithmIdentifier(
            PKCSObjectIdentifiers.id_PBKDF2, 256 / 8);
        byte[] keyBytes = generateKey(pbkdAlgId, "PRIVATE_KEY_ENCRYPTION",
            ((password != null) ? password : new char[0]), 32);
        ...
        if (storeEncryptionAlgorithm.equals(NISTObjectIdentifiers.id_aes256_CCM)) {
          ...
          PBES2Parameters pbeParams = new PBES2Parameters(pbkdAlgId,
              new EncryptionScheme(NISTObjectIdentifiers.id_aes256_CCM,
                  CCMParameters.getInstance(algParams.getEncoded())));

          keyInfo = new EncryptedPrivateKeyInfo(
          ...
          PBES2Parameters pbeParams = new PBES2Parameters(pbkdAlgId,
              new EncryptionScheme(NISTObjectIdentifiers.id_aes256_wrap_pad));
        ...
        entries.put(alias,
            new ObjectData(PRIVATE_KEY, alias, creationDate, lastEditDate, keySeq.getEncoded(),
                null));
      } catch (Exception e) {
        throw new ExtKeyStoreException(
            "BCFKS KeyStore exception storing private key: " + e.toString(), e);
      }
    } else if (key instanceof SecretKey) {
      if (chain != null) {
        throw new KeyStoreException(
            "BCFKS KeyStore cannot store certificate chain with secret key.");
      }
      ...
      try {
        ...
        entries.put(alias,
            new ObjectData(SECRET_KEY, alias, creationDate, lastEditDate, keyData.getEncoded(),
                null));
      } catch (Exception e) {
        throw new ExtKeyStoreException(
            "BCFKS KeyStore exception storing private key: " + e.toString(), e);
  ...
  private EncryptedPrivateKeyData createPrivateKeySequence(
      EncryptedPrivateKeyInfo encryptedPrivateKeyInfo, Certificate[] chain)
      throws CertificateEncodingException {
    com.github.zhenwei.core.asn1.x509.Certificate[] certChain = new com.github.zhenwei.core.asn1.x509.Certificate[chain.length];
    for (int i = 0; i != chain.length; i++) {
      certChain[i] = com.github.zhenwei.core.asn1.x509.Certificate.getInstance(
          chain[i].getEncoded());
    }
  ...
  public void engineSetKeyEntry(String alias, byte[] keyBytes, Certificate[] chain)
    ...
    if (chain != null) {
      ...
      try {
        entries.put(alias,
            new ObjectData(PROTECTED_SECRET_KEY, alias, creationDate, lastEditDate, keyBytes,
                null));
      } catch (Exception e) {
        throw new ExtKeyStoreException(
            "BCFKS KeyStore exception storing protected private key: " + e.toString(), e);
  ...
  public void engineSetCertificateEntry(String alias, Certificate certificate)
    ...
    try {
      entries.put(alias,
          new ObjectData(CERTIFICATE, alias, creationDate, lastEditDate, certificate.getEncoded(),
              null));
    } catch (CertificateEncodingException e) {
      throw new ExtKeyStoreException(
          "BCFKS KeyStore unable to handle certificate: " + e.getMessage(), e);
    }
  ...
  private byte[] generateKey(KeyDerivationFunc pbkdAlgorithm, String purpose, char[] password,
    ...
    if (MiscObjectIdentifiers.id_scrypt.equals(pbkdAlgorithm.getAlgorithm())) {
      ...
      if (pbkdf2Params.getPrf().getAlgorithm().equals(PKCSObjectIdentifiers.id_hmacWithSHA512)) {
      ...
      } else if (pbkdf2Params.getPrf().getAlgorithm()
          .equals(NISTObjectIdentifiers.id_hmacWithSHA3_512)) {
        ...
        throw new IOException(
            "BCFKS KeyStore: unrecognized MAC PBKD PRF: " + pbkdf2Params.getPrf().getAlgorithm());
      }
    } else {
  ...
  private void verifySig(ASN1Encodable store, SignatureCheck integrityCheck, PublicKey key)
    ...
    Signature sig = helper.createSignature(
        integrityCheck.getSignatureAlgorithm().getAlgorithm().getId());
  ...
  private void verifyMac(byte[] content, PbkdMacIntegrityCheck integrityCheck, char[] password)
    ...
    byte[] check = calculateMac(content, integrityCheck.getMacAlgorithm(),
        integrityCheck.getPbkdAlgorithm(), password);
  ...
  private byte[] calculateMac(byte[] content, AlgorithmIdentifier algorithm,
      KeyDerivationFunc pbkdAlgorithm, char[] password)
      throws NoSuchAlgorithmException, IOException, NoSuchProviderException {
    ...
    try {
      ...
      mac.init(new SecretKeySpec(generateKey(pbkdAlgorithm, "INTEGRITY_CHECK",
          ((password != null) ? password : new char[0]), -1), algorithmId));
  ...
  public void engineStore(KeyStore.LoadStoreParameter parameter)
    ...
    if (parameter instanceof BCFKSStoreParameter) {
      ...
      if (bcParam.getStoreSignatureKey() != null) {
        signatureAlgorithm = generateSignatureAlgId(bcParam.getStoreSignatureKey(),
            bcParam.getStoreSignatureAlgorithm());

        hmacPkbdAlgorithm = generatePkbdAlgorithmIdentifier(bcParam.getStorePBKDFConfig(), 512 / 8);

        if (bcParam.getStoreEncryptionAlgorithm()
        ...
        if (bcParam.getStoreMacAlgorithm() == BCFKSLoadStoreParameter.MacAlgorithm.HmacSHA512) {
          hmacAlgorithm = new AlgorithmIdentifier(PKCSObjectIdentifiers.id_hmacWithSHA512,
              DERNull.INSTANCE);
        } else {
          hmacAlgorithm = new AlgorithmIdentifier(NISTObjectIdentifiers.id_hmacWithSHA3_512,
              DERNull.INSTANCE);
        }

        char[] password = ParameterUtil.extractPassword(bcParam);

        EncryptedObjectStoreData encStoreData = getEncryptedObjectStoreData(signatureAlgorithm,
            password);
        ...
        try {
          ...
          if (certs != null) {
            ...
            for (int i = 0; i != certificates.length; i++) {
              certificates[i] = com.github.zhenwei.core.asn1.x509.Certificate.getInstance(
                  certs[i].getEncoded());
            }
          ...
          ObjectStore store = new ObjectStore(encStoreData,
              new ObjectStoreIntegrityCheck(signatureCheck));
  ...
  public void engineStore(OutputStream outputStream, char[] password)
    ...
    if (MiscObjectIdentifiers.id_scrypt.equals(hmacPkbdAlgorithm.getAlgorithm())) {
      ...
      hmacPkbdAlgorithm = generatePkbdAlgorithmIdentifier(hmacPkbdAlgorithm,
          sParams.getKeyLength().intValue());
    } else {
    ...
    ObjectStore store = new ObjectStore(encStoreData, new ObjectStoreIntegrityCheck(
        new PbkdMacIntegrityCheck(hmacAlgorithm, hmacPkbdAlgorithm, mac)));
  ...
  private EncryptedObjectStoreData getEncryptedObjectStoreData(
      AlgorithmIdentifier integrityAlgorithm, char[] password)
      throws IOException, NoSuchAlgorithmException {
    ObjectData[] dataArray = (ObjectData[]) entries.values()
        .toArray(new ObjectData[entries.size()]);

    KeyDerivationFunc pbkdAlgId = generatePkbdAlgorithmIdentifier(hmacPkbdAlgorithm, 256 / 8);
    byte[] keyBytes = generateKey(pbkdAlgId, "STORE_ENCRYPTION",
        ((password != null) ? password : new char[0]), 256 / 8);

    ObjectStoreData storeData = new ObjectStoreData(integrityAlgorithm, creationDate,
        lastModifiedDate, new ObjectDataSequence(dataArray), null);
    ...
    try {
      if (storeEncryptionAlgorithm.equals(NISTObjectIdentifiers.id_aes256_CCM)) {
        ...
        PBES2Parameters pbeParams = new PBES2Parameters(pbkdAlgId,
            new EncryptionScheme(NISTObjectIdentifiers.id_aes256_CCM,
                CCMParameters.getInstance(algorithmParameters.getEncoded())));
  ...
  public void engineLoad(KeyStore.LoadStoreParameter parameter)
    ...
    if (parameter == null) {
      ...
      this.signatureAlgorithm = generateSignatureAlgId(verificationKey,
          bcParam.getStoreSignatureAlgorithm());
  ...
  private boolean isSimilarHmacPbkd(PBKDFConfig storePBKDFConfig,
      KeyDerivationFunc hmacPkbdAlgorithm) {
    ...
    if (MiscObjectIdentifiers.id_scrypt.equals(hmacPkbdAlgorithm.getAlgorithm())) {
      ...
      if (scryptConfig.getSaltLength() != sParams.getSalt().length
          ...
          || scryptConfig.getParallelizationParameter() != sParams.getParallelizationParameter()
          .intValue()) {
        return false;
      }
    } else {
  ...
  public void engineLoad(InputStream inputStream, char[] password)
    ...
    if (integrityCheck.getType() == ObjectStoreIntegrityCheck.PBKD_MAC_CHECK) {
      PbkdMacIntegrityCheck pbkdMacIntegrityCheck = PbkdMacIntegrityCheck.getInstance(
      ...
      try {
        verifyMac(store.getStoreData().toASN1Primitive().getEncoded(), pbkdMacIntegrityCheck,
            password);
      } catch (NoSuchProviderException e) {
        throw new IOException(e.getMessage());
      }
    ...
    if (sData instanceof EncryptedObjectStoreData) {
      ...
      storeData = ObjectStoreData.getInstance(
          decryptData("STORE_ENCRYPTION", protectAlgId, password,
  ...
  private byte[] decryptData(String purpose, AlgorithmIdentifier protectAlgId, char[] password,
      byte[] encryptedData)
      throws IOException {
  ...
  private AlgorithmIdentifier generateSignatureAlgId(Key key,
      BCFKSLoadStoreParameter.SignatureAlgorithm sigAlg)
      throws IOException {
    if (key == null) {
      return null;
    }
    ...
    if (key instanceof RSAKey) {
      if (sigAlg == BCFKSLoadStoreParameter.SignatureAlgorithm.SHA512withRSA) {
        ...
        return new AlgorithmIdentifier(NISTObjectIdentifiers.id_rsassa_pkcs1_v1_5_with_sha3_512,
            DERNull.INSTANCE);
      }
    }
  ...
  private KeyDerivationFunc generatePkbdAlgorithmIdentifier(PBKDFConfig pbkdfConfig,
      int keySizeInBytes) {
    if (MiscObjectIdentifiers.id_scrypt.equals(pbkdfConfig.getAlgorithm())) {
      ...
      ScryptParams params = new ScryptParams(
          ...
          scryptConfig.getParallelizationParameter(), keySizeInBytes);
      ...
      return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2,
          new PBKDF2Params(pbkdSalt, pbkdf2Config.getIterationCount(), keySizeInBytes,
              pbkdf2Config.getPRF()));
    }
  }

  private KeyDerivationFunc generatePkbdAlgorithmIdentifier(KeyDerivationFunc baseAlg,
      int keySizeInBytes) {
    if (MiscObjectIdentifiers.id_scrypt.equals(baseAlg.getAlgorithm())) {
      ...
      ScryptParams params = new ScryptParams(
          ...
          oldParams.getParallelizationParameter(), BigInteger.valueOf(keySizeInBytes));
  ...
  private KeyDerivationFunc generatePkbdAlgorithmIdentifier(
      ASN1ObjectIdentifier derivationAlgorithm, int keySizeInBytes) {
    byte[] pbkdSalt = new byte[512 / 8];
    getDefaultSecureRandom().nextBytes(pbkdSalt);

    if (PKCSObjectIdentifiers.id_PBKDF2.equals(derivationAlgorithm)) {
      return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2,
          new PBKDF2Params(pbkdSalt, 50 * 1024, keySizeInBytes,
  ...
  private static class SharedKeyStoreSpi
    ...
    public Key engineGetKey(
      ...
      } catch (InvalidKeyException e) {   // this should never happen...
        throw new UnrecoverableKeyException(
            "unable to recover key (" + alias + "): " + e.getMessage());
      }
    ...
    private byte[] calculateMac(String alias, char[] password)
      ...
      byte[] encoding;
      if (password != null) {
        encoding = Arrays.concatenate(Strings.toUTF8ByteArray(password),
            Strings.toUTF8ByteArray(alias));
      } else {
