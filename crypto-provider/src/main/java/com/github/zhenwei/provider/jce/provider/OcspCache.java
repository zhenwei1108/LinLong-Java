...
class OcspCache {
  ...
  private static Map<URI, WeakReference<Map<CertID, OCSPResponse>>> cache
      ...
      new WeakHashMap<URI, WeakReference<Map<CertID, OCSPResponse>>>());
    ...
    try {
      ...
      if (OCSPResponseStatus.SUCCESSFUL == response.getResponseStatus().getIntValue()) {
        ...
        if (respBytes.getResponseType().equals(OCSPObjectIdentifiers.id_pkix_ocsp_basic)) {
          ...
          validated = ProvOcspRevocationChecker.validatedOcspResponse(basicResp, parameters, nonce,
              responderCert, helper);
        }

        if (!validated) {
          throw new CertPathValidatorException(
              "OCSP response failed to validate", null, parameters.getCertPath(),
              parameters.getIndex());
        }
