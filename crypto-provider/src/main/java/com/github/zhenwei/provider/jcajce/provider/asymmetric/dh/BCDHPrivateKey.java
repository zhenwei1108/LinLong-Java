...
public class BCDHPrivateKey
  ...
  public BCDHPrivateKey(
    ...
    if (id.equals(PKCSObjectIdentifiers.dhKeyAgreement)) {
      ...
      this.dhSpec = new DHDomainParameterSpec(params.getP(), params.getQ(), params.getG(),
          params.getJ(), 0);
      this.dhPrivateKey = new DHPrivateKeyParameters(x,
    ...
    try {
      ...
      if (dhSpec instanceof DHDomainParameterSpec
        ...
        if (validationParameters != null) {
          vParams = new ValidationParams(validationParameters.getSeed(),
              validationParameters.getCounter());
        }
        info = new PrivateKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.dhpublicnumber,
            new DomainParameters(params.getP(), params.getG(), params.getQ(), params.getJ(),
                vParams).toASN1Primitive()), new ASN1Integer(getX()));
      } else {
        info = new PrivateKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.dhKeyAgreement,
  ...
  DHPrivateKeyParameters engineGetKeyParameters() {
    ...
    return new DHPrivateKeyParameters(x,
        new DHParameters(dhSpec.getP(), dhSpec.getG(), null, dhSpec.getL()));
  }
  ...
  private void readObject(
    ...
    this.dhSpec = new DHParameterSpec((BigInteger) in.readObject(), (BigInteger) in.readObject(),
        in.readInt());
    this.info = null;
