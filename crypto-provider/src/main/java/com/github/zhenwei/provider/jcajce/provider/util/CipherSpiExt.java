...
public abstract class CipherSpiExt
  ...
  /**
   * The operation mode for this cipher ({@link #ENCRYPT_MODE} or {@link #DECRYPT_MODE}).
   */
  ...
  /**
   * Initialize this cipher object with a proper key and some random seed. Before a cipher object is
   * ready for data processing, it has to be initialized according to the desired cryptographic
   * operation, which is specified by the <tt>opMode</tt> parameter.
   * <p>
   * If this cipher (including its underlying mode or padding scheme) requires any random bytes, it
   * will obtain them from <tt>random</tt>.
   * </p><p>
   * Note: If the mode needs an initialization vector, a blank array is used in this case.
   *
   * @param opMode the operation mode ({@link #ENCRYPT_MODE} or {@link #DECRYPT_MODE})
   * @param key    the key
   ...
   * The cipher is initialized for encryption or decryption, depending on the value of
   * <tt>opMode</tt>.
   * <p>
   * If this cipher (including its underlying mode or padding scheme) requires any random bytes, it
   * will obtain them from <tt>random</tt>. Note that when a cipher object is initialized, it loses
   * all previously-acquired state. In other words, initializing a Cipher is equivalent to creating
   * a new instance of that Cipher and initializing it.
   * </p><p>
   * Note: If the mode needs an initialization vector, a try to retrieve it from the
   ...
   * @param random    the random seed
   * @throws InvalidKeyException                if the key is inappropriate for initializing this
   *                                            block cipher.
   * @throws InvalidAlgorithmParameterException if the parameters are inappropriate for initializing
   *                                            this block cipher.
   */
  protected final void engineInit(int opMode, Key key,
   ...
   * wrapping or key unwrapping, depending on the value of opMode. If this cipher (including its
   * underlying feedback or padding scheme) requires any random bytes (e.g., for parameter
   * generation), it will get them from random. Note that when a Cipher object is initialized, it
   * loses all previously-acquired state. In other words, initializing a Cipher is equivalent to
   * creating a new instance of that Cipher and initializing it.
   *
   * @param opMode   the operation mode ({@link #ENCRYPT_MODE} or {@link #DECRYPT_MODE})
   ...
   *                                            parameters and the parameters are null.
   */
  ...
  /**
   * Return the result of the last step of a multi-step en-/decryption operation or the result of a
   * single-step en-/decryption operation by processing the given input data and any remaining
   * buffered data. The data to be processed is given in an input byte array. Beginning at
   * inputOffset, only the first inputLen bytes are en-/decrypted, including any buffered bytes of a
   * previous update operation. If necessary, padding is performed. The result is returned as a
   * output byte array.
   *
   * @param input the byte array holding the data to be processed
   * @param inOff the offset indicating the start position within the input byte array
  ...
  /**
   * Perform the last step of a multi-step en-/decryption operation or a single-step en-/decryption
   * operation by processing the given input data and any remaining buffered data. The data to be
   * processed is given in an input byte array. Beginning at inputOffset, only the first inputLen
   * bytes are en-/decrypted, including any buffered bytes of a previous update operation. If
   * necessary, padding is performed. The result is stored in the given output byte array, beginning
   * at outputOffset. The number of bytes stored in this byte array are returned.
   *
   * @param input  the byte array holding the data to be processed
   * @param inOff  the offset indicating the start position within the input byte array
   * @param inLen  the number of bytes to be processed
   * @param output the byte array for holding the result
   * @param outOff the offset indicating the start position within the output byte array to which
   *               the en/decrypted data is written
  ...
  /**
   * Return the initialization vector. This is useful in the context of password-based encryption or
   * decryption, where the IV is derived from a user-provided passphrase.
   *
   * @return the initialization vector in a new buffer, or <tt>null</tt> if the underlying algorithm
  ...
  /**
   * Return the length in bytes that an output buffer would need to be in order to hold the result
   * of the next update or doFinal operation, given the input length inputLen (in bytes).
   * <p>
   * This call takes into account any unprocessed (buffered) data from a previous update call, and
   * padding.
   * </p><p>
   * The actual output length of the next update or doFinal call may be smaller than the length
   ...
   * <p>
   * The returned parameters may be the same that were used to initialize this cipher, or may
   ...
   * @throws NoSuchAlgorithmException if neither the mode with the given name nor the default mode
   *                                  can be found
   */
  ...
  /**
   * Return the result of the next step of a multi-step en-/decryption operation. The data to be
   * processed is given in an input byte array. Beginning at inputOffset, only the first inputLen
   * bytes are en-/decrypted. The result is returned as a byte array.
   *
   * @param input the byte array holding the data to be processed
   * @param inOff the offset indicating the start position within the input byte array
   * @param inLen the number of bytes to be processed
  ...
  /**
   * Perform the next step of a multi-step en-/decryption operation. The data to be processed is
   * given in an input byte array. Beginning at inputOffset, only the first inputLen bytes are
   * en-/decrypted. The result is stored in the given output byte array, beginning at outputOffset.
   * The number of bytes stored in this output byte array are returned.
   *
   * @param input  the byte array holding the data to be processed
   * @param inOff  the offset indicating the start position within the input byte array
   * @param inLen  the number of bytes to be processed
   ...
   * for encryption.
   * <p>
   * If this cipher requires any algorithm parameters and paramSpec is null, the underlying cipher
   * implementation is supposed to generate the required parameters itself (using provider-specific
   * default or random values) if it is being initialized for encryption, and raise an
   * InvalidAlgorithmParameterException if it is being initialized for decryption. The generated
   * parameters can be retrieved using engineGetParameters or engineGetIV (if the parameter is an
   * IV).
   * </p><p>
   * If this cipher (including its underlying feedback or padding scheme) requires any random bytes
   * (e.g., for parameter generation), it will get them from random.
   * </p><p>
   ...
   * If this cipher requires any algorithm parameters and paramSpec is null, the underlying cipher
   * implementation is supposed to generate the required parameters itself (using provider-specific
   * default or random values) if it is being initialized for encryption, and throw an {@link
   * InvalidAlgorithmParameterException} if it is being initialized for decryption. The generated
   * parameters can be retrieved using engineGetParameters or engineGetIV (if the parameter is an
   * IV).
   * </p><p>
  ...
  /**
   * Continue a multiple-part encryption or decryption operation (depending on how this cipher was
  ...
  /**
   * Finish a multiple-part encryption or decryption operation (depending on how this cipher was
   * initialized).
   *
   * @return a new buffer with the result
   * @throws IllegalBlockSizeException if this cipher is a block cipher and the total input length
   *                                   is not a multiple of the block size (for encryption when no
   *                                   padding is used or for decryption).
