...
public class BCEdDSAPublicKey
  ...
  public byte[] getEncoded() {
    if (eddsaPublicKey instanceof Ed448PublicKeyParameters) {
      byte[] encoding = new byte[KeyFactorySpi.Ed448Prefix.length
      ...
      ((Ed448PublicKeyParameters) eddsaPublicKey).encode(encoding,
          KeyFactorySpi.Ed448Prefix.length);

      return encoding;
    } else {
      byte[] encoding = new byte[KeyFactorySpi.Ed25519Prefix.length
          + Ed25519PublicKeyParameters.KEY_SIZE];

      System.arraycopy(KeyFactorySpi.Ed25519Prefix, 0, encoding, 0,
          KeyFactorySpi.Ed25519Prefix.length);

      ((Ed25519PublicKeyParameters) eddsaPublicKey).encode(encoding,
          KeyFactorySpi.Ed25519Prefix.length);
