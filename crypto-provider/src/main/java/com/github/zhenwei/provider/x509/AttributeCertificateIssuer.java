...
import com.github.zhenwei.core.asn1.x509.V2Form;
 ...
 *
 * @deprecated use com.github.zhenwei.pkix.cert.AttributeCertificateIssuer
 */
      ...
      if (gn.getTagNo() == GeneralName.directoryName) {
        try {
          if (new X500Principal(
              ((ASN1Encodable) gn.getName()).toASN1Primitive().getEncoded()).equals(subject)) {
            return true;
          }
        } catch (IOException e) {
        }
    ...
    if (form instanceof V2Form) {
      ...
      if (issuer.getBaseCertificateID() != null) {
        return issuer.getBaseCertificateID().getSerial().hasValue(x509Cert.getSerialNumber())
            && matchesDN(x509Cert.getIssuerX500Principal(),
