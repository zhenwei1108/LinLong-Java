...
public class BaseBlockCipher
  ...
  private static final Class gcmSpecClass = ClassUtil.loadClass(BaseBlockCipher.class,
  ...
  protected BaseBlockCipher(
      com.github.zhenwei.core.crypto.BlockCipher engine,
      int ivLength) {
  ...
  protected AlgorithmParameters engineGetParameters() {
    if (engineParams == null) {
      if (pbeSpec != null) {
        ...
        if (baseEngine == null) {
          try {
            engineParams = createParametersInstance(
          ...
          try {
            ...
            engineParams.init(
                new GCMParameters(aeadParams.getNonce(), aeadParams.getMacSize() / 8).getEncoded());
          } catch (Exception e) {
            throw new RuntimeException(e.toString());
          }
        }
  ...
  protected void engineSetPadding(
    ...
    } else if (paddingName.equals("WITHCTS") || paddingName.equals("CTSPADDING")
  ...
  protected void engineInit(
    ...
    if (!(key instanceof SecretKey)) {
      throw new InvalidKeyException(
          "Key for algorithm " + ((key != null) ? key.getAlgorithm() : null)
    ...
    //
    if (params == null && (baseEngine != null && baseEngine.getAlgorithmName()
        .startsWith("RC5-64"))) {
    ...
    if (scheme == PKCS12 || key instanceof PKCS12Key) {
      ...
      if (key instanceof BCPBEKey) {
        ...
        if (pbeKeyParam instanceof ParametersWithIV) {
          ...
          param = Util.makePBEParameters(k.getEncoded(), PKCS12, digest, keySizeInBits,
              ivLength * 8, pbeSpec, cipher.getAlgorithmName());
        } else {
          throw new InvalidKeyException("Algorithm requires a PBE key suitable for PKCS12");
        }
      } else {
        param = Util.makePBEParameters(k.getEncoded(), PKCS12, digest, keySizeInBits, ivLength * 8,
      ...
      param = Util.makePBEParameters(k.getEncoded(), PKCS5S1, digest, keySizeInBits, ivLength * 8,
          pbeSpec, cipher.getAlgorithmName());
      if (param instanceof ParametersWithIV) {
        ivParam = (ParametersWithIV) param;
      }
      ...
      param = Util.makePBEParameters(k.getEncoded(), scheme, digest, keySizeInBits, ivLength * 8,
          pbeSpec, cipher.getAlgorithmName());
      if (param instanceof ParametersWithIV) {
        ivParam = (ParametersWithIV) param;
      }
    } else if (!(key instanceof RepeatedSecretKeySpec)) {
      if (scheme == PKCS5S1 || scheme == PKCS5S1_UTF8 || scheme == PKCS5S2
          || scheme == PKCS5S2_UTF8) {
        throw new InvalidKeyException("Algorithm requires a PBE key");
      }
    ...
    if (params instanceof AEADParameterSpec) {
      ...
      param = aeadParams = new AEADParameters(keyParam, aeadSpec.getMacSizeInBits(),
          aeadSpec.getNonce(), aeadSpec.getAssociatedData());
    } else if (params instanceof IvParameterSpec) {
      if (ivLength != 0) {
        IvParameterSpec p = (IvParameterSpec) params;

        if (p.getIV().length != ivLength && !(cipher instanceof AEADGenericBlockCipher)
            && fixedIv) {
      ...
      if (gost28147Param.getIV() != null && ivLength != 0) {
        if (param instanceof ParametersWithIV) {
          param = new ParametersWithIV(((ParametersWithIV) param).getParameters(),
      ...
      param = new RC2Parameters(key.getEncoded(),
      ...
      if (baseEngine.getAlgorithmName().startsWith("RC5")) {
        ...
        throw new InvalidAlgorithmParameterException(
            "RC5 parameters passed to a cipher that is not RC5.");
      }
      ...
      param = new FPEParameters((KeyParameter) param, spec.getRadix(), spec.getTweak(),
  ...
  private boolean isAEADModeName(
    ...
    return "CCM".equals(modeName) || "EAX".equals(modeName) || "GCM".equals(modeName)
        || "GCM-SIV".equals(modeName) || "OCB".equals(modeName);
  }
  ...
  static private interface GenericBlockCipher {
    ...
    public com.github.zhenwei.core.crypto.BlockCipher getUnderlyingCipher();
  ...
  private static class BufferedGenericBlockCipher
    ...
    BufferedGenericBlockCipher(com.github.zhenwei.core.crypto.BlockCipher cipher) {
  ...
  private static class AEADGenericBlockCipher
    ...
    static {
      Class aeadBadTagClass = ClassUtil.loadClass(BaseBlockCipher.class,
