...
public class McElieceCCA2KeyFactorySpi
      ...
      try {
        if (PQCObjectIdentifiers.mcElieceCca2.equals(pki.getAlgorithm().getAlgorithm())) {
          ...
          return new BCMcElieceCCA2PublicKey(
              new McElieceCCA2PublicKeyParameters(key.getN(), key.getT(), key.getG(),
   ...
   * the following key specifications are supported: {@link PKCS8EncodedKeySpec}.
   *
   * @param keySpec the key specification
      ...
      try {
        if (PQCObjectIdentifiers.mcElieceCca2.equals(pki.getPrivateKeyAlgorithm().getAlgorithm())) {
          ...
          return new BCMcElieceCCA2PrivateKey(
              new McElieceCCA2PrivateKeyParameters(key.getN(), key.getK(), key.getField(),
                  key.getGoppaPoly(), key.getP(),
                  Utils.getDigest(key.getDigest()).getAlgorithmName()));
        } else {
   ...
   * keys are supported: {@link BCMcElieceCCA2PrivateKey}, {@link BCMcElieceCCA2PublicKey}.
   *
   * @param key the key
  ...
  public PrivateKey generatePrivate(PrivateKeyInfo pki)
    ...
    return new BCMcElieceCCA2PrivateKey(
        new McElieceCCA2PrivateKeyParameters(key.getN(), key.getK(), key.getField(),
            key.getGoppaPoly(), key.getP(), null));
  }
