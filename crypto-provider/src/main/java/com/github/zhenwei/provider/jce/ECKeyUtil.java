...
/**
 * Utility class to allow conversion of EC key parameters to explicit from named curves and back
 * (where possible).
 */
public class ECKeyUtil {

  /**
   * Convert a passed in public EC key to have explicit parameters. If the key is already using
   * explicit parameters it is returned.
   *
  ...
  public static PublicKey publicToExplicitParameters(PublicKey key, Provider provider)
    ...
    try {
      SubjectPublicKeyInfo info = SubjectPublicKeyInfo.getInstance(
          ASN1Primitive.fromByteArray(key.getEncoded()));
      ...
      if (info.getAlgorithm().getAlgorithm().equals(CryptoProObjectIdentifiers.gostR3410_2001)) {
        ...
        if (params.isNamedCurve()) {
          ...
          curveParams = new X9ECParameters(
              WeGooProvider.CONFIGURATION.getEcImplicitlyCa().getCurve(),
              new X9ECPoint(WeGooProvider.CONFIGURATION.getEcImplicitlyCa().getG(), false),
              WeGooProvider.CONFIGURATION.getEcImplicitlyCa().getN(),
              WeGooProvider.CONFIGURATION.getEcImplicitlyCa().getH());
        } else {
  ...
  /**
   * Convert a passed in private EC key to have explicit parameters. If the key is already using
   * explicit parameters it is returned.
   *
   * @param key          key to be converted
   * @param providerName provider name to be used.
  ...
  public static PrivateKey privateToExplicitParameters(PrivateKey key, Provider provider)
    ...
    try {
      ...
      if (info.getPrivateKeyAlgorithm().getAlgorithm()
