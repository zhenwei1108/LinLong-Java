...
import com.github.zhenwei.core.pqc.asn1.RainbowPublicKey;
...
import com.github.zhenwei.provider.jcajce.spec.RainbowPublicKeySpec;
...
public class RainbowKeyFactorySpi
  ...
  /**
   * Converts, if possible, a key specification into a {@link BCRainbowPrivateKey}. Currently, the
   * following key specifications are supported: {@link RainbowPrivateKeySpec}, {@link
   * PKCS8EncodedKeySpec}.
   * <p>
  ...
  /**
   * Converts, if possible, a key specification into a {@link BCRainbowPublicKey}. Currently, the
   * following key specifications are supported:{@link X509EncodedKeySpec}.
   * <p>
  ...
  /**
   * Converts a given key into a key specification, if possible. Currently the following specs are
   * supported:
   * <ul>
  ...
  /**
   * Translates a key into a form known by the FlexiProvider. Currently the following key types are
   * supported: RainbowPrivateKey, RainbowPublicKey.
   *
   * @param key the key
  ...
  public PrivateKey generatePrivate(PrivateKeyInfo keyInfo)
    ...
    return new BCRainbowPrivateKey(pKey.getInvA1(), pKey.getB1(), pKey.getInvA2(), pKey.getB2(),
        pKey.getVi(), pKey.getLayers());
  }
  ...
  public PublicKey generatePublic(SubjectPublicKeyInfo keyInfo)
    ...
    return new BCRainbowPublicKey(pKey.getDocLength(), pKey.getCoeffQuadratic(),
        pKey.getCoeffSingular(), pKey.getCoeffScalar());
  }
}