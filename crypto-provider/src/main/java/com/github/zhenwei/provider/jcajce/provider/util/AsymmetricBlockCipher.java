...
/**
 * The AsymmetricBlockCipher class extends CipherSpiExt. NOTE: Some Ciphers are using Padding.
 * OneAndZeroesPadding is used as default padding. However padding can still be specified, but mode
 * is not supported; if you try to instantiate the cipher with something else than "NONE" as mode
...
public abstract class AsymmetricBlockCipher
  ...
  /**
   * Return the block size (in bytes). Note: although the ciphers extending this class are not block
   * ciphers, the method was adopted to return the maximal plaintext and ciphertext sizes for non
   * hybrid ciphers. If the cipher is hybrid, it returns 0.
   *
   * @return if the cipher is not a hybrid one the max plain/cipher text size is returned, otherwise
   * 0 is returned
   */
   ...
   * of the next update or doFinal operation, given the input length <tt>inLen</tt> (in bytes). This
   * call takes into account any unprocessed (buffered) data from a previous update call, and
   * padding. The actual output length of the next update() or doFinal() call may be smaller than
   * the length returned by this method.
   * <p>
   * If the input length plus the length of the buffered data exceeds the maximum length, <tt>0</tt>
   * is returned.
   * </p>
   *
   ...
   * underlying cipher implementation is supposed to generate the required parameters itself (using
   * provider-specific default or random values) if it is being initialized for encryption, and
   * raise an InvalidKeyException if it is being initialized for decryption. The generated
  ...
  /**
   * Initialize this cipher for encryption by forwarding it to initEncrypt(Key, FlexiSecureRandom,
   ...
   *                                            this cipher.
   * @throws InvalidAlgorithmParameterException if the given algortihm parameters are inappropriate
   *                                            for this cipher, or if this cipher is being
   *                                            initialized for decryption and requires algorithm
   *                                            parameters and params is null.
   */
  ...
  /**
   * Initialize the cipher for decryption by forwarding it to {@link #initDecrypt(Key,
   * AlgorithmParameterSpec)}.
   * <p>
   * If this cipher requires any algorithm parameters that cannot be derived from the given key, the
   * underlying cipher implementation is supposed to generate the required parameters itself (using
  ...
  /**
   * Continue a multiple-part encryption or decryption operation. This method just writes the input
  ...
  /**
   * Check if the message length plus the length of the input length can be en/decrypted. This
   * method uses the specific values {@link #maxPlainTextSize} and {@link #cipherTextSize} which are
   * set by the implementations. If the input length plus the length of the internal buffer is
   * greater than {@link #maxPlainTextSize} for encryption or not equal to {@link #cipherTextSize}
   * for decryption, an {@link IllegalBlockSizeException} will be thrown.
   *
  ...
  /**
   * Encrypt the message stored in input. The method should also perform an additional length
   * check.
   *
   * @param input the message to be encrypted (usually the message length is less than or equal to
   *              maxPlainTextSize)
  ...
  /**
   * Decrypt the ciphertext stored in input. The method should also perform an additional length
   * check.
   *
   * @param input the ciphertext to be decrypted (the ciphertext length is less than or equal to
   *              maxCipherTextSize)
