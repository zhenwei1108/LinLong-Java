...
import com.github.zhenwei.core.crypto.generators.Ed25519KeyPairGenerator;
import com.github.zhenwei.core.crypto.generators.Ed448KeyPairGenerator;
import com.github.zhenwei.core.crypto.generators.X25519KeyPairGenerator;
import com.github.zhenwei.core.crypto.generators.X448KeyPairGenerator;
import com.github.zhenwei.core.crypto.params.Ed25519KeyGenerationParameters;
import com.github.zhenwei.core.crypto.params.Ed448KeyGenerationParameters;
import com.github.zhenwei.core.crypto.params.X25519KeyGenerationParameters;
import com.github.zhenwei.core.crypto.params.X448KeyGenerationParameters;
import com.github.zhenwei.provider.jcajce.provider.asymmetric.util.ECUtil;
import com.github.zhenwei.provider.jcajce.spec.EdDSAParameterSpec;
import com.github.zhenwei.provider.jcajce.spec.XDHParameterSpec;
...
public class KeyPairGeneratorSpi
  ...
  public KeyPair generateKeyPair() {
    ...
    switch (algorithmInitialized) {
      case Ed25519:
      case Ed448:
        return new KeyPair(new BCEdDSAPublicKey(kp.getPublic()),
            new BCEdDSAPrivateKey(kp.getPrivate()));
      case X25519:
      case X448:
        return new KeyPair(new BCXDHPublicKey(kp.getPublic()),
            new BCXDHPrivateKey(kp.getPrivate()));
      default:
        throw new IllegalStateException("generator not correctly initialized");
    }
  }
  ...
  private static int getAlgorithmForName(String name)
      throws InvalidAlgorithmParameterException {
    if (name.equalsIgnoreCase(XDHParameterSpec.X25519) || name.equals(
        EdECObjectIdentifiers.id_X25519.getId())) {
      return X25519;
    } else if (name.equalsIgnoreCase(EdDSAParameterSpec.Ed25519) || name.equals(
        EdECObjectIdentifiers.id_Ed25519.getId())) {
      return Ed25519;
    } else if (name.equalsIgnoreCase(XDHParameterSpec.X448) || name.equals(
        EdECObjectIdentifiers.id_X448.getId())) {
      return X448;
    } else if (name.equalsIgnoreCase(EdDSAParameterSpec.Ed448) || name.equals(
        EdECObjectIdentifiers.id_Ed448.getId())) {
      return Ed448;
    }
