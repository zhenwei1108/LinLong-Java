...
import com.github.zhenwei.core.pqc.asn1.McEliecePrivateKey;
import com.github.zhenwei.core.pqc.asn1.McEliecePublicKey;
...
public class McElieceKeyFactorySpi
    ...
    if (keySpec instanceof X509EncodedKeySpec) {
      ...
      try {
        if (PQCObjectIdentifiers.mcEliece.equals(pki.getAlgorithm().getAlgorithm())) {
          ...
          return new BCMcEliecePublicKey(
              new McEliecePublicKeyParameters(key.getN(), key.getT(), key.getG()));
        } else {
      ...
      try {
        if (PQCObjectIdentifiers.mcEliece.equals(pki.getPrivateKeyAlgorithm().getAlgorithm())) {
          ...
          return new BCMcEliecePrivateKey(
              new McEliecePrivateKeyParameters(key.getN(), key.getK(), key.getField(),
  ...
  /**
   * Translates a key into a form known by the FlexiProvider. Currently, only the following "source"
   * keys are supported: {@link BCMcEliecePrivateKey}, {@link BCMcEliecePublicKey}.
   *
   * @param key the key
  ...
  public PrivateKey generatePrivate(PrivateKeyInfo pki)
    ...
    return new BCMcEliecePrivateKey(
        new McEliecePrivateKeyParameters(key.getN(), key.getK(), key.getField(), key.getGoppaPoly(),
            key.getP1(), key.getP2(), key.getSInv()));
  }
